...
! this is in fact much more complicated than (1._q,0._q) would expect
...
! loop over all k-points NK1
       DO NK1=1, WDES%NKPTS
! if the k-point has (0._q,0._q) weight we do not need to include it
...
! add to weight of q-point (difference vecor) of this k-point pair
          ...
          ELSE
! strange the weight was (0._q,0._q)
! this can happen if auxiliary k-points with (0._q,0._q) weights are used
...
! (1._q,0._q) that maps onto the supplied k-point VKPT2 when the symmetry operation
...
! for negative b the number of grid points close to (0._q,0._q) is increased
...
! logarithmic with modified behaviour for small y
! this (1._q,0._q) choses B automatically such that the smallest value on
! the grid matches SIGMA (the broadening width)
! as a rule of thumb:
!  (1._q,0._q) gets exact integrals if SIGMA equals the band gap / 5
          OMIN=SIGMA
...
!*********************************************************************
  ...
  SUBROUTINE INTEGRATE_W_2E_SPLINE(SCREENED_TWO_ELECTRON_INTEGRALS, OMEGA, & 
    ...
    LOGICAL :: LSHIFT_BOUNDARY_TO_ZERO           ! set boundary values to (0._q,0._q)
...
!*********************************************************************
  ...
  SUBROUTINE INTEGRATE_W_2E_SIMPLE(SCREENED_TWO_ELECTRON_INTEGRALS, OMEGA, & 
    ...
    REAL(q) :: EPSILON                           ! eigenvalue
    REAL(q) :: ISIGN                             ! sign(epsilon-mu)
    LOGICAL :: LSHIFT_BOUNDARY_TO_ZERO           ! set boundary values to (0._q,0._q), and integrate
...
! select gamma point only for the test calculations
       ...
       IF (W%WDES%WTKPT(NK1)==0) CYCLE ! (0._q,0._q) k-point weighted points are useless right now in the GW code
...
! screened two electron integrals
    ...
    LOGICAL :: LG0W0              ! do not update the position of the poles of the (1._q,0._q) electron Green function
    INTEGER :: IU6                ! stdout
    INTEGER :: IU0                ! stdin
! local
             ...
             IF (S2E%NUMBER_OF_REDUNDANT(NK1, NK2)/=0) THEN
                DO NB2=1,W%WDES%NBANDS
                   ...
                   ELSE
                      ISIGN= 1.0
                   ENDIF
! epsilon (E_n') is the pole of the (1._q,0._q)-electron Green's function
! for state n'
...
! select if (1._q,0._q) or less frequencies along real axis are known
...
! half occupied states should give no contribution use smooth interpolation
                   ...
                   ELSE IF (NOMEGA==2) THEN
! COHSEX
! two frequencies, higher (1._q,0._q) is assumed to be exchange
...
! two frequencies: static COHSEX
    ...
    ENDDO
! update CELTOT
! CELTOT is used to determine the poles in the (1._q,0._q) electron Green function
! if this update is *not 1._q*, the poles of the (1._q,0._q) electron Green function
...
! first  (1._q,0._q) allows to interpolate the value at EDIF [W_INTER(:,1)]
! second (1._q,0._q) allows to determine the derivative at EDIF [W_INTER(:,2)]
!
!**********************************************************************
...
! file
! IERR is (0._q,0._q) if the operation was sucessfull and otherwise
...
! if (1._q,0._q) fails it returns an error
!
!**********************************************************************
...
! if there is only (1._q,0._q) orbital with fraction occupancy, the
