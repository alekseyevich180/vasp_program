...
! elements WOPT(NRES+N1: NRES+ITER, NP)
                   CALL W1_GEMV( (1._q,0._q), ELEMENTS( WOPT, NRES+N1, NRES+ITER, NP),  WTMP(NP), (0._q,0._q), CWORK1, 1)
...
! elements WOPT(N1: ITER, NP)
                   CALL W1_GEMV( (1._q,0._q), ELEMENTS( WOPT, N1, ITER, NP),  ELEMENT( WOPT, N1, NP), (0._q,0._q), CWORK1, 1, CQIJ)
...
! just to be sure merge results from all nodes
                CALL M_bcast_z(WDES%COMM_INB, CHAM, NRES*NRES)
...
! if (1._q,0._q) denotes these wavefunctions as phi_j, and R_j=  (H - e S) phi_j
...
! B(n1) = < phi_n1 |  ( H - e S) | phi_ini >
                CALL W1_GEMV( (1._q,0._q), ELEMENTS( WOPT, NRES+1, NRES+ITER, NP),   &
                                   ELEMENT( W_INI,  NP), (0._q,0._q), B(1), 1)
...
! set CWORK1(1) to < xi | xi >
                   ...
                   DO N1=1,ITER
                      DO N2=1,ITER
                         C=C+CONJG(B(N2))*CTMP(N2,N1)*B(N1)
                      ENDDO
...
! change of the (1._q,0._q)-electron energy is smaller than EBREAK
