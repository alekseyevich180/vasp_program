...
! this handle can be used to quickly extract the (1._q,0._q) center
! occupancies up to a certain main quantum number LMAX
! these (1._q,0._q) center terms are stored in consecutive order
! and can be used to calculate e.g. (1._q,0._q) center corrections
! in MP2
  TYPE one_center_handle
     INTEGER :: LMAX_ONE_CENTER             ! maximum L for (1._q,0._q) center terms
...
! this index is (1._q,0._q) dimensional and given by first_index+ WDES(NT)%LMMAX*second_index
! where first_index and second_index are the usual indices into CRHODE or CDIJ
! like arrays
     COMPLEX(q), POINTER :: POT(:)             ! kernel for four orbital integrals
...
! set number of ions and types to (0._q,0._q) (indicates FAST_AUG is not set up)
    FAST_AUG%NTYP =0
    FAST_AUG%NIONS=0
...
!  and (1._q,0._q) would like to restore the AE charge density accurately
!  on the plane wave grid.
!  This is 1._q by developing the difference between the PS
!  and AE charge density into a set of mutual orthogonal functions
!  with (0._q,0._q) moment l, where l is the angular quantum number
!
!*******************************************************************
...
!
! lower panel 0:LMAX (these are the standard (1._q,0._q) normalized augmentation charges)
...
!
! upper panel: these are the (0._q,0._q) normalized augmentation charges
!
    ...
    DO L=0,LMAX_BESSEL
       IF (QMAX>0) THEN
          ...
          CALL M_exit(); stop
       ENDIF
! find q values (either (1._q,0._q) per function or two
...
!
!  AUG_SETQ_NTQ
!  find a set of (1._q,0._q) or two {q_i} and coefficients {A_i} such that
!     sum_i  A_i j_l(q_i,Rc) =0
!  and condition
!     sum_i  A_i d/d r j_l(q_i,Rc) =0
!  is fulfilled.
!  Furthermore the l moment of the functions is (1._q,0._q)
...
!-----------------------------------------------------------------------
! derivative and r^(2+l) norm (0._q,0._q)
    A(1,1)=0
    A(2,1)=0
...
!-----------------------------------------------------------------------
! norm of squared function should be (1._q,0._q)
...
! screwed if we do not have (0._q,0._q) charge
...
! VERSION I
! fit the Besseltransform of the AE-PS charge density at (1._q,0._q) or
! two selected q values
! this might be changed to a least square fit, but (1._q,0._q) would need
...
!*******************************************************************
!
! the (1._q,0._q) center handle allows to retrieve selected
! (1._q,0._q) center occupancies in order to treat the (1._q,0._q)-center terms
! exactly
!
!*******************************************************************
...
! L is the maximum (1._q,0._q) center angular momentum number
...
! convert index LM+M-1,LMP+MP-1 into (1._q,0._q) dim index
...
! (1._q,0._q) could use this to speed up the code significantly
!-----------------------------------------------------------------------
...
!*******************************************************************
!
! deallocate (1._q,0._q) center handle(WDES%NTYP)
!
!*******************************************************************
...
!  to work out what happens if (1._q,0._q) of the wavefunctions shifts by
!  e^(i G r), and how this changes the corresponding CPROJ
...
!***********************************************************************
  ...
  SUBROUTINE APPLY_PHASE_ONE_CENTER(WDES, H, CRHO, VKPT )
    ...
    COMPLEX(q) :: CRHO(:,:)       ! (1._q,0._q) center charge density for a number of vectors
    REAL(q) :: VKPT(3)
! local
...
!*******************************************************************
!
! apply (1._q,0._q)-center potential to a set of (1._q,0._q) center occupancies
!
!*******************************************************************
  ...
  SUBROUTINE APPLY_ONE_CENTER_H( WDES, H, CRHO, CRHO_TMP, N)
    ...
    TYPE (one_center_handle) :: H
    COMPLEX(q) :: CRHO(:,:)       ! (1._q,0._q) center charge density for a number of vectors
    COMPLEX(q) :: CRHO_TMP(:,:)   ! temporary work array
...
! local
    ...
    DO NT=1,WDES%NTYP
       ...
       DO NI=NIS,WDES%NITYP(NT)+NIS-1
          ...
          CALL ZGEMM('N','N', H%ENTRIES_FOR_TYPE(NT), N, H%ENTRIES_FOR_TYPE(NT), (1._q,0._q), &
               H%POT(NPOT), H%ENTRIES_FOR_TYPE(NT), CRHO(NRHO,1), SIZE(CRHO,1), &
               (0._q,0._q), CRHO_TMP(NRHO,1), SIZE(CRHO,1) )
...
! for efficiency reason (1._q,0._q) should use this subroutine
...
!
! this subroutine calculates  the occupancy matrix for (1._q,0._q) ion
...
! loop over PAW alpha and alpha'
         DO L=1,LMMAXC
            DO LP=1,LMMAXC
               GO_GU_PROJ(1:NRP-NR+1)=GO_R_PROJ(NR:NRP, L+LMBASE)*CONJG(GU_R_PROJ(NR:NRP, LP+LMBASE))
...
!
! this version is very similar but does not include the additional (1._q,0._q)-center
...
!             CPROJ1(L+LMBASE,N,NK)*CONJG(CPROJ2(LP+LMBASE_,N,NK))
!
...
!***********************************************************************
    ...
    SUBROUTINE DEPSUM_TWO_BANDS( CPROJ1, CPROJ2, WDES, ISP, CRHODE, WEIGHT, LOVERL)
      ...
      INTEGER ISP
      LOGICAL LOVERL
      COMPLEX(q)  :: CPROJ1(WDES%NPRO)
      COMPLEX(q)  :: CPROJ2(WDES%NPRO)
      REAL(q)  :: CRHODE(:,:,:,:)
      COMPLEX(q)  :: WEIGHT
! local
...
!OCL NOVREC
               DO LP=1,LMMAXC
                  CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)= CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)+  &
                       CPROJ1(L+LMBASE)*CONJG(CPROJ2(LP+LMBASE_))*WEIGHT
               ENDDO
            ENDDO
...
!
! this subroutine calculates the total charge from two bands
! it also accumulates the (1._q,0._q) center occupancies up to a maximum
! L quantum number
!
!***********************************************************************
    ...
    SUBROUTINE DEPSUM_TWO_BANDS_ONE_CTR_TR( CPROJ1, CPROJ2, WDES, AUG_DES, & 
      ...
      COMPLEX(q)  :: CRHOLM(:)
      TYPE (one_center_handle) :: H
      COMPLEX(q)  :: CRHO_ONE_CENTER(H%TOTAL_ENTRIES)
      REAL(q) :: FACT              ! arbitrary multiplication factor
! local
      INTEGER :: ISPINOR, LMBASE, NT, LMMAXC, NIS, NI, L, LP, LM, NAUG
...
!***********************************************************************
    ...
    SUBROUTINE CALC_DLLMM_VECTOR( GO_R_PROJ, GI_R_PROJ, GWORK, NP, TRANS_MATRIX, LMMAXC, LMBASE, NLM_LMMAX )
      ...
      COMPLEX(q) :: GO_R_PROJ(:,:) ! out:  G'(r', N alpha)
      COMPLEX(q) :: GI_R_PROJ(:,:) ! in:   G(r', N beta)
      COMPLEX(q) :: GWORK(:,:)     ! in:   W(r',NLM)
      INTEGER :: NP        ! number of grid points
      REAL(q) :: TRANS_MATRIX(:, :, :)
      INTEGER :: LMMAXC    ! maximum number of orbital channels
      INTEGER :: LMBASE    ! base index
...
!***********************************************************************
  ...
  SUBROUTINE APPLY_PHASE_ONE_CENTER_NOINT(WDES, H, CRHO, N, VKPT )
    ...
    TYPE (one_center_handle) :: H
    INTEGER :: N
    COMPLEX(q) :: CRHO(H%TOTAL_ENTRIES,N)    ! (1._q,0._q) center charge density for a number of vectors
    REAL(q) :: VKPT(3)
! local
    REAL(q),PARAMETER :: TINY=1E-4_q
    COMPLEX (q) CSHIFT
...
!*******************************************************************
!
! calculate the squared (1._q,0._q) center occupancy matrix
!
!*******************************************************************
  ...
  SUBROUTINE GEMM_ONE_CENTER( LREALSTORE, LREAL, WDES, H, CRHO, CRHOW, N, & 
    ...
    TYPE (one_center_handle) :: H
    COMPLEX(q) :: CRHO(H%TOTAL_ENTRIES,N)  ! (1._q,0._q) center charge density for a number of vectors
    COMPLEX(q) :: CRHOW(H%TOTAL_ENTRIES,N) ! (1._q,0._q) center charge density for a number of vectors times weight
...
! eat up 2 real entries from CRHOW ((1._q,0._q) complex)
...
!
  SUBROUTINE RHOLM_KERNEL( CPROJ1, CPROJ2, TRANS_MATRIX, LD, CRHOLM, FACT, LMMAXC, LMMAX)
    ...
    INTEGER LMMAXC
    COMPLEX(q)  :: CPROJ1(LMMAXC)
    COMPLEX(q)  :: CPROJ2(LMMAXC)
    REAL(q) :: CRHOLM(LMMAX)
    REAL(q) :: FACT              ! arbitrary multiplication factor
! local
# 2675
...
! real part
    CALL DGEMV( 'T' , LD*LD, LMMAX, (1._q,0._q) , TRANS_MATRIX(1,1) , &
                  LD*LD, CTMP(1,1), 2 , (1._q,0._q) ,  CRHOLM(1), 2)
! imaginary part
    CALL DGEMV( 'T' , LD*LD, LMMAX, (1._q,0._q) , TRANS_MATRIX(1,1) , &
                  LD*LD, CTMP(2,1), 2 , (1._q,0._q) ,  CRHOLM(2), 2)
...
!OCL NOVREC
       DO LP=1,LMMAXC
          CTMP(LP,L)=CPROJ1(L)*CONJG(CPROJ2(LP))*FACT
       ENDDO
    ENDDO
  END SUBROUTINE
# 2757
...
!
  SUBROUTINE RHOLM_KERNEL_DGEMM( CPROJ1, CPROJ2, TRANS_MATRIX, LD, CRHOLM, FACT, LMMAXC, LMMAX, NIONS)
    ...
    INTEGER :: NIONS
    REAL(q) :: TRANS_MATRIX(LD* LD, LMMAX)
    COMPLEX(q)    :: CPROJ1(LMMAXC,NIONS)
    COMPLEX(q)    :: CPROJ2(LMMAXC,NIONS)

!   COMPLEX(q)    :: CRHOLM(LMMAX,NIONS)
...
# 2781
    ...
    REAL(q)  :: CTMP(LD, LD, 2*NIONS)
    REAL(q)  :: CRHOLM_(LMMAX, 2*NIONS)
...
!OCL NOVREC
          DO LP=1,LMMAXC
             CM=CPROJ1(L,NI)*CONJG(CPROJ2(LP,NI))*FACT
             CTMP(LP, L, NI)=REAL(CM, KIND=q)

             CTMP(LP, L, NIONS+NI)=AIMAG(CM )
...
# 2808
    ...
    CALL DGEMM( 'T' , 'N', LMMAX, 2*NIONS, LD*LD, (1._q,0._q) , TRANS_MATRIX(1,1) , &
                  LD*LD, CTMP(1,1,1), LD*LD , (0._q,0._q) ,  CRHOLM_(1,1), LMMAX)

! add back to complex array
!   CRHOLM(1:LMMAX,1:NIONS)=CRHOLM(1:LMMAX,1:NIONS)+CMPLX( CRHOLM_(1:LMMAX, 1:NIONS), CRHOLM_(1:LMMAX, NIONS+1:2*NIONS), KIND=q)
    CALL DAXPY(LMMAX*NIONS, (1._q,0._q) ,CRHOLM_(1,1),      1,CRHOLM(1,1),2)
    CALL DAXPY(LMMAX*NIONS, (1._q,0._q) ,CRHOLM_(1,NIONS+1),1,CRHOLM(2,1),2)
...
!
! low level F77 subroutine to calculate RHOLM
! and contracted (1._q,0._q) center occupancy matrix
!
!**********************************************************************
  ...
  SUBROUTINE RHOLM_ONE_CENTER_KERNEL( CPROJ1, CPROJ2, TRANS_MATRIX, LD, & 
    ...
    COMPLEX(q)    :: CRHO_ONE_CENTER(ENTRIES_FOR_TYPE)
    ...
    REAL(q) :: FACT                            ! arbitrary multiplication factor
! local
    COMPLEX(q)  :: CTMP(LMMAXC* LMMAXC), CTMP_
    INTEGER L, LP, LM, I, J
...
!OCL NOVREC
       DO LP=1,LMMAXC
          CTMP_=CPROJ1(L)*CONJG(CPROJ2(LP))*FACT
...
! real part
    CALL DGEMV( 'N' , LD*LD, LMMAX, (1._q,0._q) , TRANS_MATRIX(1,1) , &
                  LD*LD, CDLM(1), 2 , (1._q,0._q) ,  CDIJ(1), 2)
! imaginary part
    CALL DGEMV( 'N' , LD*LD, LMMAX, (1._q,0._q) , TRANS_MATRIX(1,1) , &
                  LD*LD, CDLM(2), 2 , (1._q,0._q) ,  CDIJ(2), 2)
...
!   COMPLEX(q) CDLM(LMMAX,NIONS)
!   COMPLEX(q) CDIJ(LDCDIJ*LDCDIJ,NIONS)
    REAL(q) CDLM(2*LMMAX,NIONS)
    REAL(q) CDIJ(2*LDCDIJ*LDCDIJ,NIONS)

    REAL(q) CDLM_(LMMAX, 2*NIONS)
    REAL(q) CDIJ_(LDCDIJ*LDCDIJ, 2*NIONS)
...
! copy data to real work array
!   CDLM_(1:LMMAX,1:NIONS)        =REAL (CDLM(1:LMMAX,1:NIONS), KIND=q)
!   CDLM_(1:LMMAX,NIONS+1:2*NIONS)=AIMAG(CDLM(1:LMMAX,1:NIONS))
    CALL DCOPY(LMMAX*NIONS,CDLM(1,1),2,CDLM_(1,1),      1)
    CALL DCOPY(LMMAX*NIONS,CDLM(2,1),2,CDLM_(1,NIONS+1),1)

! CDIJ(L)= \sum_LM TRANS_MATRIX(L,LM)*CDLM(LM)
    CALL DGEMM( 'N', 'N', LD*LD, 2*NIONS, LMMAX, (1._q,0._q) , TRANS_MATRIX(1,1), LD*LD, & 
          CDLM_(1,1), LMMAX , (0._q,0._q), CDIJ_(1,1), LDCDIJ*LDCDIJ)

! add back to complex array
!   CDIJ(1:LDCDIJ*LDCDIJ,1:NIONS)=CDIJ(1:LDCDIJ*LDCDIJ,1:NIONS)+ &
!  &   CMPLX( CDIJ_(1:LDCDIJ*LDCDIJ,1:NIONS), CDIJ_(1:LDCDIJ*LDCDIJ,NIONS+1:2*NIONS), KIND=q)
    CALL DAXPY(LDCDIJ*LDCDIJ*NIONS, (1._q,0._q) , CDIJ_(1,1),      1,CDIJ(1,1),2)
    CALL DAXPY(LDCDIJ*LDCDIJ*NIONS, (1._q,0._q) , CDIJ_(1,NIONS+1),1,CDIJ(2,1),2)  
