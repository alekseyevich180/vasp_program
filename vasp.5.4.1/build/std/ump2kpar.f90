...
! structures
    ...
    INTEGER              LMAXMP2     ! maximum L index for (1._q,0._q) center terms
! local
...
130 FORMAT (5X, //, &
    ...
    ENDDO
! set FIRST_EMPTY to (0._q,0._q) to allow calculation of HF energy
    FIRST_EMPTY=1
...
! TWOELECTRON3O(N4, N3, N2, N1, KI2) the (1._q,0._q) with interchanged
...
!**********************************************************************
  ...
  SUBROUTINE ADD_MP2(LATT_CUR, W, K1, N1, ISP1, K2, N2, ISP2, NB1_INTO_TOT, NB2_INTO_TOT, CBMAX, CBMIN, CBMAX4, CBMIN4, &
    ...
    COMPLEX(qs), TARGET :: TWOELECTRON3O(:,:,:,:,:)
! local
    ...
    COMPLEX(qs), POINTER :: TWOELECTRON3O_TRANS(:,:,:,:,:)
             ...
             IF (K1==K4) THEN
                ...
                DO I=1,3
                   INTE_HARTREE=INTE_HARTREE+(1._q/3._q)* &
                       ...
                       &       CONJG(TWOELECTRON3O(N3, N4, N2, N1, KI)+HEAD1(I,I)))
!                  INTE_HARTREE=INTE_HARTREE+ &
!                       ((TWOELECTRON3O(N3, N4, N2, N1, KI))* &
!                      &       CONJG(TWOELECTRON3O(N3, N4, N2, N1, KI)))/3
                ENDDO
             ELSE
                INTE_HARTREE= &
                     ((TWOELECTRON3O(N3, N4, N2, N1, KI))* &
                    &       CONJG(TWOELECTRON3O(N3, N4, N2, N1, KI)))
             ENDIF
             ...
             IF (K4==K1 .OR. K1==K3 .AND. ISP1==ISP2) THEN
                ...
                DO I=1,3
                   INTE_EXCHANGE=INTE_EXCHANGE-(1._q/3._q)* &
                        REAL(CONJG(TWOELECTRON3O(N3, N4, N2, N1, KI)+HEAD1(I,I))* &
                        (TWOELECTRON3O_TRANS(N4, N3, N2, N1, KI2)+HEAD2(I,I)),KIND=q)
                ENDDO
             ELSE
                INTE_EXCHANGE=- &
                     REAL(CONJG(TWOELECTRON3O(N3, N4, N2, N1, KI))* &
...
!**********************************************************************
  ...
  SUBROUTINE TRANSPOSE_MP2(WDES, TWOELECTRON3O_TRANS, TWOELECTRON3O, CBMIN4_, CBMAX4_)
    ...
    COMPLEX(qs) :: TWOELECTRON3O_TRANS(:,:,:,:,:)
    COMPLEX(qs), ALLOCATABLE :: TWOELECTRON3O_TMP(:,:,:,:,:)
...
! local variables
    ...
    COMPLEX(q)      , ALLOCATABLE :: CRHO14(:,:)    ! (1._q,0._q) center charge
    COMPLEX(q)      , ALLOCATABLE :: CRHO23(:,:)    ! (1._q,0._q) center charge
...
!==========================================================================
          ...
          IF (ASSOCIATED(H)) THEN
             CALL ZGEMM('C','N',  NSTRIP1*NSTRIP4, NSTRIP2*NSTRIP3, H%TOTAL_ENTRIES, -(1._q,0._q), &
                  CRHO14(1,1), SIZE(CRHO14,1), &
                  CRHO23(1,1), SIZE(CRHO23,1), &
                  (1._q,0._q) , CHAM(1,1,1,1), SIZE(CHAM,1)*SIZE(CHAM,2))
# 1056

             IF (NCYCLES>1) THEN
                CALL M_cycle_d(WHF%WDES%COMM, CRHO23, 2*H%TOTAL_ENTRIES*NSTRIP2*NSTRIP3)
             ENDIF
...
!              < c_k1-q,n3 c'_k2+q,n4  | W |  v'_k2,n2 v_k1,n1>
                      TWOELECTRON3O( NB3, NB4, N2, N1, KINDEX)=CONJG(CHAM( N1, N4, N2, N3))
# 1105
