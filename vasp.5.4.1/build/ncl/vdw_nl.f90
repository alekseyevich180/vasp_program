...
!spin polarised version of the previous routine
    ...
    REAL(q) DCHARG(GRIDC%RL%NP) !charge density on input and output
    REAL(q) DWORKG(GRIDC%RL%NP) !df/drho input, add the same non-local here
!don't touch this (1._q,0._q) yet
...
!(0._q,0._q) the border components
...
!and thus (1._q,0._q) just needs to perform sin(kr) transformation in principle
...
SUBROUTINE VDW_CALC(DCHARG,DWORK,DWORKG,DWORK4,EXC,GRIDC,LATT_CUR,CINP,DINP,spl, phik, d2phidk2,qs_vdw,qsmin,k_k,stress, ISIF)
  ...
  IMPLICIT NONE
  INTEGER NP
!I guess LATT_CURR is the (1._q,0._q) that we need not OMEGA only
  TYPE (latt) LATT_CUR
  TYPE (grid_3d) GRIDC
  REAL(q) OMEGA
  REAL(q) EXC
...
!REAL(q),parameter :: Zab=-0.8491_q !stored in setexm.inc to use vdw_df2 easily
  ...
  NP=GRIDC%RL%NP 
!  IF (IU0>=0) WRITE(IU0,*)  "MPLWV: ", GRIDC%MPLWV, GRIDC%MPLWV, GRIDC%RC%NP
...
!here we calculate the lda_c potential, and derivatives of q0 wrt density and density gradient
    ...
    dtheadn=0._q
    dtheadgr=0._q
!(1._q,0._q) loop is needed to calculate the potential
    DO alf=1,Nalp
      coef=spl(alp(i),alf,:)
      diff=dq0(i)
