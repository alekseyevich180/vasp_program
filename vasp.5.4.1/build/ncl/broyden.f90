...
! and the (1._q,0._q)-center density for the relaxed-core PAW
...
! copy kinetic energy density
         ...
         ENDIF
! (1._q,0._q) center occupancy matrix
...
! charge density
         DO I=1,GRIDC%RC%NP
            ...
            RMST=RMST+FAKT* CONJG(CHTOT(I,ISP)-CHTOTL(I,ISP))* &
...
!              RMST=RMST+FAKT* CONJG(KINEDEN%TAU(I,ISP)-KINEDEN%TAUL(I,ISP))* &
!    &              (KINEDEN%TAU(I,ISP)-KINEDEN%TAUL(I,ISP))
               RMS_tau=RMS_tau+FAKT* CONJG(KINEDEN%TAU(I,ISP)-KINEDEN%TAUL(I,ISP))* &
     &              (KINEDEN%TAU(I,ISP)-KINEDEN%TAUL(I,ISP))
...
!OCL NOVREC
         DO I=1,GRIDB%RC%NP
            ...
            CWRK3(IP)= (CWRK3(IP)-CWRK4(IP))*SFAKT
            CWRK4(IP)=  CWRK4(IP)*SFAKT
         ENDDO
! kinetic energy density
         IF (LDO_METAGGA().AND.LMIX_TAU()) THEN
            DO I=1,GRIDB%RC%NP
...
! copy occupancies
      RHOLM_LAST=RHOLM
! copy (1._q,0._q)-center densities
      IF (PRESENT(N_RHO_ONE_CENTRE)) THEN
...
! for  the present charge dielectric function)
      ...
      ENDIF
      ENDIF
! usually it makes no sense to proceed if (1._q,0._q) cannot converge after so
...
! is that within 1.5-1.7*GCUT (1._q,0._q) finds already 99.9999...% of all
...
! occur for ultrasoft potentials). Already within GCUT (1._q,0._q) should
...
! dyn_put   puts (1._q,0._q) complex array <data(ndim)> at position <irec> in a
!           list <root>, the list is extened dynamically if required
! dyn_get   retrives (1._q,0._q) complex array data(ndim) from position irec
