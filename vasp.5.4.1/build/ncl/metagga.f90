...
! MBJ related constants and variables
    ...
    COMPLEX(q), ALLOCATABLE, SAVE  :: CMBJ_AUX(:)      ! grid quantity, usually stores C values for MBJ
...
! Methods that are private
    ...
    TYPE tau_handle
       ...
       COMPLEX(q), POINTER     :: TAUC(:)     ! kinetic energy density of the core electrons
    END TYPE tau_handle
!
...
!***********************************************************************
      ...
      COMPLEX(q) , POINTER    :: MU(:,:)     ! soft version of the above

      IF (LCALCMU()) THEN
         ALLOCATE(MU(GRID%MPLWV,WDES%NCDIJ),MUTOT(GRIDC%MPLWV,WDES%NCDIJ))
...
!***********************************************************************
     ...
     &   )
      ...
      COMPLEX(q) SV(GRID%MPLWV,WDES%NCDIJ),DENCOR(GRIDC%RL%NP)
...
!=======================================================================
           FOR=WORK(NG)* AIMAG(CONJG(MUTOT(NG))*CEXPF)
...
!***********************************************************************
     ...
     &   CKINEDEN(GRIDC%MPLWV,ISPIN),CWORK(GRIDC%MPLWV,ISPIN)
      COMPLEX(q) DWGRAD(GRIDC%MPLWV,ISPIN),DHTOT(GRIDC%MPLWV,ISPIN), &
     &   DKINEDEN(GRIDC%MPLWV,ISPIN),DWORK(GRIDC%MPLWV,ISPIN)
              
      COMPLEX(q) DENCOR(GRIDC%RL%NP),TAUC(GRIDC%RL%NP)
      REAL(q) XCSIF(3,3)
     ...
     &   DWORK2(GRIDC%RL%NP,ISPIN),DWORK3(GRIDC%RL%NP,ISPIN),DVC(GRIDC%RL%NP)
      ...
      COMPLEX(q)  DMUWORK(GRIDC%MPLWV,ISPIN)
...
!***********************************************************************
     ...
     &   CKINEDEN(GRIDC%MPLWV,NCDIJ),CWORK(GRIDC%MPLWV,NCDIJ)
      COMPLEX(q) DWGRAD(GRIDC%MPLWV,NCDIJ),DHTOT(GRIDC%MPLWV,NCDIJ), &
     &   DKINEDEN(GRIDC%MPLWV,NCDIJ),DWORK(GRIDC%MPLWV,NCDIJ)
     ...
     &   DWORK2(GRIDC%RL%NP,NCDIJ),DWORK3(GRIDC%RL%NP,NCDIJ),DVC(GRIDC%RL%NP)
      ...
      COMPLEX(q)  DMUWORK(GRIDC%MPLWV,NCDIJ)
...
! Test dumps:
      ...
      TYPE(latt) LATT_CUR
      INTEGER NCDIJ      
      COMPLEX(q) DMUTOT(GRIDC%MPLWV,NCDIJ)
      COMPLEX(q) DKINEDEN(GRIDC%MPLWV,NCDIJ),TAUC(GRIDC%RL%NP)
      REAL(q) DE
! local variables
      INTEGER I
      ...
      IF (NCDIJ==1) THEN
         DO I=1,GRIDC%RL%NP
            DE=DE+DMUTOT(I,1)*CONJG(DKINEDEN(I,1)+TAUC(I))
         ENDDO
      ELSEIF (NCDIJ==2) THEN
         DO I=1,GRIDC%RL%NP
            DE=DE+DMUTOT(I,1)*CONJG(DKINEDEN(I,1)+TAUC(I)/2._q)+ &
           &       DMUTOT(I,2)*CONJG(DKINEDEN(I,2)+TAUC(I)/2._q)
         ENDDO
      ELSEIF (NCDIJ==4) THEN
         DO I=1,GRIDC%RL%NP
            DE=DE+DMUTOT(I,1)*CONJG(DKINEDEN(I,1)+TAUC(I)/2._q)+ &
           &       DMUTOT(I,2)*CONJG(DKINEDEN(I,2))+DMUTOT(I,3)*CONJG(DKINEDEN(I,3))+ &
           &        DMUTOT(I,4)*CONJG((DKINEDEN(I,4)+TAUC(I)/2._q))
         ENDDO
      ELSE
...
# 8395
      ...
      CALL M_sum_z(WDES%COMM_INTER, CW5(1), GRID%RL%NP)
...
# 8412
      ...
      CALL M_sum_z(WDES%COMM_INTER, CW2(1), GRID%RL%NP)
      CALL M_sum_z(WDES%COMM_INTER, CW4(1), GRID%RL%NP)
...
# 8570
      ...
      CALL M_sum_z(WDES%COMM_INTER, CKIN(1), GRID%RL%NP)
      CALL M_sum_z(WDES%COMM_KINTER, CKIN(1), GRID%RL%NP)
