...
! on the logarithmic PAW grid ((1._q,0._q) centre terms)
!
...
! where V_x is the exact exchange potential, c_ab are (1._q,0._q) centre
...
!
!  calculates and stores the PAW (1._q,0._q) centre (on site) Slater energy density e(r)
...
! **********************************************************************************
    ...
    SUBROUTINE CALC_PAW_SLATERPOT(COCC, PP, POTAE_AVERAGE, POTPS_AVERAGE, &
      ...
      COMPLEX(q) :: COCC(:,:), DFOCKAE(:,:), DFOCKPS(:,:)
...
!
!  calculates and stores the PAW (1._q,0._q) centre (on site) EXX-KLI energy density for the
...
!
! determine (1._q,0._q) centre strenght parameter from current local potential
...
!
!  calculates the PAW (1._q,0._q) centre correction terms to the energy density e(r)
!  in the LHF approach
...
! local
      ...
      DO LM1=1,LMMAX
         DO LM2=1,LMMAX
            HF_ENERGY=HF_ENERGY+DFOCK(LM1,LM2)*CONJG(CRHODE(LM1,LM2))
         ENDDO
      ENDDO
...
!   CALC_LHF_ONE_CENTRE
!
! calculate the local Hartree Fock (1._q,0._q) centre correction terms
! DFOCK_LOCAL
...
! **********************************************************************************
    ...
    SUBROUTINE CALC_LHF_ONE_CENTRE(NT, PP, CRHODE, DFOCK_LOCAL, DLOCAL, D_CORE, &
      ...
      INTEGER NT
      TYPE (potcar), POINTER ::  PP
      COMPLEX(q) :: CRHODE(:,:),  DFOCK_LOCAL(:,:), DLOCAL(:,:)
      COMPLEX(q) :: D_CORE
      COMPLEX(q), POINTER :: COCC_ADD(:,:)
      COMPLEX(q), OPTIONAL :: CRHODE_ADD(:,:)
...
! if LCORE is set the pseudo core is therefore added
      LOGICAL LCORE
! (1._q,0._q) centre correction energy density
...
!local
      ...
      COMPLEX(q), ALLOCATABLE :: COCC(:,:), DFOCKAE(:,:),  DFOCKAE_CORE(:,:), &
...
! there is a sign change here, compared to what (1._q,0._q) whould expect
...
!   CALC_OEP_ONE_CENTRE
!
! calculate the OEP (1._q,0._q) centre correction terms
! the (1._q,0._q) centre occupancies are supplied in CRHODE_ADD
! from those the (1._q,0._q) centre charge density is calculated and this is projected
...
! **********************************************************************************
    ...
    SUBROUTINE CALC_OEP_ONE_CENTRE(NT, PP, CRHODE_ADD, DLOCAL)
      ...
      INTEGER NT
      TYPE (potcar), POINTER ::  PP
      COMPLEX(q) :: CRHODE_ADD(:,:), DLOCAL(:,:)
!local
      COMPLEX(q) :: DCORRAE(SIZE(DLOCAL,1),SIZE(DLOCAL,1)), DCORRPS(SIZE(DLOCAL,1),SIZE(DLOCAL,1))
      INTEGER :: RNMAX
...
!*******************************************************************
    ...
    SUBROUTINE UP_DOWN_CORRECTION(D_CORE,D_CORE_UP_DOWN, MAG)
      ...
      COMPLEX(q) :: D_CORE(:)
      COMPLEX(q) :: D_CORE_UP_DOWN(:)
      REAL(q) :: MAG(:)
! local
      REAL(q) :: D(4),ABS_MAG
      COMPLEX(q) C00,C01,C10,C11
...
!*******************************************************************
    ...
    SUBROUTINE SET_DD_LHF(WDES, P , T_INFO, LOVERL, LCORR, LCORE, LMDIM, & 
      ...
      COMPLEX(q)  CDIJ(:,:,:,:)
      COMPLEX(q)  D_CORE(:,:)
      COMPLEX(q)  CRHODE(:,:,:,:)  ! onsite occupancies
      COMPLEX(q), OPTIONAL :: CRHODE_ADD(:,:,:,:)
      REAL(q) :: DOUBLEC_HF
! local variables
...
! variables required to store core wavefunctions
      COMPLEX(q) :: CRHODE_UP_DOWN(LMDIM,LMDIM,2),CRHODE_ADD_UP_DOWN(LMDIM,LMDIM,2), &
           CDIJC_UP_DOWN(LMDIM,LMDIM,2), CDIJ_UP_DOWN(LMDIM,LMDIM,2)
      COMPLEX(q) :: D_CORE_UP_DOWN(2)
      REAL(q) :: MAG(4)
      REAL(q) :: DOUBLEC
...
# 1552
      ...
      CALL M_sum_z(WDES%COMM_INTER, CDIJ, LMDIM*LMDIM*WDES%NIONS*WDES%NCDIJ)
      CALL M_sum_z(WDES%COMM_INTER, CDIJC,LMDIM*LMDIM*WDES%NIONS*WDES%NCDIJ)
...
! local (1._q,0._q) centre potentials instead of the corrections only
!
!*******************************************************************
...
! local variables
      ...
      TYPE (potcar), POINTER ::  PP
! variables required to store core wavefunctions
      COMPLEX(q) :: CRHODE_UP_DOWN(LMDIM,LMDIM,2)
      COMPLEX(q) :: CDIJ(LMDIM,LMDIM), CDIJC(LMDIM, LMDIM)
      COMPLEX(q) :: D_CORE_UP_DOWN(2)
      REAL(q) :: MAG(4)
      REAL(q) :: SCALE_DENSITY
      REAL(q) :: DOUBLEC
...
!*******************************************************************
!
! this routine calculates the (1._q,0._q) center corrections
! to the (1._q,0._q)-center strength parameters
...
! does not seem to work, if (1._q,0._q) restricts the space to valence
...
! local variables
      ...
      COMPLEX(q) CTMP(LMDIM,LMDIM,MAX(2,WDES%NCDIJ))
...
# 1903
      ...
      CALL M_sum_d(WDES%COMM_INTER, CDIJ, LMDIM*LMDIM*WDES%NIONS*NCDIJ*2)
...
!
!  RAD_OEP_POT
!  calculate the (1._q,0._q) center potential corrections from the
