...
!*********************************************************************************
!
! module implements the (1._q,0._q) centre (on site) Hartree Fock (exact exchange)
! routines
...
!
!  MIND: S must be set to (0._q,0._q) before call, otherwise the results are added to S
...
!*********************************************************************************
    ...
    SUBROUTINE COLOUMB_4TERM (W, L, R, CHANNELS, S, LONECENTERMAX)
      ...
      INTEGER :: LONECENTERMAX ! maximum l for pseudo (1._q,0._q) center development
...
! on the plane wave grid and for handling the (1._q,0._q) center terms
! currently the (1._q,0._q) center terms are calculated up to the maximum possible L
! for pseudo and AE contributions
!
! But maybe (1._q,0._q) should truncate at FOCK_LMAXONECENTER (maximum L on plane wave grid)
! or (1._q,0._q) should truncate only the pseudo term at  FOCK_LMAXONECENTER
! the last choice makes a lot of sense for localized electrons
! but does not work well for most systems
!
! Currently for the (1._q,0._q) center terms all possible L are included
...
!        a (1._q,0._q) dimensional array is used here entries
...
! local
      ...
      COMPLEX(q) :: DHARTREE(SIZE(DFOCK,1),SIZE(DFOCK,1))
      INTEGER :: LM1, LM2
      REAL(q) :: HF_ENERGY
