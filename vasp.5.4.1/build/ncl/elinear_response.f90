...
! eigenfunction pairs
! except for setting the corresponding rotation matrix to (0._q,0._q)
! in EDDIAG_LR
! reasoning:
...
!   hence the response into other occupied states is essentially (0._q,0._q)
...
! if degenerated eigenvalue pairs have not been resolved
    ...
    COMPLEX(q),ALLOCATABLE :: CDIJ2(:,:,:,:)! work array
...
! derivative of occupancies is set to (0._q,0._q) since the dielectric
...
# 832
                      ...
                      DO L=1,LMMAXC
                         ENL(NI)=ENL(NI)+CPROJXYZ(LBASE+L,N,I)*CONJG(WXI%CPROJ(LBASE+L,N,NK,ISP))
                      ENDDO
