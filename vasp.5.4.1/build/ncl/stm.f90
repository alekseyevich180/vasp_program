...
! local
    ...
    REAL(q) :: EIGENVAL
    INTEGER :: NBANDS_WRITE       ! how many bands must be written for (1._q,0._q) k-point
...
!=======================================================================
  ...
  SUBROUTINE WRT_IETS(LMDIM, NIONS, NSPINORS, CQIJ, WDES, W)
    ...
    COMPLEX(q)            CQIJ(LMDIM,LMDIM,NIONS,NSPINORS) ! overlap of PP
    TYPE (wavedes)     WDES       ! description of wavefunctions
...
! need to send the projector CPROJ to the I/O node
        ...
        CALL M_bcast_z_from(WDES%COMM,CPROJ,WDES1%NPRO_TOT,NNODE)


! finally we write out the projectors acting on the wavefunction

       IF (NODE_ME==IONODE) WRITE (IU) CPROJ(1:WDES1%NPRO_TOT)  ! OK se NPRO in NIcolas'
