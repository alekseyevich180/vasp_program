...
! threshhold wether a wavevector G^2 is (0._q,0._q) or not
...
!
  TYPE responsefunction
     ...
     COMPLEX(q),POINTER :: RESPONSEONE(:,:)    ! (1._q,0._q) center responsefunction  xi_q(kappa,nu,w)
     REAL(q),POINTER    :: RESPONSEONER(:,:)   ! real valued (1._q,0._q) center responsefunction (LREALSTORE)
     COMPLEX(q),POINTER :: STORAGE(:,:,:)! cache structure: to be added to responsefunction (:,NCACHE_DIM,NOMEGA)
     COMPLEX(q),POINTER :: STRONE(:,:,:) ! cache structure: for (1._q,0._q) center terms
...
! for each cartesian direction (1._q,0._q) column
...
! (1._q,0._q) has 2*NP complex data points (in total 4*NP real data points)
!
!**********************************************************************
...
!   REAL(q) :: TMPR(SIZE(CHI%RESPONSEFUN,1),SIZE(CHI%RESPONSEFUN,1))
    ...
    COMPLEX(q), ALLOCATABLE :: CRHO(:,:,:)            ! (1._q,0._q) center charge
    COMPLEX(q), ALLOCATABLE :: CRHOW(:,:,:)           ! (1._q,0._q) center charge
...
! CDER_BETWEEN_STATE =  <u_k1,n1|- i d/dq_j | u_k1+q,n2>
                IF (ABS(SUM(CHI%VKPT*CHI%VKPT))<G2ZERO) THEN
                   ...
                   DO I=1,3
                   DO J=1,3
                      CHI%HEAD(J,I,NOMEGA)= CHI%HEAD(J,I,NOMEGA)+REAL(CONJG(CDER_BETWEEN_STATE(J))*CDER_BETWEEN_STATE(I),q)*WEIGHT
                   ENDDO
                   ENDDO
...
! complex array with NP entries
                      DO I=1,3
                         CHI%WING(1:NP,I,NOMEGA) =CHI%WING(1:NP,I,NOMEGA) +GCHG(1:NP,N,N2_IND  )*(0.0_q,1.0_q)*CONJG(CDER_BETWEEN_STATE(I))*WEIGHT
                         CHI%CWING(1:NP,I,NOMEGA)=CHI%CWING(1:NP,I,NOMEGA)+CONJG(GCHG(1:NP,N,N2_IND  )*(0.0_q,1.0_q)*CONJG(CDER_BETWEEN_STATE(I)))*WEIGHT
                      ENDDO
                   ENDIF
                ENDIF
             ENDDO
...
! CDER_BETWEEN_STATE =  <u_k1,n1|- i d/dq_j | u_k1+q,n2>
          IF (ABS(SUM(CHI%VKPT*CHI%VKPT))<G2ZERO) THEN
             ...
             IF (NOMEGA1>0) THEN
                DO I=1,3
                   DO J=1,3
                      CHI%HEAD(J,I,NOMEGA1)= CHI%HEAD(J,I,NOMEGA1)+CONJG(CDER_BETWEEN_STATE(J))*CDER_BETWEEN_STATE(I)*WEIGHT*WEIGHT1
                   ENDDO
                ENDDO
! the wing stores the limit G -> 0 (second index towards 0)
! accumulate  1/N (\sum_r' u_1(r') u*_2(r') e -iG'r')*  (\int -ir u_k1,n1(r) u*_k1-q,n2(r))
...
! complex array with NP entries
                   DO I=1,3
                      CHI%WING(1:NP,I,NOMEGA1) =CHI%WING(1:NP,I,NOMEGA1) +GCHG(1:NP)*(0.0_q,1.0_q)*CONJG(CDER_BETWEEN_STATE(I))*WEIGHT*WEIGHT1
                      CHI%CWING(1:NP,I,NOMEGA1)=CHI%CWING(1:NP,I,NOMEGA1)+CONJG(GCHG(1:NP)*(0.0_q,1.0_q)*CONJG(CDER_BETWEEN_STATE(I)))*WEIGHT*WEIGHT1
                   ENDDO
                ENDIF
             ENDIF
             IF (NOMEGA2>0) THEN
                DO I=1,3
                   DO J=1,3
                      CHI%HEAD(J,I,NOMEGA2)= CHI%HEAD(J,I,NOMEGA2)+CONJG(CDER_BETWEEN_STATE(J))*CDER_BETWEEN_STATE(I)*WEIGHT*WEIGHT2
                   ENDDO
                ENDDO
                IF (CHI%LREALSTORE) THEN
                   DO I=1,3
                ...
                ELSE
                   DO I=1,3
                      CHI%WING(1:NP,I,NOMEGA2) =CHI%WING(1:NP,I,NOMEGA2) +GCHG(1:NP)*(0.0_q,1.0_q)*CONJG(CDER_BETWEEN_STATE(I))*WEIGHT*WEIGHT2
                      CHI%CWING(1:NP,I,NOMEGA2)=CHI%CWING(1:NP,I,NOMEGA2)+CONJG(GCHG(1:NP)*(0.0_q,1.0_q)*CONJG(CDER_BETWEEN_STATE(I)))*WEIGHT*WEIGHT2
                   ENDDO
                ENDIF
             ENDIF
          ENDIF
...
! local variables
    ...
    COMPLEX(q), ALLOCATABLE       :: CRHO(:)    ! (1._q,0._q) center charge density
    COMPLEX(q) :: CPHASE(GRIDHF%MPLWV)
    COMPLEX(q), ALLOCATABLE :: CRHOLM(:)        ! augmentation occupancy matrix
    INTEGER :: ierror
    TYPE (wavespin) WHF
...
! we must shift the result by e+iGr
! CPHASE stores the required "shift" e+iGr
! magic number for no phase factor is (0._q,0._q) (phase factor is always e^(i x) hence never 0
    CPHASE(1)=0
    CALL SETPHASE(W%WDES%VKPT(:,NK1)-W%WDES%VKPT(:,NK2)-CHI%VKPT(:), &
         GRIDHF,CPHASE(1),LPHASE)
...
! local variables
    ...
    REAL(q) :: XCSIF(3,3)
    INTEGER :: IRDMAA
    COMPLEX(q) :: CDIJ1(LMDIM,LMDIM,WDES%NIONS, WDES%NCDIJ), CDIJ_HARTREE_ION(LMDIM,LMDIM,WDES%NIONS, WDES%NCDIJ) 
    COMPLEX(q)   :: SV1(GRID%MPLWV,WDES%NCDIJ), SV_HARTREE_ION(GRID%MPLWV,WDES%NCDIJ)
...
! enforce calculation of HF (1._q,0._q) center contributions -> CDIJ1
    LHFCALC_FORCE=.TRUE.
    CALL SET_DD_PAW(WDES, P , T_INFO, INFO%LOVERL, &
...
! (1._q,0._q) could also use the full original Hamiltonian
    ENDIF
...
! COVL(2,1) = <u_2 | d sigma / d E | u_1 >
       ...
       CALL M_sum_z(WDES%COMM_KIN,CTMP(1,1),NB_TOT*NB_TOT)
...
!sitting on a node with kpt NK
       ...
       DO  N=1,NBANDSGW
          DO  NP=N,NBANDSGW
             CHAM(N,NP)=(CHAM(N,NP)+CONJG(CHAM(NP,N)))/2
             CHAM(NP,N)=CONJG(CHAM(N,NP))
          ENDDO
       ENDDO
...
! copy wave function based array (COMPLEX) to COMPLEX(q)
...
# 2248
       ...
       CALL M_bcast_z(COMM_KINTER, SV(1,ISP), GRID%RL%NP)
...
# 2352
       ...
       CALL M_bcast_z(COMM_INTER, SV1(1,ISP), GRID%RL%NP)
       CALL M_bcast_z(COMM_KINTER, SV1(1,ISP), GRID%RL%NP)
...
!    ENDIF
! first determine the (1._q,0._q)-center contributions
    CDIJ1=CDIJ
...
! add (1._q,0._q) center OEP contributions
    CDIJ=CDIJ+CDIJ1
...
! set the phase factors ((0._q,0._q) here)
...
! since (1._q,0._q) complex number allows to store two real numbers
...
! leave as is
    ...
    CHI%NCACHE = 0  ! initialize cache to (0._q,0._q)
...
! to be sure no (1._q,0._q) is still using the shmem segment
...
!
! (1._q,0._q) can then show analytically that
...
! wait on all nodes for (0._q,0._q) setting to finish
       CALL M_barrier(CHI%COMM_SHMEM)
    ELSE
...
! I believe the lock in not necessary, since the never two nodes write to (1._q,0._q) destination
...
!***********************************************************************
!
! small subroutine to copy in real space an COMPLEX(q) array (local potential)
...
! 1. dimension is N1, second (1._q,0._q) N2 and third N3
!
!=======================================================================
