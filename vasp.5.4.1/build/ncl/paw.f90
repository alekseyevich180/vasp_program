...
! local variables
      ...
      COMPLEX(q)  CRHODE(LMDIM,LMDIM,1),CHF(LMDIM,LMDIM,1)
...
! variables required to store core wavefunctions
      ...
      TYPE (potcar),POINTER:: PP
      COMPLEX(q)              :: CRHODE(:,:)   ! valence occupancy
...
!local
      ...
      REAL(q) :: NORM
      COMPLEX(q), ALLOCATABLE :: COCC(:,:),DFOCK(:,:),DHARTREE(:,:)
...
!      COMPLEX(q) COCC(LMDIM,LMDIM,MAX(2,WDES%NCDIJ)),COCC_IM(LMDIM,LMDIM)
      ...
      COMPLEX(q),ALLOCATABLE :: CTMP(:,:,:),CSO(:,:,:),CHF(:,:,:)
      COMPLEX(q),ALLOCATABLE :: COCC(:,:,:),COCC_IM(:,:)
      COMPLEX(q),ALLOCATABLE :: CMETAGGA(:,:,:)
...
# 1924
      ...
      CALL M_sum_d(WDES%COMM_KINTER,CDIJ, LMDIM*LMDIM*WDES%NIONS*NCDIJ*2)
