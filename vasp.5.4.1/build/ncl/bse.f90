...
!             resonant-antiresonant coupling with w=0 ((0._q,0._q) frequency) exact
...
! local
    ...
    COMPLEX(q), ALLOCATABLE :: TWOELECTRON3O(:,:,:,:)   ! resonant-resonant coupling
    COMPLEX(q), ALLOCATABLE :: BTWOELECTRON3O(:,:,:,:)  ! resonant-antiresonant coupling
    TYPE (wavespin) WHF
...
# 244
    ...
    COMPLEX(q), ALLOCATABLE :: AMAT(:,:), BMAT(:,:), COVL(:,:)
    COMPLEX(q), ALLOCATABLE :: AMAT_SCALA(:), BMAT_SCALA(:), DIAG_ASCALA(:)
...
! spin polarized calculation weight for Hartree term is (1._q,0._q)
       AHARTREE=1.0_q
    ELSE
...
# 814
       ...
       CALL M_sum_z(WGW%COMM_INTER, AMAT  , SIZE(AMAT))
       IF (ANTIRES>=2) THEN 
          CALL M_sum_z(WGW%COMM_INTER, BMAT  , SIZE(BMAT))
       ENDIF
...
# 1113
    ...
    INTEGER :: NCV
! local variables
    COMPLEX(q), ALLOCATABLE ::  TWOELECTRON3O_LOCAL(:,:,:,:)
...
! test on NB4_INTO_TOT
                ...
                IF (NCV13> NCV24) THEN
                   AMAT(IROW+(JCOL-1)*DESCSTD(LLD_))= &
                        CONJG(TWOELECTRON3O_LOCAL(N1, NB3, N2, NB4 ))*WHF%WDES%WTKPT(1)
                ENDIF
             ENDDO
          ENDDO
       ENDDO
...
# 1390
    ...
    COMPLEX(q) :: AMAT(:), IDIAG(:)       ! matrix and its diagonal elements
...
! store matrix in transposed form, this allows to change the DGEMM calls in bse_te.F ZGEMM('N', -> ZGEMM('T',
...
! this is 1._q quite analogously to the calculation of the
...
!**********************************************************************
!
! diagonalize BSE matrix using (1._q,0._q) of the many possible calls
!
!**********************************************************************
...
# 2451
       ...
       CALL M_sum_z(WDES%COMM, AMAT(1,1)  , SIZE(AMAT))
...
! only (1._q,0._q) temporary matrix is used
!
!**********************************************************************
...
!  U+ (A+B) -> T
    CALL ZGEMM( 'C','N', NCV, NCV, NCV, (1._q,0._q), & 
         U(1,1), NCV, A(1,1), NCV, (0._q,0._q), T(1,1), NCV)
!  [U+ (A+B)] U -> A
    CALL ZGEMM( 'N','N', NCV, NCV, NCV, (1._q,0._q), & 
         T(1,1), NCV, U(1,1), NCV, (0._q,0._q), A(1,1), NCV)
...
!  U A -> T
    CALL ZGEMM( 'N', 'N', NCV, NCV, NCV, (1._q,0._q), & 
         U(1,1), NCV, A(1,1), NCV, (0._q,0._q), T(1,1), NCV)
!  [U A ] U+ -> A
    CALL ZGEMM( 'N','C', NCV, NCV, NCV, (1._q,0._q), & 
         T(1,1), NCV, U(1,1), NCV, (0._q,0._q), A(1,1), NCV)
...
! furthermore (1._q,0._q) can show the
!  (X+Y) (X-Y)+  = 1
! and
!  X Y+  - Y X+  = 0
! and
...
!  U+ Z -> T2
    CALL ZGEMM( 'C','N', NCV, NCV, NCV, (1._q,0._q), & 
         U(1,1), NCV, Z(1,1), NCV, (0._q,0._q), T(1,1), NCV)
...
!  store Z =(X+Y)= U T1 -> A
    CALL ZGEMM( 'N', 'N', NCV, NCV, NCV, (1._q,0._q), & 
         U(1,1), NCV, T(1,1), NCV, (0._q,0._q), Z(1,1), NCV)
    U=0
...
!  U+ Z -> T2
    CALL ZGEMM( 'C','N', NCV, NCV, NCV, (1._q,0._q), & 
         U(1,1), NCV, Z(1,1), NCV, (0._q,0._q), T2(1,1), NCV)
! T1= SQRT(RB) U+ Z 1/SQRT(R)
    SRB=SQRT(RB)
    SR =1/SQRT(R)
    DO I=1,NCV
...
!  Z =(X+Y)= U T1 -> A
    CALL ZGEMM( 'N', 'N', NCV, NCV, NCV, (1._q,0._q), & 
         U(1,1), NCV, T1(1,1), NCV, (0._q,0._q), Z(1,1), NCV)

!  T1=(X-Y)= U T2 -> A
    CALL ZGEMM( 'N', 'N', NCV, NCV, NCV, (1._q,0._q), & 
         U(1,1), NCV, T2(1,1), NCV, (0._q,0._q), T1(1,1), NCV)

!  test multiply Z x U should be equal unity matrix
! U=T1
! CALL ZGEMM( 'N', 'C', NCV, NCV, NCV, (1._q,0._q), &
!     Z(1,1), NCV, T1(1,1), NCV, (0._q,0._q), T2(1,1), NCV)
!DO I=1,NCV
...
! CALL ZGEMM( 'N', 'C', NCV, NCV, NCV, (1._q,0._q), &
!   Z(1,1), NCV, Z(1,1), NCV, (0._q,0._q), T1(1,1), NCV)
! CALL ZGEMM( 'N', 'C', NCV, NCV, NCV, -(1._q,0._q), &
!   U(1,1), NCV, U(1,1), NCV, (1._q,0._q), T1(1,1), NCV)
! DO I=1,NCV
...
!  U+ (A+B) -> T
    CALL PZGEMM( 'C','N', NCV, NCV, NCV, (1._q,0._q), & 
         U(1), 1, 1, DESCSTD, A(1), 1, 1, DESCSTD, (0._q,0._q), T(1), 1, 1, DESCSTD)
!  [U+ (A+B)] U -> A
    CALL PZGEMM( 'N','N', NCV, NCV, NCV, (1._q,0._q), & 
         T(1), 1, 1, DESCSTD, U(1), 1, 1, DESCSTD, (0._q,0._q), A(1), 1, 1, DESCSTD)
! multiply with sqrt of eigenvalues from left and right
    SR=SQRT(ABS(RB))
...
!  U A -> T
    CALL PZGEMM( 'N', 'N', NCV, NCV, NCV, (1._q,0._q), & 
         U(1), 1, 1, DESCSTD, A(1), 1, 1, DESCSTD, (0._q,0._q), T(1), 1, 1, DESCSTD)
!  [U A ] U+ -> A
    CALL PZGEMM( 'N','C', NCV, NCV, NCV, (1._q,0._q), & 
         T(1), 1, 1, DESCSTD, U(1), 1, 1, DESCSTD, (0._q,0._q), A(1), 1, 1, DESCSTD)
...
!  U+ Z -> T2
    CALL PZGEMM( 'C','N', NCV, NCV, NCV, (1._q,0._q), & 
         U(1), 1, 1, DESCSTD, Z(1), 1, 1, DESCSTD, (0._q,0._q), T(1), 1, 1, DESCSTD)
! T1= SQRT(RB) U+ Z 1/SQRT(R)
    SRB=SQRT(RB)
    SR =1/SQRT(R)
...
!  Z =(X+Y)= U T1 -> A
    CALL PZGEMM( 'N', 'N', NCV, NCV, NCV, (1._q,0._q), & 
         U(1), 1, 1, DESCSTD, T(1), 1, 1, DESCSTD, (0._q,0._q), Z(1), 1, 1, DESCSTD)
    U=0
...
!  U+ Z -> T2
    CALL PZGEMM( 'C','N', NCV, NCV, NCV, (1._q,0._q), & 
         U(1), 1, 1, DESCSTD, Z(1), 1, 1, DESCSTD, (0._q,0._q), T2(1), 1, 1, DESCSTD)
! T1= SQRT(RB) U+ Z 1/SQRT(R)
    SRB=SQRT(RB)
    SR =1/SQRT(R)
...
!  Z =(X+Y)= U T1 -> A
    CALL PZGEMM( 'N', 'N', NCV, NCV, NCV, (1._q,0._q), & 
         U(1), 1, 1, DESCSTD, T1(1), 1, 1, DESCSTD, (0._q,0._q), Z(1), 1, 1, DESCSTD)

!  T1=(X-Y)= U T2 -> A
    CALL PZGEMM( 'N', 'N', NCV, NCV, NCV, (1._q,0._q), & 
         U(1), 1, 1, DESCSTD, T2(1), 1, 1, DESCSTD, (0._q,0._q), T1(1), 1, 1, DESCSTD)

!  test multiply Z x U should be equal unity matrix
! U=T1
! CALL PZGEMM( 'N', 'C', NCV, NCV, NCV, (1._q,0._q), &
!     Z(1), 1, 1, DESCSTD, T1(1), 1, 1, DESCSTD, (0._q,0._q), T2(1), 1, 1, DESCSTD)
!DO I=1,NCV
!   WRITE(77,'(10E20.10)') T2(:,I)
!ENDDO
...
! test calculate X X+ - Y Y+  should be equal unity matrix
! CALL PZGEMM( 'N', 'C', NCV, NCV, NCV, (1._q,0._q), &
!   Z(1), 1, 1, DESCSTD, Z(1), 1, 1, DESCSTD, (0._q,0._q), T1(1), 1, 1, DESCSTD)
! CALL PZGEMM( 'N', 'C', NCV, NCV, NCV, -(1._q,0._q), &
!   U(1), 1, 1, DESCSTD, U(1), 1, 1, DESCSTD, (1._q,0._q), T1(1), 1, 1, DESCSTD)
! DO I=1,NCV
!    WRITE(77,'(10E20.10)') T1(:,I)
! ENDDO
    DEALLOCATE(T1,T2)
