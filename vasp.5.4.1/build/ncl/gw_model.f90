...
MODULE gw_model
  ...
  COMPLEX(q), ALLOCATABLE, SAVE :: RHOMAT(:,:,:)
...
! state dependent
    ...
    IF (W%WDES%COMM_KINTER%NCPU.NE.1) THEN
       CALL M_stop('GW_MODEL_SET_RHOMAT: KPAR>1 not implemented, sorry.')
       CALL M_exit(); stop
    END IF
...
! global
    ...
    COMPLEX(q)          SIGMA(GRID%MPLWV,ISPIN)
...
! charge density rho [a.u.] (1/OMEGA is due to the special storage mode of VASP)
       RHO    = ABS(DENS)*A3BOM
! avoid floating point errors / special handling for (0._q,0._q) charge densities
       RHO    = MAX(RHO,1E-60_q)
! rho^(1/6), rho^(1/3)
