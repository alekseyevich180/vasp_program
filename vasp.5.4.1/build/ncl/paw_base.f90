...
!  RHO(LM) at (1._q,0._q) site (this routine is called from us.F) for
!  direction icart
!
!  RHO(lm,l'm') are the (1._q,0._q) center occupancies for each channel
!
...
! loop over all channels' (lmepsilon)
            DO CH2=1,SIZE(JPAW,1)
               RHOLM(LM)=RHOLM(LM)+ JPAW(CH2, CH1, LM, ICART)*CONJG(RHOLLMM( CH2, CH1 ))
            ENDDO
         ENDDO
      ENDDO
...
!*******************************************************************
    ...
    SUBROUTINE CALC_DLLMM_AVEC( DLLMM, DLM, P, ICART)
      USE pseudo
      IMPLICIT NONE
      COMPLEX(q) DLLMM(:,:)  ! contribution to non-local strength parameter
      COMPLEX(q) DLM(:)      ! local moment of vector field for each LM
      TYPE (potcar) P
...
!
! LCOMPACT = T RHOLM in a compact storage (i.e. no (0._q,0._q) elements
!              inbetween)
...
! between nodes that share (1._q,0._q) DIJ
            DO_LOCAL(NI)= .TRUE.
...
# 1622
       ...
       CALL M_sum_z(WDES%COMM_INTER, CRHODE, SIZE(CRHODE))
