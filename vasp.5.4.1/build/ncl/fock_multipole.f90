...
! used for multipole correction
  COMPLEX(q), SAVE  :: MQ_MAT(10,10)
  COMPLEX(q), SAVE  :: MQ_VEC(10)
  COMPLEX(q), SAVE  :: MQ_VAL(10)
! setting up the correction:
  COMPLEX(q), ALLOCATABLE, SAVE     :: POT_MULTIPOLE_CORR_MQ(:,:)
  COMPLEX(q), ALLOCATABLE, SAVE     :: POT_MULTIPOLE_PROJ(:,:)
  COMPLEX(q),ALLOCATABLE,SAVE :: P_PROJ(:,:), P_PROJ2(:,:)
  COMPLEX(q),POINTER,SAVE     :: PROJ_WEIGHT(:)
  COMPLEX(q) :: POTFAK0 
  COMPLEX(q), SAVE  :: A_MAT(10,10),AH_MAT(10,10)
  COMPLEX(q), PRIVATE, SAVE  :: MONOPOL, DIPOLE(3), QUAD(3,3)
...
!   local variables
    ...
    COMPLEX(q)    :: M, D1, D2, D3, Q11, Q12, Q13, Q22, Q23, Q33
    REAL(q), EXTERNAL :: ERRF
    REAL(q) :: A(3,3),QF1,QF2,QF3
    REAL(q) :: FACTM,RTMP,TSUM
    
    COMPLEX(q)    :: TMP_MAT(10,10),PF_MQ(10,10)
...
!==========================================================================
       ...
       ALLOCATE(GTMP( GRIDHF%MPLWV)) ! GRIDHF%MPLWV is the number of
! complex words required for an FFW work array
! GTMP real values hence  -> 2*
...
! on real space grid GTMP (COMPLEX(q)) is has real storage layout
! for gamma only
...
!          P_PROJ(:,N)=P_PROJ(:,N)*NORM*NORM  ! or just (1._q,0._q) NORM ??? ; line probably not even needed
...
! DIPOLES <--> MONOPOLES/QUADRUPOLES don't mix:
       ...
       DO N=1,10
          DO NP=N,10
             MQ_MAT(N,NP)=0.5_Q*(MQ_MAT(N,NP)+CONJG(MQ_MAT(NP,N)))
             MQ_MAT(NP,N)=CONJG(MQ_MAT(N,NP))
          ENDDO
       ENDDO
...
!                      PROJ_WEIGHT(NP)=FACTM
!                      IF(NP>1) THEN
!                         OV_SUM=OV_SUM+(P_PROJ2(NP,N)*CONJG(P_PROJ2(NP,NG))*FACTM)/POTFAK(NP)
!                      ELSE
!                         OV_SUM=OV_SUM+(P_PROJ2(NP,N)*CONJG(P_PROJ2(NP,NG))*FACTM)/POTFAK0
!                      ENDIF
!                      NP=NP+1
!                   ENDDO
!                ENDDO
...
!               which is admissible as mq_mat(1,1) is now (0._q,0._q)
...
!***********************************************************************
   ...
   SUBROUTINE APPLY_GFAC_MULTIPOLE(GRID, CWORK, POTFAK)
     ...
     DO N=1,10
        DO NP=1,GRID%RC%NP
           TSUM(N)=TSUM(N)+(CWORK(NP)*CONJG(P_PROJ2(NP,N)*PROJ_WEIGHT(NP)))
        ENDDO
     ENDDO
...
!
   SUBROUTINE APPLY_GFAC_MULTIPOLE_SQRT(GRID, CWORK, POTFAK)
     ...
     TSUM=0
     DO N=1,10
        TSUM(N)=TSUM(N)+(CWORK(1)*CONJG(P_PROJ2(1,N)*PROJ_WEIGHT(1)))/SQRT(POTFAK0)
        DO NP=2,GRID%RC%NP
           TSUM(N)=TSUM(N)+(CWORK(NP)*CONJG(P_PROJ2(NP,N)*PROJ_WEIGHT(NP)))/SQRT(POTFAK(NP))
        ENDDO
     ENDDO
...
!           TSUM(N)=TSUM(N)+(CWORK(NP)*CONJG(P_PROJ(NP,N)*PROJ_WEIGHT(NP)))
           TSUM(N)=TSUM(N)+(CWORK(NP)*CONJG(P_PROJ2(NP,N)*PROJ_WEIGHT(NP)))
        ENDDO
     ENDDO
