...
!     QIJ=PP%QION(LP,L)
      ...
      DO M =0,MMAX-1
         ...
         CTMP=W%CPROJ(LMIND,N,NK,ISP)*CONJG(W%CPROJ(LMIND_,N,NK,ISP))*QIJ
...
! Orbital moment: start
      CALL M_sum_d( WDES%COMM_INB,PARAUG_lmom,WDES%ISPIN*T_INFO%NIONP*LDIMP*3)
...
! Orbital moment: start
      CALL M_sum_d(WDES%COMM_INTER, PAR_lmom,SIZE(PAR_lmom))
      CALL M_sum_d(WDES%COMM_KINTER,PAR_lmom,SIZE(PAR_lmom))
# 663
