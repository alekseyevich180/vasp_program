...
! AUG_FOCK is added
     ...
     REAL(q),POINTER :: QDEP_FOCK(:,:,:) ! L-dependent charge with (0._q,0._q) moment on regular grid
...
! quantities defined on radial grid
     ...
     REAL(q),POINTER :: AUG_FOCK(:,:,:)! L-dependent charge with (0._q,0._q) moment on r-grid
...
!-----reciprocal projection operator
        ...
        IF (INFO%NLSPLINE) THEN
           ...
           ENDDO
! possibly this works, but (1._q,0._q) would need to inspect visually the behaviour at small q
...
! use bottom most entry from ENERGY
         P%E(I)=ENERGY(1,LL)
! remove (1._q,0._q) entry from ENERGY
         LN(LL)=LN(LL)-1
!        DO J=LN(LL),1,-1
         DO J=1,LN(LL)
