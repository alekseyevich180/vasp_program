...
! alternatively (1._q,0._q) must guarantee that degenerated initial  states |phi(0)>
...
! when shifts are used
    ...
    COMPLEX(q),ALLOCATABLE :: CDIJ1(:,:,:,:)! derivative of non local strength
    COMPLEX(q),ALLOCATABLE::CRHODE1(:,:,:,:)! derivative of onsite occupancies
    COMPLEX(q),ALLOCATABLE::CTMP(:,:,:,:)   ! temporary
    COMPLEX(q),ALLOCATABLE::CRHODE2(:,:,:)  ! derivative for the ion moved
    COMPLEX(q),ALLOCATABLE   :: SV1(:,:)      ! derivative of soft potential
    COMPLEX(q),ALLOCATABLE   :: SV2(:,:)      ! work array
    COMPLEX(q),ALLOCATABLE:: CHDEN1(:,:) ! 1st order change of pseudo charge
    REAL(q)  RHOLM1(N_MIX_PAW,WDES%NCDIJ)! (1._q,0._q) center occupancy matrix (mixed)
...
!=======================================================================
    ...
    ALLOCATE(CHTOT1(GRIDC%MPLWV,WDES%NCDIJ), &
             ...
             SV1(GRID%MPLWV,WDES%NCDIJ), &
             SV2(GRID%MPLWV,WDES%NCDIJ), &
...
!
!  calculate
!  ) the first order change of the (1._q,0._q)-centre occupancies
...
!=======================================================================
! calculate first derivative of (1._q,0._q) electron occupancies
!=======================================================================
...
! set the (1._q,0._q) center occupancy matrix passed through mixer (RHOLM1)
       CALL SET_RHO_PAW(WDES, P, T_INFO, INFO%LOVERL, WDES%NCDIJ, LMDIM, &
            CRHODE1, RHOLM1)
...
# 929
    ...
    WXI%CPROJ =W0%CPROJ +STEP* W1%CPROJ
...
! and W1%CPROJ the corresponding first order change of the character
...
! the last (1._q,0._q) drops out (a dipole moving in a field gives no contribution)
