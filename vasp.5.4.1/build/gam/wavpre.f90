...
! work arrays
      ...
      REAL(q),ALLOCATABLE :: CPRO0(:),CPRO1(:),CPROL(:)
...
!-----------------------------------------------------------------------
     ...
     &                           (W%GPROJ(M,N,NK,ISP),M=1,WDES%NPRO)
...
! calculate the matrix U(N,M) = <N,Mp>
      ...
      CALL ORTH2( &
     &  W%CPTWFP(1,1,NK,ISP),CPTWFL(1),W%GPROJ(1,1,NK,ISP),CPRO0(1),WDES%NBANDS, &
...
!-----------------------------------------------------------------------
      CALL DGEMM( 'T', 'N', WDES%NBANDS, WDES%NBANDS, WDES%NBANDS, 1._q, CU, WDES%NBANDS, &
     &            CU, WDES%NBANDS, 0._q, CAP, WDES%NBANDS)
!-----------------------------------------------------------------------
...
! caclulate AP(N,M)=A(N,K)*CONJG(U(M,K))
      CALL DGEMM( 'N', 'T', WDES%NBANDS, WDES%NBANDS, WDES%NBANDS, 1._q, CA, WDES%NBANDS, &
     &            CU, WDES%NBANDS, 0._q , CAP, WDES%NBANDS)
!-----------------------------------------------------------------------
...
! CU(N,M)=CONJG(CAP(K,N))*CA(K,M)
      ...
      CALL DGEMM('T', 'N', WDES%NBANDS, WDES%NBANDS, WDES%NBANDS, 1._q, CAP, WDES%NBANDS, &
     &             CA, WDES%NBANDS, 0._q , CU, WDES%NBANDS)
!=======================================================================
...
! read in old wavefunction CPTWFL and rotate it by multiplying with CAP
      ...
      DO N=1,WDES%NBANDS
      READ(PIUDIR,REC=IREC)      ICALL,CELEL,CDCEL1, &
     &  (W%CPTWFP(M,N,NK,ISP),M=1,NPL),(W%GPROJ(M,N,NK,ISP),M=1,WDES%NPRO)
      IREC=IREC+IBLK*2
      ENDDO

      CALL LINCOM('F',W%CPTWFP(:,:,NK,ISP),W%GPROJ(:,:,NK,ISP),CAP(1,1), &
...
!***********************************************************************
      IF (ICALLS>=2) THEN
      CALL LINCOM('F',W%CPTWFP(:,:,NK,ISP),W%GPROJ(:,:,NK,ISP),CU(1,1), &
           WDES%NBANDS,WDES%NBANDS,NPL,WDES%NPRO,WDES%NRPLWV,WDES%NPROD,WDES%NBANDS, &
           W%CPTWFP(:,:,NK,ISP),W%GPROJ(:,:,NK,ISP))
      ENDIF
!=======================================================================
...
!-----------------------------------------------------------------------
      ...
      WFMAGLL=WFMAGL
      IF (LOVERL) THEN
      WFMAG =WFMAG + REAL( DDOT( WDES%NPRO,W%GPROJ(1,N,NK,ISP),1,CPRO0,1) ,KIND=q)
      WFMAGL=WFMAGL+ REAL( DDOT( WDES%NPRO,CPROL            ,1,CPRO0,1) ,KIND=q)
      ENDIF
      ...
      IF (ICALL/=0 .AND.ABS(WFMAG)>0.90_q) THEN
        ...
        ENDDO
        DO M=1,WDES%NPRO
          CPRO0(M)=W%GPROJ (M,N,NK,ISP)-CPROL (M)
        ENDDO
...
!-----------------------------------------------------------------------
      IF (ICALL>=3) THEN
        ...
        ENDDO
        DO M=1,WDES%NPRO
          W%GPROJ (M,N,NK,ISP)=W%GPROJ (M,N,NK,ISP)+ &
     ...
     &      W%CELEN(N,NK,ISP)+PRED%ALPHA*CDCEL0+PRED%BETA*CDCEL1
      ELSE IF (ICALL>=2) THEN
        ...
        ENDDO
        DO M=1,WDES%NPRO
          W%GPROJ (M,N,NK,ISP)=W%GPROJ (M,N,NK,ISP)+CPRO0(M)
        ENDDO
...
!-----------------------------------------------------------------------
!      IF (ICALLS>=2) THEN
!      CALL LINCOM('F',W%CPTWFP(:,:,NK,ISP),W%GPROJ(:,:,NK,ISP),CAP,
!     &      WDES%NBANDS,WDES%NBANDS,NPL,WDES%NPRO,WDES%NRPLWV,WDES%NPROD,WDES%NBANDS,
!     &      W%CPTWFP(:,:,NK,ISP),W%GPROJ(:,:,NK,ISP))
!      ENDIF
  ENDIF pre
