...
! in addition calculation of Fock part is bypassed
    REAL(q), OPTIONAL :: CHAMHF(WDES%NB_TOT,WDES%NB_TOT,WDES%NKPTS,WDES%ISPIN)
...
! work arrays (do max of 16 strips simultaneously)
    ...
    REAL(q),ALLOCATABLE,TARGET::  CHAM(:,:),COVL(:,:) ! Hamiltonian and overlap matrix
...
!  caclulate D |cfin_n> (D = non local strength of PP)
       ...
       DO N=1,NBANDS
          CALL PEAD_ACC_ADD_CPROJ(WNONL%GPROJ(:,N),N,NK,ISP)
       ENDDO

       IF (IFLAG==5) THEN
          CALL OVERL(WDES1, WDES1%LOVERL,LMDIM,CQIJ(1,1,1,ISP), WA%GPROJ(1,1),WOVL%GPROJ(1,1))
       ENDIF
...
! add lower triangle
       ...
       IF (PRESENT(CHAMHF).AND.PRESENT(LFIRST)) THEN
          IF (LFIRST) THEN
             CHAMHF(:,:,NK,ISP)=(CHAMHF(:,:,NK,ISP))-CHAM(:,:)
          ENDIF
...
! use the first line in that case
          ...
          IF ( LJACOBI .AND. IFLAG /=13 ) THEN
             ...
             CALL M_sum_d(WDES%COMM_KIN, CHAM(1,1),NB_TOT*NB_TOT)
             CALL M_sum_d(WDES%COMM_KIN, R , NB_TOT)
...
! 
! CALL M_bcast_d(WDES%COMM, CHAM(1,1), NB_TOT*NB_TOT)
...
! ) calculate onsite (1._q centre) contribution to the Hamilton matrix
!  if LOVERL is .TRUE.
!
...
!
!  is added as well
!  the calling routine must initialize CCORR to 0._q
!
!***********************************************************************
  ...
  SUBROUTINE ONE_CENTER_BETWEEN_STATES(HAMILTONIAN, LATT_CUR, LOVERL, WDES, W, NK, ISP, LMDIM, &
    ...
    REAL(q) ::  CCORR(WDES%NB_TOT,WDES%NB_TOT)
    REAL(q), OPTIONAL  :: SV(WDES%GRID%MPLWV*2,WDES%NCDIJ)   ! local potential
! local
    REAL(q)   , POINTER :: CPROJ_RED(:,:)
    INTEGER NPOS
...
!-----------------------------------------------------------------------
    IF (LOVERL) THEN
       ...
       CALL OVERL(WDES1, .TRUE., LMDIM, CDIJC(1,1,1,ISP), WOVL%GPROJ(1,1), WNONL%GPROJ(1,1))
       
       IF (WDES%DO_REDIS) THEN
          CALL REDIS_PROJ(WDES1, WDES%NBANDS, WNONL%GPROJ(1,1))
          CALL REDIS_PROJ(WDES1, WDES%NBANDS, WOVL%GPROJ (1,1))
       ENDIF
       ...
       DO NPOS=1,WDES%NBANDS,NSTRIP
          ...
          CALL ORTH1('U', &
               W%CPTWFP(1,1,NK,ISP),W%CPTWFP(1,1,NK,ISP),WOVL%GPROJ(1,1), &
...
! deallocation ...
    ...
    CALL M_sum_d(WDES%COMM_KIN,CCORR(1,1),WDES%NB_TOT*WDES%NB_TOT)
...
! diagonal density matrix F (1._q-electron occupancies)
! then diagonalize the resulting matrix and rotate the
! 1._q electron orbitals
!
!***********************************************************************
...
! updated 1._q-electron occupancies
...
! copy data back to work array
       ...
       W_TMP%GPROJ(:,1:WDES%NBANDS,:,:) =W%GPROJ(:,1:WDES%NBANDS,:,:)
...
! copy data back to original array
       ...
       W%GPROJ(:,1:WDES%NBANDS,:,:) =W_TMP%GPROJ(:,1:WDES%NBANDS,:,:)
...
!***********************************************************************
  ...
  SUBROUTINE DUMP_HAM( STRING, WDES, CHAM)
    ...
    TYPE (wavedes)     WDES
    REAL(q) ::  CHAM(WDES%NB_TOT,WDES%NB_TOT)
    INTEGER N1, N2, NPL2
    INTEGER NB_TOT
...
!***********************************************************************
  ...
  SUBROUTINE DUMP_HAM_SINGLE( STRING, WDES, CHAM)
    ...
    CHARACTER (LEN=*) :: STRING
    TYPE (wavedes)     WDES
    REAL(qs) ::  CHAM(WDES%NB_TOT,WDES%NB_TOT)
    INTEGER N1, N2, NPL2
    INTEGER NB_TOT
...
!=======================================================================
  ...
  SUBROUTINE DUMP_HAM_DISTRI( STRING, WDES, CHAM_DISTRI, NB_TOT, DESCA, IU)
    ...
    REAL(q)              :: CHAM_DISTRI(*)    ! distributed matrix
    INTEGER           :: NB_TOT
...
! local
    ...
    CALL M_sum_d(WDES%COMM_KIN, CHAM(1,1), SIZE(CHAM))
...
! normalise the vector
       ...
       WFMAG=DDOT(NPL,CPTWFP(1,N),1,CPTWFP(1,N),1)
       CALL DSCAL(NPL,1/SQRT(WFMAG),CPTWFP(1,N),1)

! now orthogonalise all higher vectors to the
! present vector

       IF (NBANDS/=N ) THEN
          CALL DGEMV( 'T', NPL , NBANDS-N ,1._q , CPTWFP(1,N+1), &
               &             NRPLWV, CPTWFP(1,N), 1 , 0._q ,  CPRO, 1)

          DO I=1,NBANDS
             CPRO(I)=(CPRO(I))
          ENDDO

          CALL DGEMM( 'N', 'T' , NPL , NBANDS-N , 1 , -1._q , &
               &             CPTWFP(1,N), NRPLWV , CPRO , NBANDS , &
               &             1._q , CPTWFP(1,N+1) , NRPLWV )
       ENDIF
    ENDDO
    RETURN
  END SUBROUTINE ORSP
...
!*******************************************************************
  ...
  SUBROUTINE LOCAL_BETWEEN_STATES( HAMILTONIAN, W, LATT_CUR, P, T_INFO, IRDMAX, LMDIM, &
    ...
    INTEGER LMDIM
    REAL(q)       CHAM(W%WDES%NB_TOT,W%WDES%NB_TOT,W%WDES%NKPTS,W%WDES%ISPIN)
! local
    INTEGER ISP, NK
    REAL(q) ::   SV(W%WDES%GRID%MPLWV*2,W%WDES%NCDIJ)   ! local potential
