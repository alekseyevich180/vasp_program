...
!  if only 1._q ion no shift is removed
!*********************************************************************
...
! set here center of BOX
      ...
      DO N=1,NIONS
         ...
         ENDDO
      ENDDO
! uncomment return for 0._q rotation
      RETURN
...
!       D = r x e x r
      ...
      ENDDO
!
! if you want to set to 0._q only for 1._q component
...
!---- set GAMMA to 0._q if line minimization was not sufficient
...
!----- set GAMMA to 0._q for initial steepest descent steps (Robin Hirschl)
...
!            ! 0._q out those forces that are antiparallel
!            IF (PROJ<0) VEL(:,NI)=0
!         ENDDO
...
! 0._q out all velocities and make vel parallel to force
         IF (PROJ<0) THEN
            VEL(:,1:NIONS)=0
         ELSE 
         ...
         VEL=VEL+F
! 1._q peculiarity of the original algorithm was that the center of mass
...
!*********************************************************************
!
! subroutine to set selected force components to 0._q
!
!*********************************************************************
