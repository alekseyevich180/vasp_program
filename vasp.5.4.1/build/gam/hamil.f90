       ...
       DO ISPINOR_=0,WDES1%NRSPINORS-1
          ...
          DO NT=1,WDES1%NTYP
             ...
             DO NI=NIS,WDES1%NITYP(NT)+NIS-1
                CALL ECCP_NL(LMDIM,LMMAXC,CDIJ(1,1,NI,1+ISPINOR_+2*ISPINOR),W1%GPROJ(NPRO_+1),W2%GPROJ(NPRO+1),CNL)
                NPRO = LMMAXC+NPRO
                NPRO_= LMMAXC+NPRO_
             ENDDO
310          NIS = NIS+WDES1%NITYP(NT)
    ...
    CE=REAL(CLOCAL+CKIN+CNL,KIND=q)
...
!***********************************************************************
  ...
  SUBROUTINE ECCP_VEC(WDES1,W1,W2,LMDIM,CDIJ,GRID,SV,AVEC, CE)
    ...
    INTEGER NGVECTOR, ISPINOR
    REAL(q)      CNL
    REAL(q)   SV(GRID%MPLWV*2,WDES1%NRSPINORS*WDES1%NRSPINORS)
    REAL(q)   AVEC(:,:)
...
!***********************************************************************
  ...
  SUBROUTINE ECCP_TAU(WDES1,W1,W2,LMDIM,CDIJ,GRID,SV,LATT_CUR,MU,CE)
    ...
    REAL(q)      CNL
    REAL(q)   SV(GRID%MPLWV*2,WDES1%NRSPINORS*WDES1%NRSPINORS)
    REAL(q)   MU(GRID%MPLWV*2,WDES1%NRSPINORS*WDES1%NRSPINORS)
...
!PK MRG_AUX already zeros contributions from non-kpoint nodes, but be cautious and 0._q workspace anyway
             DO NB=1,WDES%NBANDS
                W%AUX(NB,NK,ISP)=0.
             END DO
          ELSE
          ...
          DO NT=1,WDES1%NTYP
             ...
             DO NI=NIS,WDES1%NITYP(NT)+NIS-1
                CALL ECCP_NL_(SIZE(CDIJ,1),LMMAXC, &
                     ...
                     W1%GPROJ(NPRO_+1),W2%GPROJ(NPRO+1),CE)
                NPRO = LMMAXC+NPRO
                NPRO_= LMMAXC+NPRO_
             ENDDO
310          NIS = NIS+WDES1%NITYP(NT)
       ...
       DO ISPINOR_=0,WDES1%NRSPINORS-1
          ...
          DO NT=1,WDES1%NTYP
             ...
             DO NI=NIS,WDES1%NITYP(NT)+NIS-1
                CALL ECCP_NL_(SIZE(CDIJ,1),LMMAXC, &
                     CDIJ(1,1,NI,1+ISPINOR_+2*ISPINOR),CQIJ(1,1,NI,1+ISPINOR_+2*ISPINOR),EVALUE, &
                     W1%GPROJ(NPRO_+1),W2%GPROJ(NPRO+1),CE(NI))
                NPRO = LMMAXC+NPRO
                NPRO_= LMMAXC+NPRO_
             ENDDO
310          NIS = NIS+WDES1%NITYP(NT)
...
!OCL NOVREL
   ...
   SUBROUTINE VHAMIL_TRACE(WDES1, GRID, SV, CR, CVR, WEIGHT)
     ...
     REAL(q)   SV(GRID%MPLWV*2) ! local potential
     COMPLEX(q) :: CR(GRID%MPLWV*WDES1%NRSPINORS),CVR(GRID%MPLWV*WDES1%NRSPINORS)
     REAL(q)    :: WEIGHT
! local variables
     INTEGER ISPINOR, M, MM
...
!
! this version yields an REAL(q) array spinor density
!
   ...
   SUBROUTINE PW_CHARGE(WDES1,  CHARGE, NDIM, CR1, CR2, WEIGHT)
     ...
     TYPE (wavedes1)    WDES1
     INTEGER NDIM
     REAL(q)   CHARGE(NDIM,WDES1%NRSPINORS*WDES1%NRSPINORS)
...
!
! this version yields a REAL(q) charge array, spinor density
...
! results array is REAL(q) (used by exchange routines)
   SUBROUTINE PW_CHARGE_TRACE(WDES1,  CHARGE, CR1, CR2)
     USE prec
     USE mgrid
     USE wave
...
!
! this version traces of spinors to yield only the density
! result array as well as orbital input is REAL(q)
   ...
   SUBROUTINE PW_CHARGE_TRACE_GDEF(WDES1,  CHARGE, CR1, CR2)
     ...
     TYPE (wavedes1)    WDES1
     REAL(q) :: CHARGE(WDES1%GRID%MPLWV)
     REAL(q) :: CR1(WDES1%GRID%MPLWV*WDES1%NRSPINORS),CR2(WDES1%GRID%MPLWV*WDES1%NRSPINORS)
! local variables
     REAL(q) RINPLW
     INTEGER M,MM
...
!***********************************************************************
   ...
   SUBROUTINE KINHAMIL_VEC( WDES1, GRID, CVR,  LADD, & 
     ...
     REAL(q)      :: AVEC(GRID%MPLWV*2,3)     ! magnetic vector potential
     COMPLEX(q) :: CWORK1(WDES1%NRPLWV)
...
!***********************************************************************
   ...
   SUBROUTINE KINHAMIL_TAU( WDES1, GRID, CVR, LADD, LKINETIC, DATAKE, & 
     ...
     INTEGER    :: IGY(WDES1%NGDIM)                ! component of each G vector in direction of second rec lattice vector
     INTEGER    :: IGZ(WDES1%NGDIM)                ! component of each G vector in direction of third rec lattice vector
     REAL(q)      :: MU(GRID%MPLWV*2*WDES1%NRSPINORS*WDES1%NRSPINORS) ! \mu = d f_xc / d \tau
     REAL(q)    :: EVALUE                          ! subtract EVALUE*wavefunction
     COMPLEX(q) :: CPTWFP(WDES1%NRPLWV)                ! wavefunction
     COMPLEX(q) :: CH(WDES1%NRPLWV)                ! result
     LOGICAL    :: LKINETIC
...
!OCL NOVREL
           DO M=1,GRID%RL%NP
              MM =M+ISPINOR *WDES1%GRID%MPLWV
              MM_=M+(ISPINOR_+2*ISPINOR)*GRID%MPLWV*2
...
!OCL NOVREL
        DO M=1,WDES1%NGVECTOR
           ...
           DCMU=DCMU-CWORK1(M)*GX*CITPI*(CPTWFP(MM))
        ENDDO
...
!OCL NOVREL
        DO M=1,WDES1%NGVECTOR
           ...
           DCMU=DCMU-CWORK1(M)*GY*CITPI*(CPTWFP(MM))
        ENDDO
...
!OCL NOVREL
        DO M=1,WDES1%NGVECTOR
           ...
           DCMU=DCMU-CWORK1(M)*GZ*CITPI*(CPTWFP(MM))
        ENDDO
     ENDDO
...
!***********************************************************************
   ...
   SUBROUTINE PSCURRENT( WDES1, GRID, CR, & 
     ...
     REAL(q)      :: C(GRID%RL%NP)                   ! charge density
! local
     INTEGER ISPINOR, M, MM
...
!***********************************************************************
  ...
  SUBROUTINE ECCP_NL_(LMDIM,LMMAXC,CDIJ,CQIJ,EVALUE,CPROJ1,CPROJ2,CNL)
    ...
    COMPLEX(q)  CNL
    INTEGER LMMAXC, LMDIM
    REAL(q) CDIJ(LMDIM,LMDIM),CQIJ(LMDIM,LMDIM)
    REAL(q) EVALUE
...
! local
    ...
    DO L=1,LMMAXC
       DO LP=1,LMMAXC
          CNL=CNL+(CDIJ(LP,L)-EVALUE*CQIJ(LP,L))*CPROJ1(LP)*(CPROJ2(L))
       ENDDO
    ENDDO
  END SUBROUTINE ECCP_NL_
