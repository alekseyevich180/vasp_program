...
!***********************************************************************
     ...
     &   T_INFO,DYN,SYMM,P,NONL_S,NONLR_S,LMDIM,CDIJ,CQIJ,SV,E,INFO,IO)
      ...
      REAL(q)   SV(GRID%MPLWV*2,W%WDES%NCDIJ)
...
! local variables
      ...
      REAL(qs), ALLOCATABLE ::  RPHI_CPROJ(:,:,:,:,:)
...
!***********************************************************************
     ...
     &   LMDIM,CDIJ,CQIJ,SV,E,INFO,IO,BDIR,RPHI)
      ...
      REAL(q) :: S_BARE(W%WDES%GRID%RL%NP,3)   ! three component vector storing current density in real space,
...
!           CALL SPHER_DER(GRID,NONL_D,P,W%WDES,LATT_CUR,VDIR)
         ...
         qloop: DO IQ=-CHI_BARE_STENCIL,CHI_BARE_STENCIL
            ...
            WGKPQ%CPTWFP=(0._q,0._q); WGKPQ%GPROJ=0._q; WXI%CPTWFP=(0._q,0._q); WXI%GPROJ=0._q
...
! the sign of \hat{B}_bdir x \hat{q}_idir is given by the Levi-Civita symbol
            WGKPQ%CPTWFP=EIJK(VDIR,BDIR,IDIR)*WGKPQ%CPTWFP; WGKPQ%GPROJ=EIJK(VDIR,BDIR,IDIR)*WGKPQ%GPROJ

! mupltiply with the sign of q
            WGKPQ%CPTWFP=WGKPQ%CPTWFP*IQ; WGKPQ%GPROJ=WGKPQ%GPROJ*IQ
...
! here 1._q has to divide by q, both soft and 1._q centre
...
! here 1._q has to add j_bare,Q_R and j_dp,Q_R
...
! sum contributions to 1._q-centre magnetic fields along BDIR
...
! write out 1._q column of the tensor, should maybe go elsewhere
...
!     W0_tmp%WDES%DATAKE=>W0%WDES%DATAKE(:,:,NKPTS_ORIG+1:W0%WDES%NKPTS)
      ...
      W0_tmp%GPROJ=>W0_tmp%GPROJ(:,:,NKPTS_ORIG+1:W0%WDES%NKPTS,:)
...
!***********************************************************************
     ...
     & W0,GRID,NONL_S,NONLR_S,NONL_D,NONLR_D,LATT_CUR,LMDIM,CQIJ,IO,INFO,IDIR,WXI,W1)
      ...
      TYPE (latt) LATT_CUR
      INTEGER LMDIM
      REAL(q) CQIJ(LMDIM,LMDIM,W0%WDES%NIONS,W0%WDES%NCDIJ)
      INTEGER IDIR ! component of velocity operator 1=x, 2=y, 3=z
! local variables
...
! local variables
      ...
      ALLOCATE(CH(W0%WDES%NRPLWV,NSIM),GC(W0%WDES%NRPLWV),GPROJ(W0%WDES%NPROD,NSIM))
...
! store in WXI
            DO NP=1,NSIM
               ...
               DO ISPINOR=0,WDES1%NRSPINORS-1
                  ...
                  ENDDO
! we store  i< p_j,k | r_idir | u_nk >  in WXI%GPROJ
                  WXI%GPROJ(:,N,NK,ISP)=W0_1(NP)%GPROJ*(0._q,1._q)
               ENDDO             
            ENDDO
         ENDDO bands
...
! local variables
      ...
      ALLOCATE(CH(W0%WDES%NRPLWV,NSIM),GPROJ(W0%WDES%NPROD,NSIM))
            ...
            DO N=1,NSTRIP_ACT
               ...
               DO ISPINOR=0,WDES1%NRSPINORS-1
                  DO M=1,WDES1%NGVECTOR
                     ...
                     COVL=COVL+(WTMP(N)%CPTWFP(MM))*WXI%CPTWFP(MM,NB,NK,ISP)
                  ENDDO
               ENDDO               
! test
...
! calculate COVL(i,j)=<phi(0)_i|phi(1)_j>
      COVL=0._q
      IF (NPL/=0) THEN
!     CALL DGEMM('C','N',NB_TOT,NB_TOT,2* NPL,1._q,CW0_RED(1,1),2* NRPLWV_RED,CW1_RED(1,1),2* NRPLWV_RED,0._q,COVL(1,1),NB_TOT)
!     NSTRPB=NSTRIP
      NSTRPB=NB_TOT
      DO NPOSB1=1,NB_TOT,NSTRPB; DO NPOSB2=1,NB_TOT,NSTRPB
      CALL DGEMM('C','N',MIN(NSTRPB,NB_TOT-NPOSB2+1),MIN(NSTRPB,NB_TOT-NPOSB1+1),2* NPL,1._q,CW0_RED(1,NPOSB2),2* NRPLWV_RED,CW1_RED(1,NPOSB1),2* NRPLWV_RED,0._q,COVL(NPOSB2,NPOSB1),NB_TOT)
      ENDDO; ENDDO
      ENDIF

      CALL M_sum_d(W0%WDES%COMM_KIN,COVL(1,1),NB_TOT*NB_TOT)

! test
!     IF (W0%WDES%COMM%IONODE==W0%WDES%COMM%NODE_ME) THEN
!        DO N=1,W0%WDES%NB_TOT
!           IF (W0%FERTOT(N,NK,ISP)<1E-4_q) COVL(N,:)=0._q
...
! subtract S|phi(0)_j><phi(0)_j|phi(1)_i> from |phi(1)_i>
!     CALL DGEMM('N','N',2* NPL,NB_TOT,NB_TOT,-1._q,CH_RED(1,1),2* NRPLWV_RED,COVL(1,1),NB_TOT,1._q,CW1_RED(1,1),2* NRPLWV_RED)
!     NSTRPL=2* NPL
!     DO NPOSPL=1,2* NPL,NSTRPL
!     CALL DGEMM('N','N',MIN(NSTRPL,2* NPL-NPOSPL+1),NB_TOT,NB_TOT,-1._q,CH_RED(NPOSPL,1),2* NRPLWV_RED,COVL(1,1),NB_TOT,1._q,CW1_RED(NPOSPL,1),2* NRPLWV_RED)
!     ENDDO
      IF (NPL/=0) THEN
      NSTRPB=NB_TOT; NSTRPL=2* NPL
      DO NPOSB1=1,NB_TOT,NSTRPB; DO NPOSPL=1,2* NPL,NSTRPL
      CALL DGEMM('N','N',MIN(NSTRPL,2* NPL-NPOSPL+1),MIN(NSTRPB,NB_TOT-NPOSB1+1),NB_TOT,-1._q,CH_RED(NPOSPL,1),2* NRPLWV_RED,COVL(1,NPOSB1),NB_TOT,1._q,CW1_RED(NPOSPL,NPOSB1),2* NRPLWV_RED)
      ENDDO; ENDDO
      ENDIF
...
!     NSTRPB=NSTRIP
      ...
      IF (NPL/=0) THEN
      DO NPOSB1=1,NB_TOT,NSTRPB; DO NPOSB2=1,NB_TOT,NSTRPB
      CALL DGEMM('C','N',MIN(NSTRPB,NB_TOT-NPOSB2+1),MIN(NSTRPB,NB_TOT-NPOSB1+1),2* NPL,1._q,CH_RED(1,NPOSB2),2* NRPLWV_RED,CW1_RED(1,NPOSB1),2* NRPLWV_RED,0._q,COVL(NPOSB2,NPOSB1),NB_TOT)
      ENDDO; ENDDO
      ENDIF
...
!     ENDDO
      ...
      IF (NPL/=0) THEN
      DO NPOSB1=1,NB_TOT,NSTRPB; DO NPOSPL=1,2* NPL,NSTRPL
      CALL DGEMM('N','N',MIN(NSTRPL,2* NPL-NPOSPL+1),MIN(NSTRPB,NB_TOT-NPOSB1+1),NB_TOT,-1._q,CW0_RED(NPOSPL,1),2* NRPLWV_RED,COVL(1,NPOSB1),NB_TOT,1._q,CW1_RED(NPOSPL,NPOSB1),2* NRPLWV_RED)
      ENDDO; ENDDO
      ENDIF
...
! calculate COVL(i,j)=<phi(0)_i|WXI_j>
      ...
      NSTRPB=NB_TOT
      DO NPOSB1=1,NB_TOT,NSTRPB; DO NPOSB2=1,NB_TOT,NSTRPB
      CALL DGEMM('C','N',MIN(NSTRPB,NB_TOT-NPOSB2+1),MIN(NSTRPB,NB_TOT-NPOSB1+1),2* NPL,1._q,CW0_RED(1,NPOSB2),2* NRPLWV_RED,CWX_RED(1,NPOSB1),2* NRPLWV_RED,0._q,COVL(NPOSB2,NPOSB1),NB_TOT)
      ENDDO; ENDDO
      ENDIF
...
!***********************************************************************
      ...
      IMPLICIT NONE
      TYPE (wavespin) W,WP
      REAL(q) :: S_BARE(W%WDES%GRID%RL%NP,3) 
! local variables
      ...
      kpoint: DO NK=1,WP%WDES%NKPTS
         ...
         CALL SETWDES(WP%WDES,WDESP1,NK)  ! but this 1._q is shifted with q
...
!***********************************************************************************
      ...
      COMPLEX(q) :: CRHODE(:,:)         ! 1._q center occupancy matrix
      REAL(q) :: B_OUT(3)
      INTEGER :: NIP,NI,BDIR
...
! note that 1._q can save 4 lines above
                        ENDDO
                     ENDIF
                     ENDIF
!-----------------------------------------------------------------------
...
! make 1._q-centre contributions to field real
        B_OUT(1)=REAL(BF(1),q)
        B_OUT(2)=REAL(BF(2),q)
        B_OUT(3)=REAL(BF(3),q)
...
!
! this subroutine calculates  the total 1._q center (on site) "occupancy"
...
!OCL NOVREC
        DO LP=1,LMMAXC
           CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)=CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)+ &
              WEIGHT*W_RIGHT%GPROJ(L+LMBASE,N,NK,ISP)*(W_LEFT%GPROJ(LP+LMBASE_,N,NK,ISP))
        ENDDO
        ENDDO
...
!OCL NOVREC
                           DO LP=1,LMMAXC
                              CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)=CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)+ &
                                   WEIGHT*W1_RIGHT%GPROJ(L+LMBASE)*(W1_LEFT%GPROJ(LP+LMBASE_))
                           ENDDO
                        ENDDO
...
!***********************************************************************************
      ...
      REAL(q) :: JVEC_IN(WDES%GRID%RL%NP,3)
      INTEGER IU0, IU6
...
! now merge the current from all nodes
         ...
         CALL M_sum_d(WDES%COMM_INTER, JVEC_IN(1,I), WDES%GRID%RL%NP)
         CALL M_sum_d(WDES%COMM_KINTER,JVEC_IN(1,I), WDES%GRID%RL%NP)
# 4016
...
! we assume both states consist of the same set of G-vectors
      ...
      REAL(q)      :: J(GRID%RL%NP,3)   ! three component vector storing current density in real space
