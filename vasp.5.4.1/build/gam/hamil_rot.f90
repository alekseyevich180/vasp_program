...
! 1._q wavefunction and a set of wavefunctions
!   H_n' n = <C_n | H | C_n'>
!***********************************************************************
      ...
      REAL(q)       CHAM(WDES%NBANDS,WDES%NBANDS)
      COMPLEX(q) CACC(WDES%NRPLWV)
! local work arrays
      TYPE (wavedes1)    WDES1
      REAL(q)       DWORK(WDES%NBANDS)
      REAL(q)       CPROF(WDES%NPROD)
...
!-----------------------------------------------------------------------
      DO ISPINOR=0,WDES1%NRSPINORS-1
         ...
         DO M=1,NGVECTOR
            ...
            CACC(MM)=(CACC(MM)+W%CPTWFP(MM,N,NK,ISP)* WDES1%DATAKE(M,ISPINOR+1))
         ENDDO
      ENDDO
!=======================================================================
...
!OCL NOVREC
          DO LP=1,LMMAXC
            CPROF(L+NPRO)=CPROF(L+NPRO)+ (CDIJ(LP,L,NI,ISPINOR_+2*ISPINOR+ISP)*W%GPROJ(LP+NPRO_,N,NK,ISP))
          ENDDO; ENDDO
          NPRO = LMMAXC+NPRO
          NPRO_= LMMAXC+NPRO_
        ENDDO
...
!=======================================================================
      CALL DGEMV( 'T',2* NPL , WDES%NBANDS-N+1 ,1._q ,W%CPTWFP(1,N,NK,ISP), &
     &             2* WDES%NRPLWV, CACC, 1 , 0._q ,  DWORK, 1)

      CALL DGEMV( 'T',  WDES%NPRO, WDES%NBANDS-N+1 ,1._q ,W%GPROJ(1,N,NK,ISP), &
     &             WDES%NPROD, CPROF, 1 , 1._q ,  DWORK, 1)

      N1=N
      DO N2=N1,WDES%NBANDS
        CHAM(N2,N1)= REAL(DWORK(N2-N1+1),KIND=q)
        CHAM(N1,N2)= REAL((DWORK(N2-N1+1)),KIND=q)
      ENDDO
...
! space CR and the projection operators must be stored in GPROJ(..,N)
...
!***********************************************************************
      ...
      COMPLEX(q) CACC(WDES%NRPLWV)
! work arrays
      REAL(q)       CWORK(WDES%NBANDS)

      REAL(q)       CWORK2(WDES%NPROD),CWORK3(WDES%NPROD)
...
!-----------------------------------------------------------------------
      CALL DGEMM( 'N', 'N' ,2* NPL , 1 , WDES%NBANDS , -1._q, &
     &             W%CPTWFP(1,1,NK,ISP),2* WDES%NRPLWV , CWORK , WDES%NBANDS , &
     &             1._q , CACC ,2* WDES%NRPLWV )

      CALL DGEMM( 'N', 'N' , WDES%NPRO , 1 , WDES%NBANDS  , -1._q, &
     &             W%GPROJ(1,1,NK,ISP),  WDES%NPROD , CWORK , WDES%NBANDS , &
     &             0._q , CWORK3 , WDES%NPROD  )
...
!-----------------------------------------------------------------------
      ...
      DO NT=1,WDES1%NTYP
        ...
        DO NI=NIS,WDES1%NITYP(NT)+NIS-1
          ...
          DO L =1,LMMAXC
          DO LP=1,LMMAXC
           CWORK2(L+NPRO)=CWORK2(L+NPRO)+CDIJ(LP,L,NI,ISPINOR_+2*ISPINOR+ISP)*W%GPROJ (LP+NPRO_,N,NK,ISP) &
