...
! 1. dimension is N1 second 1._q N2
!
!=======================================================================
...
!=======================================================================
      ...
      SUBROUTINE DIS_PROJ(WDES1,GPROJ,CPROJ_LOCAL)
      USE prec
      USE wave
      IMPLICIT NONE
      ...
      REAL(q) CPROJ_LOCAL(WDES1%NPRO)

      INTEGER NIS,NPRO,NT,LMMAXC,NPRO_POS,L,NI,NPRO_TOT
!
! quick copy if possible
      IF (WDES1%COMM_INB%NCPU==1) THEN
         CPROJ_LOCAL(1:WDES1%NPRO)=GPROJ(1:WDES1%NPRO)
         RETURN
      ENDIF

      CALL M_sum_d(WDES1%COMM_INB, GPROJ, WDES1%NPRO_TOT)
      ...
      DO NT=1,WDES1%NTYP
        ...
        IF (LMMAXC==0) GOTO 100
          ...
          DO L=1,LMMAXC
             CPROJ_LOCAL(L+NPRO)=GPROJ(L+NPRO_POS)
          ENDDO
          NPRO= LMMAXC+NPRO
          ENDDO
 100    NIS = NIS+WDES1%NITYP(NT)
      ...
      IF (WDES1%LNONCOLLINEAR) THEN
         ...
         DO NT=1,WDES1%NTYP
            ...
            NPRO_POS=WDES1%NPRO_POS(NI)
            DO L=1,LMMAXC
               CPROJ_LOCAL(L+NPRO)=GPROJ(L+NPRO_POS+NPRO_TOT)
            ENDDO
            NPRO= LMMAXC+NPRO
            ENDDO
...
!=======================================================================
      ...
      SUBROUTINE MRG_PROJ(WDES1,GPROJ,CPROJ_LOCAL)
      USE prec
      USE wave
      IMPLICIT NONE
...
! quick copy if possible
      IF (WDES1%COMM_INB%NCPU==1) THEN
         GPROJ(1:WDES1%NPRO)=CPROJ_LOCAL(1:WDES1%NPRO)
         RETURN
      ENDIF

      GPROJ(1:WDES1%NPRO_TOT)=0
      ...
      DO NT=1,WDES1%NTYP
        ...
        IF (LMMAXC==0) GOTO 100
          ...
          NPRO_POS=WDES1%NPRO_POS(NI)
          DO L=1,LMMAXC
            GPROJ(L+NPRO_POS)=CPROJ_LOCAL(L+NPRO)
          ENDDO;
          NPRO= LMMAXC+NPRO
          ENDDO
 100    NIS = NIS+WDES1%NITYP(NT)
      ...
      IF (WDES1%LNONCOLLINEAR) THEN
         ...
         DO NT=1,WDES1%NTYP
            ...
            NPRO_POS=WDES1%NPRO_POS(NI)
            DO L=1,LMMAXC
               GPROJ(L+NPRO_POS+NPRO_TOT)=CPROJ_LOCAL(L+NPRO)
            ENDDO
            NPRO= LMMAXC+NPRO
            ENDDO
 200        NIS = NIS+WDES1%NITYP(NT)
...
!=======================================================================
!  distribute plane wave part of 1._q wavefunction over all COMM_INB nodes
...
!=======================================================================
! distribute 1._q specific band to nodes
...
!
! first 0._q out all components from other nodes
!
...
! possibly we have to search for it
      ...
      IF (H%BAND(IND) /= BANDINDEX) THEN
         ...
         ENDIF
      ENDIF
! fill in 0._q into the band index array
      H%BAND(IND)=0
...
!***********************************************************************
      ...
      SUBROUTINE REDIS_PROJ(WDES1, NBANDS, GPROJ)
      USE prec
      USE wave
      IMPLICIT NONE
      ...
      TYPE (wavedes1)  WDES1
      INTEGER NBANDS
      REAL(q) :: GPROJ(WDES1%NPROD*NBANDS)
# 1026
...
# 1052
      ...
      DO NB=0,NBANDS-1
        ...
        CALL M_alltoall_d(WDES1%COMM_INTER, NPROD, GPROJ(NB*NPROD+1), CWORK(1))
# 1060

        DO N=1,NPROD
          GPROJ(NB*NPROD+N)=CWORK(N)
        ENDDO
      ENDDO
# 1067
      ...
      IF (WDES%COMM_KINTER%NCPU.GT.1) THEN
         ...
         DO ISP=1,WDES%ISPIN
            DO K=1,WDES%NKPTS
               ...
               CALL M_bcast_d_from(WDES%COMM_KINTER, W%GPROJ(1,1,K,ISP),SIZE(W%GPROJ,1)*SIZE(W%GPROJ,2),inode)
# 1137
