...
! gamma only go from seriel layout to parallel layout if required
          CALL DIS_PW_BAND_GAMMA(WDES1, J, CW1, W%CPTWFP(1,1,K,ISP), INDEX(1), LCONJG(1))
# 375
...
! write eigenvalues in real format
       ...
       DO J=1,WDES%NB_TOT
         ...
         CALL MRG_PW_BAND_GAMMA(WDES1, J, CPTWFP, W%CPTWFP(1,1,K,ISP), INDEX(1), LCONJG(1))
# 784
...
! that 1._q according to new positions
!=======================================================================
...
! copy that to the right place and initialise everything else to 0
 ...
 2000 CONTINUE
! 1._q centre corrections are passed through mixer and added now
...
! if you do not care, 1._q may simply supply -1 in the file
...
! only 1._q core must call the routine
!
! READGAMMA may be either called by all cores, or better
! called only by 1._q core with a subsequent broadcast of the
! data to all cores
!
!***********************************************************************
      ...
      SUBROUTINE WRITEGAMMA( NK, NB_TOT, NBDIM, CHAM , LROT)
        ...
        LOGICAL :: LROT  ! 1._q can also supply an approximate unitary rotation matrix
