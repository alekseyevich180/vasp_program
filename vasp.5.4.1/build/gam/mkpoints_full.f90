...
!only  KPOINTS_F
     ...
     INTEGER NKPTS_NON_ZERO          ! number of k-points with non 0._q weight
...
!
  TYPE rotation_handle
     ...
     INTEGER :: NK                   ! k-points to which 1._q needs to rotate
...
!*************************************************************************
  ...
  INTERFACE  
     SUBROUTINE ROTATE_VECTOR_ADD(LINV,VEC,ROTVEC,MMAX,LMAX,SL,P,FAKT)
       ...
       REAL(qs):: VEC                         ! initial vector
       REAL(q) :: ROTVEC                      ! final vector
...
!=======================================================================
      ...
      DO NK=1,NKPT
         ...
         K_TMP%RSSYMOP(1,2,NK)=0._q
         K_TMP%RSSYMOP(2,1,NK)=0._q
...
! generating operation. By the way, check whether inversion is already 1._q
! of the sym ops.
!=======================================================================
 inversion: IF (.NOT. LTIME_INVERSION) THEN
      LINV=.FALSE.
      DO NK=1,NKPT
! 0._q weight k-points are not included in exchange kernel
...
! 0._q weight k-points are excluded
! since they are most likely manually entered into the k-point list
! and then 1._q would need to adjust their weight
...
! allocate arrays and set some values to 0._q
      NGX=GRID%NGX
      NGY=GRID%NGY
      NGZ=GRID%NGZ
...
! 0._q weight k-points are again excluded
...
! found this mark it so no need to do this 1._q again
...
!*********************************************************************
!
! subroutine to rotate the wavefunction character (W%GPROJ)
...
! setup index array NPRO_NI into globally summed GPROJ array
! this array is compatible to seriel version
    ...
    CALL MRG_PROJ(WDES1,GPROJ(1),W%GPROJ)
...
# 1593
             ...
             CALL ROTATE_VECTOR( KPOINTS_FULL%LINV(WDES1%NK),GPROJ(NPROP),W_NEW%GPROJ(NPRO), & 
    ...
    CALL ROTATE_WAVE_CHARACTER_SPIN(WDES1, W_NEW%GPROJ, KPOINTS_FULL%RSSYMOP(:,:,WDES1%NK) )
...
!  from 1._q k point in the IRBZ to another k'=S k and performs an FFT
...
!********************** SUBROUTINE ROTATE_WAVE     *********************
!
! rotate a single wavefunction from 1._q k-points to another 1._q
! two routines are available
! the second 1._q adds the results
!
!***********************************************************************
...
! from 1._q q-point to another
! according the VASP convention the second index transforms like G+q
! whereas the the first 1._q transforms -(G+q)
...
!
! rotate a non-local potential or Hamilton matrix
! from 1._q q-point in the BZ back into the IRZ
...
!***********************************************************************
!
! subroutine to rotate 1._q vector VEC (storing the wavefunction
