...
! FTOD_OC Stores the 1._q center part of the two-orbital response functions at q
...
!FTOD_OC(H%TOTAL_ENTRIES,NBANDS,VBMAX,NKPTS,NKPTS(KQ),ISPIN)
      ...
      REAL(q)      , ALLOCATABLE, PRIVATE, SAVE :: FTOD_OC(:,:,:,:,:,:,:)      
      REAL(q) , ALLOCATABLE, PRIVATE, SAVE :: TWOE4ORBITAL(:,:)
      REAL(q) , ALLOCATABLE, PRIVATE, SAVE :: TWOE4ORBITAL_X(:,:)
...
# 216
                        ...
                        CALL PDGEMM('T','n',(PROCS*WDES%NBANDS),(PROCS*WDES%NBANDS),&
                          2* NGVECTOR,-1._q, FTOD_PW(1,1,NBI,KI,KQ,ISP,1),1,1,&
                          desc_FTOD_PW,FTOD_PW(1,1,NBJ,KJ,KQ,ISP,ncc),1,1,&
                          desc_FTOD_PW,0._q, TWOE4ORBITAL(1,1),1,1,desc_TWOE4ORBITAL)
# 228

                        IF (ASSOCIATED(H)) THEN

                           CALL PDGEMM('T','n',(PROCS*WDES%NBANDS),(PROCS*WDES%NBANDS),&
                          NHVECTOR,-1._q, FTOD_OC(1,1,NBI,KI,KQ,ISP,1),1,1,&
                          desc_FTOD_OC,FTOD_OC(1,1,NBJ,KJ,KQ,ISP,2),1,1,&
                          desc_FTOD_OC,1._q, TWOE4ORBITAL(1,1),1,1,desc_TWOE4ORBITAL)
# 240

                        ENDIF
                        TWOE4ORBITAL=(TWOE4ORBITAL*SCALE)
...
! for the case that k_q=k_q' (k_a-k_i=k_b-k_i)
                        IF (KQ==KQ_) THEN
                           ...
                           CALL PDTRAN(PROCS*WDES%NBANDS,PROCS*WDES%NBANDS,1._q,&
                             TWOE4ORBITAL(1,1),1,1,desc_TWOE4ORBITAL,0._q,&
...
! <ba|ij>
                        IF (KQ/=KQ_) THEN
                           ...
                           CALL PDGEMM('T','n',(PROCS*WDES%NBANDS),&
                             (PROCS*WDES%NBANDS),&
                             2* NGVECTOR,-1._q, FTOD_PW(1,1,NBJ,KJ,KQ_,ISP,ncc),1,1, &
                             desc_FTOD_PW,FTOD_PW(1,1,NBI,KI,KQ_,ISP,1),1,1,&
                             desc_FTOD_PW,0._q, TWOE4ORBITAL_X(1,1),1,1,&
                             desc_TWOE4ORBITAL_X)
# 278
                           ...
                           IF (ASSOCIATED(H)) THEN
                              ...
                              CALL PDGEMM('T','n',(PROCS*WDES%NBANDS),(PROCS*WDES%NBANDS),&
                               NHVECTOR,-1._q, FTOD_OC(1,1,NBJ,KJ,KQ_,ISP,2),1,1,&
                               desc_FTOD_OC,FTOD_OC(1,1,NBI,KI,KQ_,ISP,1),1,1,&
                               desc_FTOD_OC,1._q, TWOE4ORBITAL_X(1,1),1,1,desc_TWOE4ORBITAL_X)
# 290
...
!***********************************************************************
      ...
      SUBROUTINE ADD_MP2_wb(W,KI,KJ,KA,KB,NI,NJ,ISP1,ISP2,LATT_CUR,FSG)  
         ...
         REAL(q) :: CDER_BETWEEN_STATE_IA(3),CDER_BETWEEN_STATE_JB(3)
         REAL(q) :: CDER_BETWEEN_STATE_IB(3),CDER_BETWEEN_STATE_JA(3)
...
!IF ((W%WDES%ISPIN==2) .AND. (KA==KB) .AND. (RNA==RNB)) CYCLE
                  tmp=0._q
                     ...
                     HEAD1=0._q
                     Inte_c=0._q
                     CDER_BETWEEN_STATE_IA(:)=0._q
                     CDER_BETWEEN_STATE_JB(:)=0._q
                     IF (KI==KA) THEN
...
!k-points.
                        ...
                        DO i=1,3
                           Inte_c=Inte_c+(TWOE4ORBITAL(NA,NB)+HEAD1(i,i))*((TWOE4ORBITAL(NA,NB)+HEAD1(i,i)))*(1._q/3._q) 
!IF (ediff_th>ABS(REAL(W%CELTOT(NJ,KJ,ISP2),KIND=q)-REAL(W%CELTOT(RNB,KB,ISP2),KIND=q))) THEN
                           IF (NJ==RNB) THEN
                              Inte_c=Inte_c+CDER_BETWEEN_STATE_IA(I)*(CDER_BETWEEN_STATE_IA(I))*(FSG)*(1._q/3._q)*EDEPS/LATT_CUR%OMEGA*W%WDES%WTKPT(KI)
                           ENDIF
                        ENDDO
                        
                     ELSE
                        Inte_c=TWOE4ORBITAL(NA,NB)*(TWOE4ORBITAL(NA,NB))
                     ENDIF                     
                     
                     tmp=0._q
                     HEAD2=0._q
                     Inte_x=0._q
                     CDER_BETWEEN_STATE_IB(:)=0._q
                     CDER_BETWEEN_STATE_JA(:)=0._q
...
!ENDDO
                        ...
                        tmp=0._q
                        DO i=1,3
                           Inte_x=Inte_x+(TWOE4ORBITAL(NA,NB)+HEAD1(i,i))*((TWOE4ORBITAL_X(NA,NB)+HEAD2(i,i)))*(1._q/3._q)                           
                           IF ((NJ==RNA) .and. (NJ==RNB)) THEN
                              Inte_x=Inte_x+CDER_BETWEEN_STATE_IA(I)*(CDER_BETWEEN_STATE_IB(I))*(FSG)*(1._q/3._q)*EDEPS/LATT_CUR%OMEGA*W%WDES%WTKPT(KI)
                           ENDIF
                        ENDDO
                     ELSE
                        Inte_x=Inte_x+(TWOE4ORBITAL(NA,NB))*(TWOE4ORBITAL_X(NA,NB))
                     ENDIF
                     ...
                     IF (KI==KA) THEN                      
                      ...
                      CDER_BETWEEN_STATE_JB,LATT_CUR,KJ_IN_FULL_ORIG,ISP2,RNB,NJ)
                        ...
                        DO i=1,3
                           Inte_c=Inte_c+(TWOE4ORBITAL(NA,NB)+HEAD1(i,i))*((TWOE4ORBITAL(NA,NB)+HEAD1(i,i)))*(1._q/3._q)
                           IF ((NI==RNA)) THEN
                              Inte_c=Inte_c+CDER_BETWEEN_STATE_JB(I)*(CDER_BETWEEN_STATE_JB(I))*(FSG)*(1._q/3._q)*EDEPS/LATT_CUR%OMEGA*W%WDES%WTKPT(KI)
                           ENDIF
                           IF (NJ==RNB) THEN
                              Inte_c=Inte_c+CDER_BETWEEN_STATE_IA(I)*(CDER_BETWEEN_STATE_IA(I))*(FSG)*(1._q/3._q)*EDEPS/LATT_CUR%OMEGA*W%WDES%WTKPT(KI)
                           ENDIF
         ...
         IF (( (ABS(W%WDES%VKPT(1,kq1))<0.01_q) .AND. (ABS(W%WDES%VKPT(2,kq1))<0.01_q) .AND. (ABS(W%WDES%VKPT(3,kq1))<0.01_q))  .OR. &
          ((ABS(W%WDES%VKPT(1,KQ2))<0.01_q) .AND. (ABS(W%WDES%VKPT(2,KQ2))<0.01_q) .AND. (ABS(W%WDES%VKPT(3,KQ2))<0.01_q) )) THEN
            ...
            DO NB=1,TWOE4ORBITAL_COLS
               ...
               IF (FILLED_MP2_ORBITAL(W%FERTOT(RNA,KA,ISP1))) VIRT=0._q
               IF (FILLED_MP2_ORBITAL(W%FERTOT(RNB,KB,ISP2))) VIRT=0._q
               ...
               IF (ISP2==ISP1) THEN
                  tmp=0._q
                     ...
                     IF ((KI==KB) .OR. (KI==KA)) THEN
                        ...
                        DO i=1,3
                           Inte_x=Inte_x+(TWOE4ORBITAL(NA,NB)+HEAD1(i,i))*((TWOE4ORBITAL_X(NA,NB)+HEAD2(i,i)))*(1._q/3._q)
                           tmp=tmp+(HEAD1(i,i))*HEAD2(i,i)*(1._q/3._q)
                        ENDDO                   
                     ELSE
                        Inte_x=Inte_x+(TWOE4ORBITAL(NA,NB))*(TWOE4ORBITAL_X(NA,NB))
                     ENDIF
...
!This is 1._q for the plane-wave part, as well as for the 1._q-center terms.
...
!However, this approximation cannot be made for the 1._q-center terms in the gamma-only
...
! local variables
         ...
         REAL(q)      , ALLOCATABLE :: CRHOIA(:,:)    ! 1._q-center charge
         REAL(q)      , ALLOCATABLE :: CRHOIB(:,:)    ! 1._q-center charge
         REAL(q)      , ALLOCATABLE :: CRHOLM(:)      ! augmentation occupancy matrix
         COMPLEX(q), ALLOCATABLE :: tmp_FTOD_PW(:,:,:,:)
         REAL(q)      , ALLOCATABLE :: tmp_FTOD_OC(:,:,:,:)
...
!and allocate the TWOE4ORBITAL and TWOE4ORBITAL_X matrices
         ...
         kqloop: DO KQ=1,WDES%NKPTS
            ...
            kiloop: DO KI=1,WDES%NKPTS
               ...
               IF (ASSOCIATED(H)) tmp_FTOD_OC=0._q
...
! 1._q center terms and, on the plane wave grid.
                        IF (ASSOCIATED(H)) THEN
...
!Distribute the fourier-transformed overlap integrals
         ...
         desc_FTOD_PW(9) = MAX(1,2* desc_FTOD_PW(9)) ! leading dimension of local array
         desc_FTOD_PW(5) = 2* desc_FTOD_PW(5)
         desc_FTOD_PW(3) = 2* desc_FTOD_PW(3)  
# 1493
