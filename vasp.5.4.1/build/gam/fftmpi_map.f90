...
! 0._q XDAT buffer (otherwise we have do)
...
! be a little bit speed concerned, on 1._q node no need to go through M_alltoallv_z
    IF (MAP%LOCAL) THEN
...
! 0._q XDAT buffer
    IF (NZERO >  MAP%PTR(COMM%NCPU+1) ) THEN
       ...
       ENDDO
    ELSE
!         WRITE(*,*) 'no 0._q',NZERO,MAP%PTR(COMM%NCPU+1)
    ENDIF
