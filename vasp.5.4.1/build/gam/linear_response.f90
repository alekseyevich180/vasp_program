...
! lower k-point set to 0, and that for the upper 1._q to the
! proper weight
...
! and check whether this 1._q maps onto a k-point in KPOINTS_INTER
! which we have not yet found
! if so, replace that 1._q in KPOINTS_INTER
      DO IK=1,ABS(KINTER*KINTER*KINTER)
      DO N=1,NKPTS_ORIG
         NK=KPOINT_IN_FULL_GRID_KINTER(W%WDES%VKPT(:,N)+DISPL_FOUND(:,IK),KPOINTS_INTER)
         IF (NK==-1) THEN
