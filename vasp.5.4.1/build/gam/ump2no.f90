...
! First, the virtual-virtual block of the second-order reduced 1._q-electron density
...
! <ij|ab> (q=k_b-k_j), FTOD_PW=(ia)
      ...
      REAL(q) , ALLOCATABLE, PRIVATE, SAVE :: TWOE4ORBITAL(:,:), D2(:,:,:), D2EV(:,:)
...
# 281
                           ...
                           CALL PDGEMM('T','n',(PROCS*WDES%NBANDS),(PROCS*WDES%NBANDS),&
                                  (PROCS*WDES%NBANDS),1._q, TWOE4ORBITAL(1,1),1,1,&
                                  desc_TWOE4ORBITAL,TWOE4ORBITAL_X(1,1),1,1,&
                                  desc_TWOE4ORBITAL,1._q, D2(1,1,ISP),1,1,desc_TWOE4ORBITAL)
# 300
...
# 338
                                 ...
                                 CALL PDGEMM('T','n',(PROCS*WDES%NBANDS),(PROCS*WDES%NBANDS),&
                                        ...
                                        desc_TWOE4ORBITAL,1._q, D2(1,1,ISP2),1,1,desc_TWOE4ORBITAL)
# 359
            ...
            D2=D2*(KPOINTS_ORIG%WTKPT(1)*KPOINTS_ORIG%WTKPT(1))*1._q
            call BLACS_GRIDINFO(CONTXT_GRID, NPROW, NPCOL, MYROW, MYCOL) 
            ...
            DO ISP=1,WDES%ISPIN
               DO NB=1,TWOE4ORBITAL_COLS
                  ...
                  DO NA=1,TWOE4ORBITAL_ROWS
                     ...
                     IF (RNA<=VBMAX(ISP)) D2(NA,NB,ISP)=0._q
                     IF (RNB<=VBMAX(ISP)) D2(NA,NB,ISP)=0._q
                     IF (NBANDSGW/=-1) THEN
                        IF (RNA<=NBANDSGW) D2(NA,NB,ISP)=0._q
                        IF (RNB<=NBANDSGW) D2(NA,NB,ISP)=0._q
                     ENDIF

                     IF ((RNA==RNB) .and. (RNA<=VBMAX(ISP)))   D2(NA,NB,ISP)=1._q*1000
                     IF ((RNA==RNB) .and. (RNA<=VBMAX(ISP)))   D2(NA,NB,ISP)=1._q*1000
                     IF (NBANDSGW/=-1) THEN
                        IF ((RNA==RNB) .and. (RNA<=NBANDSGW))   D2(NA,NB,ISP)=1._q*1000
                        IF ((RNA==RNB) .and. (RNA<=NBANDSGW))   D2(NA,NB,ISP)=1._q*1000
                     ENDIF
                  ENDDO
               ENDDO
...
!                  IF (IO%IU0>=0) WRITE(IO%IU0,*)'EV',WEV(I)
            ...
            DO ISP=1,WDES%ISPIN
               ...
               CALL PDTRAN(PROCS*WDES%NBANDS,PROCS*WDES%NBANDS,1._q,&
                      D2(1,1,ISP),1,1,desc_TWOE4ORBITAL,0._q,&
...
# 487
               ...
               DO NA=1,TWOE4ORBITAL_ROWS
                  DO NB=1,TWOE4ORBITAL_COLS
                     CALL LOC2GLOB(NA,MYROW,desc_TWOE4ORBITAL(3),NPROW,desc_TWOE4ORBITAL(5),RNA)
                     CALL LOC2GLOB(NB,MYCOL,desc_TWOE4ORBITAL(4),NPCOL,desc_TWOE4ORBITAL(6),RNB)
                     IF ((-RNA+(PROCS*WDES%NBANDS)+1)==RNB) D2EV(NA,NB)=1._q*1000
                  ENDDO
               ENDDO


               CALL PDGEMM('t','n',(PROCS*WDES%NBANDS),(PROCS*WDES%NBANDS),&
                      (PROCS*WDES%NBANDS),1._q,D2EV(1,1),1,1,&
                      desc_TWOE4ORBITAL,D2(1,1,ISP),1,1,&
                      desc_TWOE4ORBITAL,0._q, TWOE4ORBITAL(1,1),1,1,desc_TWOE4ORBITAL)
               ...
               DO NB=1,TWOE4ORBITAL_COLS
                  ...
                  DO NA=1,TWOE4ORBITAL_ROWS
                     ...
                     IF ((RNA==RNB) .and. (RNB<=VBMAX(ISP)))  D2(NA,NB,ISP)=1._q*1000
                  ENDDO
               ENDDO
...
# 555
               ...
               CALL PDGEMM('t','n',(PROCS*WDES%NBANDS),(PROCS*WDES%NBANDS),&
                      (PROCS*WDES%NBANDS),1._q, D2(1,1,ISP),1,1,&
...
!******************************************
            ...
            CALL PZGEMM('T','n',(PROCS*WDES%NBANDS),(WDES%NRPLWV),&
...
!               IF ((RNA<=VBMAX) .or. (RNB<=VBMAX)) VIRT=0.0_q
               IF ((NBANDSGW/=-1) .and. ((RNA<=NBANDSGW) .or. (RNB<=NBANDSGW))) THEN
                  TWOE4ORBITAL(NA,NB)=0._q
                  TWOE4ORBITAL_X(NA,NB)=0._q
                  VIRT=0._q
               ENDIF
...
! collect all valence bands at k_i
               ...
               IF (OEAPPROXIMATE_NOs) THEN
                  DO NBI=2,VBMAX(ISP)
                     ...
                     WI(1)%GPROJ(:)=WI(1)%GPROJ(:)+WI(NBI)%GPROJ(:)
                  ENDDO
               ENDIF
! k_b = k_i - k_q - G
