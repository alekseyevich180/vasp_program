...
!***********************************************************************
      ...
      REAL(q)  NABIJ(WDES%NB_TOT,WDES%NB_TOT)
      CHARACTER (1) CHARAC
      LOGICAL LDUM
...
! must be larger than 0._q and smaller or equal to the total number of bands
...
! get the matrix element of the nabla-operator in eV/Angstroem units
         ...
         CALL M_sum_d(WDES%COMM,NABIJ(1,1),NB_TOT*NB_TOT)
...
!***********************************************************************
      ...
      block: DO IBLOCK=0,NPL-1,NBLK
       ...
       CALL DGEMM('T', 'N', NB_TOT, NB_TOT, 2* ILENPL, 1._q, &
                  CBLOCK(1,1), 2* NBLK, W%CPTWFP(IADDPL+1,1,NK,ISP), &
                  2* NPLDIM, 1._q, NABIJ(1,1), NB_TOT)
...
!***********************************************************************
      ...
      REAL(q)       NABIJ(WDES%NB_TOT,WDES%NB_TOT),CSUM
...
!=======================================================================
      ...
      DO LMP=1,P(NT)%LMMAX
       ...
       DO NBP=1,WDES%NB_TOT
       DO NB =1,WDES%NB_TOT
         CSUM=(W%GPROJ(LMBASE+LM,NB,NK,ISP))* &
              P(NT)%NABLA(IDIR,LM,LMP)* &
              W%GPROJ(LMBASE+LMP,NBP,NK,ISP)
...
!                         part: some with wave(r)/r and 1._q with d/dr wave(r);
...
! no allowed transition (angular parts are 0._q)
...
! subtraction can be 1._q by subtraction of 1._q from ONEBYR_SCALE (therefore we
! have defined a ONEBYR_SCALE already reduced by 1._q above ...); since we have
! always to remultiply [d/dr psi(r) + ONEBYR_SCALE*psi(r)/r] with r in order
! to obtain the correct r**2 prefactor (where 1._q factor is contained in WAVE)
