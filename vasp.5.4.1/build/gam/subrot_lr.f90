...
! local variables
      ...
      TYPE (wavedes1)    WDES1_W0       ! descriptor for 1._q k-point for W0
      ...
      REAL(q),ALLOCATABLE,TARGET::  CHAM(:,:),COVL(:,:)
...
!=======================================================================
   IF (LSETCHAM) THEN
      ...
      CALL OVERL(WDES1, .TRUE.,LMDIM,CQIJ(1,1,1,ISP),W%GPROJ(1,1,NK,ISP),WNONL%GPROJ(1,1))
! redistribute the projected wavefunctions
! wavefunctions are still required at this point
      IF (W0%WDES%DO_REDIS) THEN
        CALL REDIS_PROJ(WDES1, W%WDES%NBANDS, WNONL%GPROJ(1,1))
        CALL REDIS_PW  (WDES1_W0, W0%WDES%NBANDS, W0%CPTWFP   (1,1,NK,ISP))
        CALL REDIS_PROJ(WDES1_W0, W0%WDES%NBANDS, W0%GPROJ(1,1,NK,ISP))
        CALL REDIS_PW  (WDES1, W%WDES%NBANDS, W%CPTWFP    (1,1,NK,ISP))
        CALL REDIS_PROJ(WDES1, W%WDES%NBANDS, W%GPROJ (1,1,NK,ISP))
        CALL REDIS_PW  (WDES1, W%WDES%NBANDS, WXI%CPTWFP  (1,1,NK,ISP))
        CALL REDIS_PROJ(WDES1, W%WDES%NBANDS, WXI%GPROJ(1,1,NK,ISP))
      ENDIF
...
! this matrix is Hermitian only if no overlap matrix exists
            CALL ORTH2( &
                 W0%CPTWFP(1,1,NK,ISP),WAXI%CW_RED(1,NPOS),W0%GPROJ(1,1,NK,ISP), &
      ...
      ENDDO
      CALL M_sum_d(W0%WDES%COMM_KIN,CHAM(1,1),NB_TOT_W0*NB_TOT)
# 161
...
!-----------------------------------------------------------------------
      ...
      DO NPOS=1,NB_TOT,NSTRIP
         ...
         CALL ORTH2( &
              W0%CPTWFP(1,1,NK,ISP),WA%CW_RED(1,NPOS),W0%GPROJ(1,1,NK,ISP), &
              WNONL%CPROJ_RED(1,NPOS),NB_TOT_W0, &
              NPOS,NSTRIP_ACT,WDES1%NPL_RED,WDES1%NPRO_O_RED,WDES1%NRPLWV_RED,WDES1%NPROD_RED,COVL(1,1))
      ENDDO

      CALL M_sum_d(W0%WDES%COMM_KIN,COVL(1,1),NB_TOT_W0*NB_TOT)
# 216
...
!=======================================================================
      IF (WDES1%NPL_RED/=0) &
      CALL DGEMM('N', 'N', 2* WDES1%NPL_RED, NB_TOT, NB_TOT_W0, 1._q, &
     &               W0%CPTWFP(1,1,NK,ISP), 2* WDES1%NRPLWV_RED, CHAM(1,1), &
     &               NB_TOT_W0, 1._q, W%CPTWFP(1,1,NK,ISP) , 2* WDES1%NRPLWV_RED)
      IF (WDES1%NPRO_RED/=0) &
      CALL DGEMM('N', 'N', WDES1%NPRO_RED, NB_TOT, NB_TOT_W0, 1._q, &
     &               W0%GPROJ(1,1,NK,ISP), WDES1%NPROD_RED, CHAM(1,1), &
     &               NB_TOT_W0, 1._q, W%GPROJ(1,1,NK,ISP) , WDES1%NPROD_RED)
      ...
      CALL SUBROT_DEG_CLUSTERS(W0%WDES, WDES1%NPL_RED, WDES1%NPRO_RED, WDES1%NRPLWV_RED, WDES1%NPROD_RED, &
          W0%CPTWFP(:,:,NK,ISP), W0%GPROJ(:,:,NK,ISP), DEG_CLUSTER(NK,ISP)%DEG_CLUSTER, .FALSE., .FALSE.)
      CALL SUBROT_DEG_CLUSTERS(W0%WDES, WDES1%NPL_RED, WDES1%NPRO_RED, WDES1%NRPLWV_RED, WDES1%NPROD_RED, &
          W%CPTWFP(:,:,NK,ISP),  W%GPROJ(:,:,NK,ISP), DEG_CLUSTER(NK,ISP)%DEG_CLUSTER, .FALSE., .FALSE.)
      CALL SUBROT_DEG_CLUSTERS(W0%WDES, WDES1%NPL_RED, WDES1%NPRO_RED, WDES1%NRPLWV_RED, WDES1%NPROD_RED, &
          WXI%CPTWFP(:,:,NK,ISP), WXI%GPROJ(:,:,NK,ISP), DEG_CLUSTER(NK,ISP)%DEG_CLUSTER, .FALSE., .FALSE.)
      ENDIF
      ENDIF
...
! W%GPROJ must store  <p(1) |  phi(0)> + <p(0) | phi(1)>
...
! wavefunctions are still required at this point
      IF (WDES%DO_REDIS) THEN
        ...
        CALL REDIS_PROJ(WDES1, NBANDS, W0%GPROJ(1,1,NK,ISP))
...
!=======================================================================
     ...
     &               W0%CPTWFP(1,1,NK,ISP), 2* WDES1%NRPLWV_RED, COVL(1,1), &
     &               NB_TOT, 1._q, W%CPTWFP(1,1,NK,ISP) , 2* WDES1%NRPLWV_RED)
      IF (WDES1%NPRO_RED/=0) &
      CALL DGEMM('N', 'N', WDES1%NPRO_RED, NB_TOT, NB_TOT, 1._q, &
     &               W0%GPROJ(1,1,NK,ISP), WDES1%NPROD_RED, COVL(1,1), &
     &               NB_TOT, 1._q, W%GPROJ(1,1,NK,ISP) , WDES1%NPROD_RED)
...
!
! this subroutine is similar to the 1._q above
! but only dumps the "overlap" matrix
! it also assumes the W%GPROJ has already been multiplied by
! CQIJ
!
!***********************************************************************
...
! converges to a small non 0._q value, which decreases the stability
      ...
      DO ISP=1,WDES%ISPIN
         DO NK=1,WDES%NKPTS
            ...
            DO N=1,WDES%NBANDS
               ...
               W1%GPROJ(:,N,NK,ISP)=W1%GPROJ(:,N,NK,ISP)- W0%GPROJ(:,N,NK,ISP)*WXI%FERWE(N,NK,ISP)/2
            ENDDO
         ENDDO
      ENDDO
