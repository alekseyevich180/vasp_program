...
! redistributed plane wave coefficients
    ...
    REAL(q),ALLOCATABLE:: CHAM(:,:),COVL(:,:),CEIGVEC(:,:),CEIGVEC_LAST(:,:),COVL_(:,:),CTMP(:,:),COVL_HARM(:,:),&
...
! first calculate the non-local part of the overlap Q acting on psi^n_iter
          IF (INFO%LOVERL .AND. WDES%NPROD>0) THEN
             DO NP=1,NSIM_LOCAL_
                ...
                CALL OVERL(WDES1, INFO%LOVERL,LMDIM,CQIJ(1,1,1,1), W1(NP)%GPROJ(1),WS1%GPROJ(1,NPP))
                IF (WDES%DO_REDIS) THEN
                   CALL REDIS_PROJ(WDES1, 1, WS%GPROJ(1,NP))
                   CALL REDIS_PROJ(WDES1, 1, WS1%GPROJ(1,NPP))
                ENDIF
             ENDDO
          ENDIF
...
! this allows for parallel scalar products used below
          ...
          IF(LHARMONIC)THEN
             ...
             DO NP=1,NSIM_LOCAL_
                ...
                IF (WDES%DO_REDIS) THEN
                   CALL REDIS_PROJ(WDES1, 1, WHAM%GPROJ(1,NPP))
                ENDIF
             ENDDO
          END IF
!***********************************************************************
...
! diagonalize W+ H V = V+ H+H V, the squared Hamiltonian
          ...
          ELSE
             CALL ORTH1('U', &
                  ...
                  NPOS_RED, NSIM_, WDES1%NPL_RED, 0 ,WDES%NRPLWV_RED,WDES%NPROD_RED,CHAM(1,1))
          ENDIF
          CALL M_sum_d(WDES%COMM,CHAM(1,NPOS_RED),NSUBD*NSIM_)
          ...
          IF(LHARMONIC)THEN
             ...
             CALL M_sum_d(WDES%COMM,COVL_HARM(1,NPOS_RED),NSUBD*NSIM_)
             COVL_HARM=(TRANSPOSE(COVL_HARM))
             CALL ORTH3( &
                  WOPT%CW_RED(1,1), WHAM%CW_RED(1,NPOS_RED), &
...
!!$#else
          ...
          CALL M_sum_d(WDES%COMM,COVL(1,NPOS_RED),NSUBD*NSIM_)
...
! add remaining elements to CHAM
             DO M=1,NSIM_*(ITER-1)
                DO I=1,NSIM_
                   CHAM(NPOS_RED-1+I,M)=(CHAM(M,NPOS_RED-1+I))
                ENDDO
             ENDDO
          END IF
...
! calculate eigenvalues
             J=0
             DO I=1,NDIM
! skip if division by 0._q
                IF(ABS(BETA(I))<1.E-14_q)CYCLE
! stop if imaginary part /= 0
!!$                IF(ABS(ALPHAI(I))>1.E-14_q)THEN
...
# 677
          ...
          CALL M_bcast_d(WDES%COMM, CEIGVEC, NSUBD*ITER*NSIM_)


       IF (IFAIL/=0) THEN
          IF (ITER>=3) THEN
! if iteration failed go back 1._q step
...
! Cholesky factorization of overlap matrix between eigenvectors
             ...
             COVL_(1:NDIM*NSIM_,1:NDIM*NSIM_)=MATMUL( (TRANSPOSE(CEIGVEC(1:ITER*NSIM_,1:NDIM*NSIM_))),COVL_(1:ITER*NSIM_,1:NDIM*NSIM_) )
...
!=======================================================================
          stop: IF (LSTOP) THEN
             ...
             IF (WDES1%NPL_RED/=0) &
                  ...
                  &               WOPT%CW_RED(1,1), 2* WDES%NRPLWV_RED, CEIGVEC(1,1), NSUBD,  &
...
! stores back the NSIM_ lowest eigenvectors; NSIM_*ITER is the dimension of the current optimization subspace
             IF (WDES1%NPL_RED/=0) &
                  CALL DGEMM('N', 'N', 2* WDES1%NPL_RED , NSIM_, NSIM_*ITER, 1._q, &
                  &               WOPT%CW_RED(1,1), 2* WDES%NRPLWV_RED, CEIGVEC_LAST(1,1), NSUBD,  &
                  &               0._q, WLAST%CW_RED(1,NPOS_RED+1), 2* WDES%NRPLWV_RED)


             IF (WDES1%NPRO_RED/=0) &
                  CALL DGEMM('N', 'N', WDES1%NPRO_RED , NSIM_, NSIM_*ITER, 1._q, &
                  &               WOPT%CPROJ_RED(1,1), WDES%NPROD_RED, CEIGVEC_LAST(1,1), NSUBD,  &
                  &               0._q, WLAST%CPROJ_RED(1,NPOS_RED+1), WDES%NPROD_RED)
...
!
             ...
             IF (UPPER-LOWER>1) THEN
                ...
                CALL M_sum_d(WDES%COMM, CORTHO(1,1), (UPPER-LOWER-1)*NSIM_)
                
                
                IF (WDES1%NPL_RED /=0 ) &
                     CALL DGEMM( 'N', 'N' , 2* WDES1%NPL_RED , NSIM_ , UPPER-LOWER-1 , -1._q , &
                     WA%CW_RED(1,LOWER+1), 2* WDES%NRPLWV_RED , CORTHO(1,1) , UPPER-LOWER-1 , &
                     1._q , WOPT%CW_RED(1,NPOS_RED) , 2* WDES%NRPLWV_RED ) ! WOPT%GPROJ(1,ITER*NSIM_LOCAL_+1)

!gK: no need to calculate WOPT%CPROJ_RED before hand, since we overwrite it here
                WOPT%CPROJ_RED(:,NPOS_RED:NPOS_RED+NSIM_-1)=0
                IF (WDES1%NPRO_RED /= 0) &
                     CALL DGEMM( 'N', 'N' ,  WDES1%NPRO_RED , NSIM_ , UPPER-LOWER-1  , -1._q , &
...
! redistribute data over bands
             ...
             IF( WDES%DO_REDIS) THEN
                ...
                CALL REDIS_PROJ(WDES1, 1,  W1(NP)%GPROJ)
             ENDIF
! multiply each band with Q_kl and store results in
! W1%GPROJ(j,k) =  - Q_kl <beta_l| \sum_i phi_i><phi_i| R_j>
             IF (INFO%LOVERL .AND. WDES%NPROD>0 ) THEN
                WDES1%NBANDS=1
                CALL OVERL(WDES1, INFO%LOVERL,LMDIM,CQIJ(1,1,1,1), W1(NP)%GPROJ(1),WS%GPROJ(1,NP))
             ENDIF
! subtract  |beta_k> W1%GPROJ(j,k)
             IF (INFO%LREAL) THEN
                W1(NP)%CR(:)=0
                CALL RACC0(NONLR_S,WDES1, WS%GPROJ(1,NP) ,W1(NP)%CR(1))
                ...
                ENDDO
             ELSE
                CALL VNLAC0(NONL_S,WDES1, WS%GPROJ(1,NP), W1(NP)%CPTWFP(1+ISPINOR*WDES1%NGVECTOR))
             ENDIF
          ENDDO
...
! recalculate CQIJ x projected character for the updated bands
             IF (INFO%LOVERL .AND. WDES%NPROD>0 ) THEN
                DO NP=1,NSIM_LOCAL_
                   ...
                   IF( WDES%DO_REDIS) CALL REDIS_PROJ(WDES1, 1,  W1(NP)%GPROJ(1))
                   WDES1%NBANDS=1
                   CALL OVERL(WDES1, INFO%LOVERL,LMDIM,CQIJ(1,1,1,1), W1(NP)%GPROJ(1),WS%GPROJ(1,NP))
                   IF( WDES%DO_REDIS) CALL REDIS_PROJ(WDES1, 1,  WS%GPROJ(1,NP))
...
# 1107
             ...
             IF (WDES1%NPL_RED /=0 ) &
                  CALL DGEMM( 'N', 'N' , 2* WDES1%NPL_RED , NSIM_ , ITER*NSIM_ , -1._q , &
                  WOPT%CW_RED(1,1), 2* WDES%NRPLWV_RED , COVL(1,NPOS_RED) , SIZE(COVL,1) , &
                  1._q , WOPT%CW_RED(1,NPOS_RED) , 2* WDES%NRPLWV_RED )
             
             IF (WDES1%NPRO_RED /= 0) &
                  CALL DGEMM( 'N', 'N' ,  WDES1%NPRO_RED , NSIM_ , ITER*NSIM_  , -1._q , &
...
! correct COVL for subtraction of other vectors
             DO I=1,NSIM_
                DO II=1,NSIM_
                   DO M=1,ITER*NSIM_
                      COVL(NPOS_RED-1+I,NPOS_RED-1+II)=COVL(NPOS_RED-1+I,NPOS_RED-1+II)-(COVL(M,NPOS_RED-1+I))*COVL(M,NPOS_RED-1+II)
                   ENDDO
                ENDDO
             ENDDO
! set COVL in lower part to 0._q
...
! local
    ...
    REAL(q), ALLOCATABLE ::  LHAM(:,:),LOVL(:,:),WORK(:),TMAT(:,:)
...
! (T+ H T) (T+ v) = lambda D (T+ v)
    ...
    LHAM(1:SDIM,1:SDIM)=MATMUL(TRANSPOSE((TMAT)),LHAM)
