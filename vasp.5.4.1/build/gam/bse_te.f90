...
!
  ...
  REAL(qs), ALLOCATABLE :: BSEMATRIX(:,:)
  REAL(qs), ALLOCATABLE :: OPTMAT(:,:)
...
! Control flags.
      ...
      INTEGER,PRIVATE  :: ISPIN                ! Number of spin components (for spin flips this is 1._q) ... just for Claudia :))
...
! calculate the base (first index) on each node (1._q based as opposed to matstart)
    FIRST_ROW_INDEX=1
...
! add number of data on node I
    ...
    IF (IO%IU0>=0) WRITE(IO%IU0,'(A,F8.3,A,I7)') ' BSE TE single prec attempting allocation of',1.0_q/1E9*NCV_LOCAL*NCV*4*1 ,' Gbyte  rank=',NCV
    IF (IO%IU6>=0) WRITE(IO%IU6,'(A,F8.3,A,I7)') ' BSE TE single prec attempting allocation of',1.0_q/1E9*NCV_LOCAL*NCV*4*1 ,' Gbyte  rank=',NCV
    CALL REGISTER_ALLOCATE(4._q*1* NCV_LOCAL*NCV , "bse")
# 237
...
!     vectors. So the second 1._q is calculated using a double timestep.
!===================================================================
...
! IF  SGEMM=DGEMM=>SGEMM
         CALL SGEMM('N','N', MATDIM_NODE(NODE_ME),DIRS*2,DIM,RONE, &
