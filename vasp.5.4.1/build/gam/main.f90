...
! or a direct optimisation of the 1._q electron wavefunctions
...
!-----description how to go from 1._q grid to the second grid
!-----density of states
...
!-----remaining mainly work arrays
      ...
      REAL(q), ALLOCATABLE  ::  CPROTM(:),CMAT(:,:)
!=======================================================================
...
!=======================================================================
      ...
      CHARACTER (LEN=80),PARAMETER :: VASP = &
        'vasp.5.4.1' // ' ' // &
        '05Feb16' // ' (build ' // "Sep 21 2016"// ' ' //"21:00:57"// ') ' // &
        'gamma-only'
# 358
...
! write AE charge density
         ...
         CLOSE(99)
         ENDIF
! reset charge densities to 0._q again
         CHTOT=0; CHDEN=0
...
! write AE core charge density
         ...
         CLOSE(99)
         ENDIF
! and reset charge densities to 0._q again
...
!=======================================================================
! set the coefficients of the plane wave basis states to 0._q before
...
!
! this is the main loop of the program during which 1._q complete step of
...
!=======================================================================
! initialize pair-correlation funtion to 0._q
! also set TMEAN und SMEAN to 0._q
...
!=======================================================================
      ...
      IF (FOURORBIT/=0) THEN
! Set all DFT exchange contributions to 0._q
...
# 4922
      ...
      CALL START_TIMING("G")
      CALL LINCOM('F',W%CPTWFP(:,:,NK,1),W%GPROJ(:,:,NK,1),CMAT(1,1), &
       WDES%NB_TOT,WDES%NB_TOT,NPL,0,NRPLWV_RED,NPROD_RED,WDES%NB_TOT, &
       W%CPTWFP(:,:,NK,1),W%GPROJ(:,:,NK,1))
      ...
      CALL LINCOM('U',W%CPTWFP(:,:,NK,1),W%GPROJ(:,:,NK,1),CMAT(1,1), &
       WDES%NB_TOT,WDES%NB_TOT,NPL,NPRO,NRPLWV_RED,NPROD_RED,WDES%NB_TOT, &
       W%CPTWFP(:,:,NK,1),W%GPROJ(:,:,NK,1))
      ...
      DO N=1,WDES%NBANDS
        ...
        CALL INIDATR(WDES%NPRO,W1%GPROJ)
