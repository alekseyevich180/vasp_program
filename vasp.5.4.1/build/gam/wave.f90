...
! wavefunctions, if 1._q node holds all data for an entire wavefunction
...
! possibly smaller than NB_TOT
     ...
     INTEGER,POINTER ::NPRO_POS(:) ! for each atom, start index of entries in GPROJ in serial version
...
! this is only .TRUE. if precompiler flag  is define
     LOGICAL LSPIRAL               ! calculate spin spirals?
     LOGICAL LZEROZ                ! set m_z to 0._q in SET_CHARGE?
  END TYPE wavedes

!
! description for 1._q k point
...
! few things which are only required in parallel version
     ...
     INTEGER,POINTER :: NPRO_POS(:)! index GPROJ would have in serial version
     INTEGER,POINTER :: LMMAX(:)   ! total number of NLM quantum numbers for each type
     INTEGER,POINTER :: NITYP(:)   ! number of ions stored locally for each type
     INTEGER,POINTER ::NT_GLOBAL(:)! global type index for this type
...
! selects special treatment
     ...
     LOGICAL LOVERL                ! overlap required
     LOGICAL DO_REDIS              ! data redistribution required
     LOGICAL LNONCOLLINEAR         ! allows 1._q to turn on noncollinear calculations
     LOGICAL LSORBIT               ! spin orbit coupling
     LOGICAL LGAMMA                ! gamma point only, projected wavefunction character is REAL
     LOGICAL LSPIRAL               ! calculate spin spirals?
     LOGICAL LZEROZ                ! set m_z to 0._q in SET_CHARGE?
...
!
! structure required to store 1._q wavefunctions (no spin no bands)
!
  TYPE wavefun1
...
!
  TYPE wavefuna
     ...
     REAL(q)      ,POINTER:: CPROJ_RED(:,:)! projectors redistributed
     REAL(q),   POINTER:: FERWE(:)    ! fermi-weight for each band
     REAL(q),   POINTER:: AUX  (:)    ! auxilary
     COMPLEX(q),POINTER:: CELEN(:)    ! eigenvalues
...
!=======================================================================
!  free a descriptor and overwrite it by a new 1._q
!=======================================================================
...
!=======================================================================
  ...
  SUBROUTINE ALLOCW(WDES,W,WUP,WDW)
    ...
    ALLOCATE(W%CPTWFP(NRPLWV,NBANDS,NKPTS,ISPIN), &
         W%GPROJ (NPROD, NBANDS,NKPTS,ISPIN), &
         W%CELTOT(NB_TOT,NKPTS,ISPIN), &
         W%FERTOT(NB_TOT,NKPTS,ISPIN), &
         W%AUXTOT(NB_TOT,NKPTS,ISPIN))
    IF (WDES%LGAMMA) THEN
       CALL REGISTER_ALLOCATE(16._q*SIZE(W%CPTWFP)+8._q *SIZE(W%GPROJ), "wavefun")
    ELSE
       CALL REGISTER_ALLOCATE(16._q*SIZE(W%CPTWFP)+16._q*SIZE(W%GPROJ), "wavefun")
    ENDIF
  ...
  SUBROUTINE DEALLOCW(W)
    ...
    IF (ASSOCIATED(W%CPTWFP)) THEN
       IF (W%WDES%LGAMMA) THEN
          CALL DEREGISTER_ALLOCATE(16._q*SIZE(W%CPTWFP)+8._q*SIZE(W%GPROJ) , "wavefun")
       ELSE
          CALL DEREGISTER_ALLOCATE(16._q*SIZE(W%CPTWFP)+16._q*SIZE(W%GPROJ), "wavefun")
       ENDIF
    ENDIF

    IF (ASSOCIATED(W%CPTWFP) ) DEALLOCATE(W%CPTWFP)
    IF (ASSOCIATED(W%GPROJ) )  DEALLOCATE(W%GPROJ)
...
!=======================================================================
!  initialize and descriptor for 1._q wavefunction  (wavedes1) from
...
!  in a descriptor for 1._q single wavefunction (wavedes1)
...
!=======================================================================
!  create storage for 1._q wavefunction W
  ...
  SUBROUTINE NEWWAV(W1, WDES1, ALLOC_REAL, ISTAT)
    ...
    IF (PRESENT(ISTAT)) THEN
       IF (ALLOC_REAL) THEN
          ...
          ALLOCATE(W1%CPTWFP(WDES1%NRPLWV),W1%GPROJ(WDES1%NPROD),W1%CR(MPLWV),STAT=ISTAT)
       ELSE
          ALLOCATE(W1%CPTWFP(WDES1%NRPLWV),W1%GPROJ(WDES1%NPROD),STAT=ISTAT)
          NULLIFY(W1%CR)
       ENDIF
    ELSE
       IF (ALLOC_REAL) THEN
          MPLWV=WDES1%GRID%MPLWV*WDES1%NRSPINORS
          ALLOCATE(W1%CPTWFP(WDES1%NRPLWV),W1%GPROJ(WDES1%NPROD),W1%CR(MPLWV))
       ELSE
          ALLOCATE(W1%CPTWFP(WDES1%NRPLWV),W1%GPROJ(WDES1%NPROD))
          NULLIFY(W1%CR)
       ENDIF
    ENDIF
    W1%FERWE=0
...
!=======================================================================
!  create storage for 1._q wavefunction W for real space storage
!=======================================================================
...
!=======================================================================
!  destroy storage for 1._q wavefunction W
...
! local variables
  ...
  SUBROUTINE DELWAV_NOCW(W)
    IMPLICIT NONE
    TYPE (wavefun1) W
    DEALLOCATE(W%CR,W%GPROJ)
    NULLIFY(W%CR,W%GPROJ)
  END SUBROUTINE DELWAV_NOCW

!=======================================================================
!  set 1._q singe wavefunction (W1) from an array of wavefunctions
...
!  local
    ...
    DO ISP=1,W%WDES%ISPIN
       DO NK=1,W%WDES%NKPTS
          ...
          DO NB=(W%WDES%NB_TOTK(NK,ISP)-W%WDES%NB_LOW)/W%WDES%NB_PAR+2, W%WDES%NBANDS
             W%FERWE(NB,NK,ISP)=0       ! occupancy 0._q
             W%CELEN(NB,NK,ISP)=1E4     ! huge eigenvalue
          ENDDO
       ENDDO
    ENDDO
...
!  )  1._q telling where to store a coefficient
!  )  and 1._q telling whether the coefficient needs to be
!     complex conjugated
!
!***********************************************************************
...
! the serial layout on 1._q single core (not parallel)
