...
!  Upon removal the counter is usually decreased by 1._q, and the
!  cache-line is cleared if the counter is 0._q
!  A routine to clear the cache, as well as 1._q to check whether all
...
MODULE WAVE_CACHER
  ...
  TYPE wave_cache_block
     ...
     REAL(q)      ,POINTER:: GPROJ(:,:) ! projector (complex)
...
!
  ...
  REAL(q), ALLOCATABLE :: CHF(:,:,:,:) ! matrix to store search direction for subspace rotation
...
!***********************************************************************
!
! allocate the chacher structure and initialize everything to 0._q
!
!***********************************************************************
...
! found an allocated block
       IF (WCACHE%C(IBLK)%NFREE/=-1) THEN
          ...
          DEALLOCATE(WCACHE%C(IBLK)%CR, WCACHE%C(IBLK)%GPROJ, WCACHE%C(IBLK)%ICNT)
...
! new entry
       ...
       WCACHE%C(IBLK)%GPROJ(:,IPOS)=W%GPROJ
...
! allocate initialize and return
       ...
       ELSE IF ( WCACHE%C(IBLK)%NFREE<0) THEN
          ...
          ALLOCATE(WCACHE%C(IBLK)%CR(SIZE(W%CR),WCACHE%NBLK), & 
                   WCACHE%C(IBLK)%GPROJ(SIZE(W%GPROJ),WCACHE%NBLK), &
...
! local
    ...
    ELSE
       ...
       W%GPROJ=WCACHE%C(IBLK)%GPROJ(:,IPOS)
...
! local
    ...
    LOGICAL :: LPHASE
    INTEGER :: I
    COMPLEX(q) :: CPHASE(2* GRIDHF%MPLWV)
    COMPLEX(q) :: WPSIP(WDESQ%NGVECTOR)
    REAL(q)    :: WPSIR(2* MAX(GRIDHF%MPLWV, WDESQ%GRID%MPLWV))
!    REAL(q)    :: CHGR(2* MAX(GRIDHF%MPLWV, WDESQ%GRID%MPLWV))
    REAL(q)    :: SUM
    REAL(q) :: WEIGHT
    REAL(q)    :: GPROJ(WDESQ%NPROD)
...
! add D_lml'm' to kappa_lm_N (sum over l'm')
       CALL OVERL_FOCK(WDES_FOCK,  WCACHE%FH%LMDIM,  WCACHE%FH%CDIJ(1,1,1,1), & 
            WCACHE%FH%WQ%GPROJ(1), GPROJ ,.FALSE.)
       WCACHE%FH%CKAPPA(:,NB1_STORE,1)=WCACHE%FH%CKAPPA(:,NB1_STORE,1)+GPROJ*W_INTER(1)
       WCACHE%FH%CKAPPA(:,NB1_STORE,2)=WCACHE%FH%CKAPPA(:,NB1_STORE,2)+GPROJ*W_INTER(2)
    ENDIF
...
!    IF (WDES1%DO_REDIS) THEN  ! why is this if here
      ...
      CALL M_sum_d(WDES1%COMM_INTER,WCACHE%FH%CKAPPA(1,1,INDEX),SIZE(WCACHE%FH%CKAPPA,1)*SIZE(WCACHE%FH%CKAPPA,2))
!    ENDIF
...
!=======================================================================
    ...
    DO ISP=1,WDES%ISPIN
       DO NK=1,WDES%NKPTS
          ...
          IF (WDES%WTKPT(NK)==0) CYCLE ! 0._q k-point weighted points are useless right now in the GW code
...
!=======================================================================
    CALL SETWDES(WDES,WDES1,0)
    CALL NEWWAVA_PROJ(WNONL, WDES1)  ! instad of WNONL 1._q could use WACC1
...
!=======================================================================
          ...
          CALL REDISTRIBUTE_PW( ACC2)  ! ACC1%GPROJ and ACC2&GPROJ is never used
          CALL REDISTRIBUTE_PW( ACC1)

          CALL OVERL(WDES1, .TRUE.,LMDIM,CDIJ(1,1,1,ISP), WA%GPROJ(1,1),WNONL%GPROJ(1,1))
...
! sum matrix  over cores
          CALL M_sum_d(WDES%COMM_INTER,CHAM(1,1,NK,ISP),NB_TOT*NB_TOT)
          CALL M_sum_d(WDES%COMM_INTER,COVL(1,1),NB_TOT*NB_TOT)

! just in case: add the right square of the matrix beyond NBANDSGW
! this can be used for COHSEX
          DO N=1,NBANDSGW_K(NK,ISP)
             CHAM(N,NBANDSGW_K(NK,ISP)+1:NB_TOT,NK,ISP)=(CHAM(NBANDSGW_K(NK,ISP)+1:NB_TOT,N,NK,ISP))
             COVL(N,NBANDSGW_K(NK,ISP)+1:NB_TOT)=(COVL(NBANDSGW_K(NK,ISP)+1:NB_TOT,N))
          ENDDO
          ...
          IF (HAMIL_HERM) THEN
             DO  N=1,MAX_NBANDSGW
                DO  NP=N,MAX_NBANDSGW
                   COVL(NP,N)=(COVL(NP,N)+(COVL(N,NP)))/2
                   COVL(N,NP)=(COVL(NP,N))
                ENDDO
             ENDDO
          ENDIF
...
! multiply from left with S-1
             CTMP=0
             CALL DGEMM( 'N', 'N', NBANDSGW_K(NK,ISP), NBANDSGW_K(NK,ISP), NBANDSGW_K(NK,ISP), 1._q, COVL(1,1), NB_TOT, &
                  CHAM(1,1,NK,ISP), NB_TOT, 0._q , CTMP(1,1), NB_TOT)
             CHAM(:,:,NK,ISP)=CTMP
...
! for S-1/2 case CTMP is Hermitian so CTMP^+ = CTMP
             CALL DGEMM( 'T', 'N', NBANDSGW_K(NK,ISP), NBANDSGW_K(NK,ISP), NBANDSGW_K(NK,ISP), 1._q, COVL(1,1), NB_TOT, &
                  CHAM(1,1,NK,ISP), NB_TOT, 0._q , CTMP(1,1), NB_TOT)

! multiply from right with inverted U matrix
             CHAM(:,:,NK,ISP)=0
             CALL DGEMM( 'N', 'N', NBANDSGW_K(NK,ISP), NBANDSGW_K(NK,ISP), NBANDSGW_K(NK,ISP), 1._q, CTMP(1,1), NB_TOT, &
                  COVL(1,1), NB_TOT, 0._q , CHAM(1,1,NK,ISP), NB_TOT)
          ENDIF
...
! force diagonal to be real
                ...
                DO  NP=N,NBANDSGW_K(NK,ISP)
                   ...
                   CHAM(NP,N,NK,ISP)=(CHAM(NP,N,NK,ISP)*MAX(W%FERTOT(N,NK,ISP),0.01) &
                              +(CHAM(N,NP,NK,ISP))*MAX(W%FERTOT(NP,NK,ISP),0.01))/ &
                              (MAX(W%FERTOT(N,NK,ISP),0.01)+MAX(W%FERTOT(NP,NK,ISP),0.01))
                   ELSE
                      CHAM(NP,N,NK,ISP)=(CHAM(NP,N,NK,ISP)+(CHAM(N,NP,NK,ISP)))/2
                   ENDIF
! copy lower triangle to upper triangle
                   CHAM(N,NP,NK,ISP)=(CHAM(NP,N,NK,ISP))
                ENDDO
             ENDDO
          ENDIF
...
!=======================================================================
    IF (STEP/=1) THEN
      ...
      CHAM=(CHAM)  ! same as in rot.F (stupid, stupid,...)
...
! add previous search direction for pseudo Hamiltonian eta to current 1._q (CHAM)
...
! make sure that all nodes use identical rotation matrix
    ...
    CALL M_bcast_d(WDES%COMM_KINTER, CHAM, SIZE(CHAM))
# 1370
...
! 0._q elements beyond NBANDSGW_K(NK,ISP)
...
! set diagonal beyond NBANDSGW_K(NK,ISP) to 1
          ...
          CALL LINCOM('F',W%CPTWFP(:,:,NK,ISP),W%GPROJ(:,:,NK,ISP),CHAM(1,1,NK,ISP), &
...
! force CUNI to be Hermitian (take Hermitian part)
    DO  N1=1,NBANDS
       DO  N2=N1,NBANDS
          CUNI(N1,N2)=(CUNI(N1,N2)+(CUNI(N2,N1)))/2
          CUNI(N2,N1)=(CUNI(N1,N2))
       ENDDO
    ENDDO
!=======================================================================
...
! try to fix the negative eigenvalue somehow
    ...
    CALL DGEMM( 'N','T', NBANDS, NBANDS, NBANDS, 1._q, CTMP, &
         &              NBANDS, CUNI(1,1), NDIM, 0._q, CEIDB, NBANDS)
...
! 1._q can Taylor expand X around X0 (X=X0+dX)
! this yields
!         -2                  -1    -2    -2        -1   -2
...
! local
    ...
    DO ITER=1,5
! C= X^2
       CALL DGEMM( 'N','N', NBANDS, NBANDS, NBANDS, 1._q, & 
            X, NDIM, X, NDIM, 0._q, C, NBANDS)
       
! D= -X^2 A
       CALL DGEMM( 'N','N', NBANDS, NBANDS, NBANDS, -1._q, & 
            C, NBANDS, A, NDIM, 0._q, D, NBANDS)
...
! C = 1/2 X D
       CALL DGEMM( 'N','N', NBANDS, NBANDS, NBANDS, 0.5_q*1._q, & 
            X, NDIM, D, NBANDS, 0._q, C, NBANDS)

       X=0
       X(1:NBANDS,1:NBANDS)=C(1:NBANDS,1:NBANDS)

! this construct should yield the 1._q matrix in D (at the end of the iteration)
!       CALL DGEMM( 'N','N', NBANDS, NBANDS, NBANDS, 1._q, &
!            X, NDIM, A, NDIM, 0._q, C, NBANDS)
!       CALL DGEMM( 'N','N', NBANDS, NBANDS, NBANDS, 1._q, &
!            C, NBANDS, X, NDIM, 0._q, D, NBANDS)
    ENDDO
...
!***********************************************************************
      ...
      REAL(q) CHF(NDIM,NDIM)
      REAL(q) CUNI(NDIM,NDIM),CEIDB(NDIM,NDIM)
...
!=======================================================================
! CEIDB = U+ CHF
      CALL DGEMM( 'T', 'N', NBANDS, NBANDS, NBANDS, 1._q, CUNI, &
           &             NDIM, CHF, NDIM, 0._q, CEIDB, NDIM)

! CHF = (U+ CHF ) U
      CALL DGEMM( 'N', 'N', NBANDS, NBANDS, NBANDS, 1._q, CEIDB, &
           &             NDIM, CUNI, NDIM, 0._q, CHF, NDIM)
!=======================================================================
