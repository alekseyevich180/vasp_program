...
MODULE subrot_cluster
  ...
  TYPE eigenf_cluster
     ...
     REAL(q),POINTER :: U(:,:)                 ! unitary rotation matrix
     REAL(q),POINTER :: U_COMMULATIVE(:,:)     ! commulative rotation matrix
...
! local
    ...
    REAL(q) :: U(NMAX_DEG,NMAX_DEG)
! work arrays for unitary transformation
    REAL(q) :: CWRK(NMAX_DEG*NMAX_DEG)
...
# 319
      ...
      2 FORMAT(1I2,3X,20E9.1)
       ...
       CALL DSYEV &
            ...
            R,CWRK,NMAX_DEG*NMAX_DEG, IFAIL)
# 331
...
! accumate the matrix in U_COMMULATIVE
       CALL DGEMM('N', 'N', NB_TOT, NB_TOT, NB_TOT, 1._q, &
     &               DEG_CLUSTER%U_COMMULATIVE(1,1), NB_TOT, DEG_CLUSTER%U(1,1), &
     &               NB_TOT, 0._q, U, NMAX_DEG)
...
!
! 0._q the Hamiltonian for the sub-matrices spanned by
...
!***********************************************************************
  ...
  SUBROUTINE SUBROT_DEG_CLUSTERS(WDES, NPL, NPRO, NRPLWV_RED, NPROD_RED, &
    ...
    REAL(q)       :: CPROJ_RED(NPROD_RED ,WDES%NB_TOT)
    LOGICAL :: LCONJG
...
! local
    ...
    DO
       ...
       ELSE
       CALL LINCOM('T',CW_RED(1:,NB_START:),CPROJ_RED(1:,NB_START:),DEG_CLUSTER%U_COMMULATIVE(:,:), &
             NB_TOT,NB_TOT,NPL,NPRO,NRPLWV_RED,NPROD_RED,NB_TOT, &
             CW_RED(1:,NB_START:),CPROJ_RED(1:,NB_START:))
       ENDIF
       ELSE
       ...
       CALL LINCOM('F',CW_RED(1:,NB_START:),CPROJ_RED(1:,NB_START:),DEG_CLUSTER%U(:,:), &
             ...
             CW_RED(1:,NB_START:),CPROJ_RED(1:,NB_START:))
       ELSE
       CALL LINCOM('T',CW_RED(1:,NB_START:),CPROJ_RED(1:,NB_START:),DEG_CLUSTER%U(:,:), &
             NB_TOT,NB_TOT,NPL,NPRO,NRPLWV_RED,NPROD_RED,NB_TOT, &
             CW_RED(1:,NB_START:),CPROJ_RED(1:,NB_START:))
       ENDIF
       ENDIF
