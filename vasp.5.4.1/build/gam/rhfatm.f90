      ...
      DO NB_GLB=1,WDES%NB_TOT
         ...
         block: DO
            ...
            DO M=1,MMAX
               ...
               RHO1AE(:)=RHO1AE(:) &
              &   +W%GPROJ(LMIND,NB,1,1)*PP%WAE(:,L)*W%GPROJ(LMIND_,NB,1,1)*PP%WAE(:,LP)
               RHO1PS(:)=RHO1PS(:) &
              &   -W%GPROJ(LMIND,NB,1,1)*PP%WPS(:,L)*W%GPROJ(LMIND_,NB,1,1)*PP%WPS(:,LP)
            ENDDO
...
! check at which radius the projectors are all 0._q
...
! At this point 1._q might choose to make the PS partial
