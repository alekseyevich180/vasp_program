...
!
!  W%GPROJ(n) = \sum_G p_n,G+k e (i G R) C_G
!
...
!
! or not. This contribution does not contribute to the 1._q center occupancy
...
! for 1._q k-point, on the compressed grid of  lattice vectors
...
! work array
    ...
    CACC=0
    CALL OVERL1_C(W1%WDES1, SIZE(CDIJ,1), CDIJ(1,1,1,ISP), CQIJ(1,1,1,ISP), CEVALUE, W1%GPROJ(1),CRESUL(1))
    CALL VNLAC0(NONL_S,W1%WDES1,CRESUL(1),CACC(1))
...
! work array
    ...
    CALL OVERL1_CCDIJ(W1%WDES1, SIZE(CDIJ,1), CDIJ(1,1,1,ISP), CQIJ(1,1,1,ISP), EVALUE, W1%GPROJ(1),CRESUL(1))
    CALL VNLAC0(NONL_S,W1%WDES1,CRESUL(1),CACC(1))
...
!************************ SUBROUTINE PROJ    ***************************
!
! this subroutine calculates the projection of all bands of 1._q
...
# 857
             ...
             l_loop: DO LM=1,LMMAXC
                SUMR=TMP(LM,1)
                SUMI=TMP(LM,2)
                GPROJ(LM+LMBASE)=REAL(CMPLX( SUMR , SUMI ,KIND=q) *NONL_S%CQFAK(LM,NT),KIND=q)
             ENDDO l_loop
...
! local variable
    ...
    REAL(q)       :: CX(WDES%NPRO_TOT) ,CY(WDES%NPRO_TOT) ,CZ(WDES%NPRO_TOT)
    REAL(q)       :: CX_, CY_, CZ_
...
!***********************************************************************
  ...
  SUBROUTINE PROJLAT_DER(P, NONL_S, WDES, W, LATT_CUR, ISP, NK, CPROJXYZ)
    ...
    TYPE (latt)        LATT_CUR
    INTEGER            ISP, NK
    REAL(q), TARGET :: CPROJXYZ(WDES%NPROD, WDES%NBANDS, 6)
! local variable
    TYPE (wavedes1)    WDES1         ! descriptor for 1._q k-point
    TYPE (wavefun1)    W1
    INTEGER    :: N
    ...
    DO IDIR=1,3
       DO JDIR=1,IDIR
          ...
          band: DO N=1,WDES%NBANDS
             CALL SETWAV(W,W1,WDES1,N,ISP)
             W1%GPROJ => CPROJXYZ(:,N,IJDIR)
             CALL PROJ1(NONL_S,WDES1,W1) ! calculate W1%GPROJ (linked to CWORK)
...
!
! this subroutine calculates the first order change of the
! wave function character upon moving 1._q ion ION in the direction IDIR
...
!OCL NOVREC
                   DO K=1,NPL                 
                      CVAL = CVAL+NONL_S%QPROJ(K,LM,NT,WDES1%NK,ISPIRAL)*REAL(CTMP(K)*CMUL,KIND=q)
                   ENDDO
...
100       NIS = NIS+NONL_S%NITYP(NT)
    ...
    CALL DIS_PROJ(WDES1,C,W1%GPROJ(1))
...
!  upon moving 1._q ion ION in the direction IDIR
...
! work arrays
    ...
    CALL OVERL1(W1%WDES1,SIZE(CDIJ,1),CDIJ(1,1,1,1),CQIJ(1,1,1,1), EVALUE, W1%GPROJ(1),CRESUL(1))
...
! work arrays
    ...
    CALL OVERL1_C(W1%WDES1,SIZE(CDIJ,1),CDIJ(1,1,1,1),CQIJ(1,1,1,1), EVALUE, W1%GPROJ(1),CRESUL(1))
    CALL VNLAC0_DER(NONL_S,W1%WDES1,CRESUL(1),CACC(1), LATT_CUR, ION, IDIR )
...
! must be stored in GPROJ on entry
! Algorithm:
! ACC(G') = SUM(L,L',R)
...
! work arrays
    ...
    REAL(q) :: CM
    REAL(q),ALLOCATABLE :: CX(:) ,CY(:) ,CZ(:), C(:)
    REAL(q),ALLOCATABLE :: CXL(:),CYL(:),CZL(:),CL(:)
...
! new version use the calculated projected wave function character
                                    (CL(LM+LMBASE))
!                                    (W%GPROJ(LM+LMBASE,N,NK,ISP))
                               ENL(NIP)=ENL(NIP) + (W%GPROJ(LMP+LMBASE_,N,NK,ISP)*CM)*WEIGHT
...
!***********************************************************************
  ...
  SUBROUTINE STRENL(GRID,NONL_S,P,W,WDES,LATT_CUR, &
    ...
    REAL(q) CQIJ(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ)
! work arrys
    REAL(q),ALLOCATABLE,TARGET ::  CWORK(:)
...
!=======================================================================
          kpoint: DO NK=1,WDES%NKPTS
             ...
             spin: DO ISP=1,WDES%ISPIN
                band: DO N=1,WDES%NBANDS
                   CALL SETWAV(W,W1,WDES1,N,ISP); W1%GPROJ => CWORK
                   CALL PROJ1(NONL_S,WDES1,W1)  ! calculate W1%GPROJ (linked to CWORK)
...
!=======================================================================
    ...
    spinor: DO ISPINOR=0,WDES1%NRSPINORS-1
       ...
       typ: DO NT=1,NONL_S%NTYP
          ...
          ion: DO NI=NIS,NONL_S%NITYP(NT)+NIS-1
             DO LM=1,LMMAXC
                CTMP= GPROJ(LMBASE+LM)*CONJG(NONL_S%CQFAK(LM,NT))
       ...
       typ: DO NT=1,NONL_S%NTYP
          ...
          ions: DO NI=NIS,NONL_S%NITYP(NT)+NIS-1
             IF (NI==ION) THEN
                CACC_ADD=0
                l_loop: DO LM=1,LMMAXC
                   CMUL=GPROJ(LMBASE+LM)*CONJG(NONL_S%CQFAK(LM,NT))
!DIR$ IVDEP
!OCL NOVREC
                   DO K=1,NPL
