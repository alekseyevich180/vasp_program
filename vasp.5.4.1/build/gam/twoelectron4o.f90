...
! if 1._q includes a lot of empty orbitals savings are very small
...
! works as well in the case KIMAGES /= 0
    IF (KIMAGES/=0) THEN
       ...
       IF (IERR==0) THEN
          CALL M_sum_d(COMM_CHAIN, CDER_BETWEEN_STATES(1,1,1,1,1), SIZE(CDER_BETWEEN_STATES))
       ELSE
...
! - <N3,K3  N2,K2 | N4,K4  N1+NN1-1,K1>* x <N2,K2  N3,K3 | N4,K4  N1+NN1-1,K1>
                               ...
                               &             (TWOELECTRON3O(N3, N2, K2, N4, K4_STORE, NN1)))

                               INTE_HARTREE=0
                               DO I=1,3
                                  INTE_HARTREE=INTE_HARTREE+ &
                                 &             2._q*((TWOELECTRON3O(N3, N2, K2, N4, K4_STORE, NN1)+HEAD1(I,I))* &
                                 &             (TWOELECTRON3O(N3, N2, K2, N4, K4_STORE, NN1)+HEAD1(I,I)))
                               ENDDO
...
! WRITE(*,'(4I2,4F14.7)') NN1,N4,N3,N2,TWOELECTRON3O(N3, N2, K2, N4, K4_STORE, NN1)
                               ...
                               DO I=1,3
                                 ...
                                 &              REAL((TWOELECTRON3O(N3,N2, K2, N4, K4_STORE, NN1)+HEAD1(I,I))* &
...
!                              INTE=2._q*(TWOELECTRON3O(N3, N2, K2, N4, K4_STORE, NN1)*(TWOELECTRON3O(N3, N2, K2, N4, K4_STORE, NN1))) &
!                                   -REAL((TWOELECTRON3O(N3,N2, K2, N4, K4_STORE, NN1))*TWOELECTRON3O(N2, N3, K3, N4, K4_STORE, NN1),KIND=q)
...
! local variables
    ...
    REAL(q), ALLOCATABLE :: GWORK_PHASE(:,:)
    REAL(q)    :: GCHG (2* GRIDHF%MPLWV)
    ...
    REAL(q),      ALLOCATABLE :: CDIJ_PHASE(:,:,:,:,:) ! D_lml'm'
...
! using strips of size m for the second band
    ...
    ALLOCATE(GWORK(2* GRIDHF%MPLWV,NSTRIP2*NCPU))
    ALLOCATE(GWORK_PHASE(2* GRIDHF%MPLWV,NSTRIP2*NCPU))
...
! multiply by 4 pi e^2/G^2 and divide by # of gridpoints to obtain potential
          ...
          DO N=1,GRIDHF%RC%NP
            ...
            &   FOCKE((NPOS1-1)*NCPU+N1-N1_GLOBAL+1,N2)+(GCHG(N))*GWORK(N,N2)*(1.0_q/GRIDHF%NPLWV)
          ENDDO
...
! add D_lml'm' to kappa_lm_N (sum over l'm')
                IF (LPHASE) THEN
                   CALL OVERL_FOCK(WHF%WDES, LMDIM, CDIJ_PHASE(1,1,1,1,N2), W3%GPROJ(1), WA_(N2)%GPROJ(1,N4),.FALSE.)
                ELSE
                   CALL OVERL_FOCK(WHF%WDES, LMDIM, CDIJ(1,1,1,1,N2), W3%GPROJ(1), WA_(N2)%GPROJ(1,N4),.FALSE.)
                ENDIF
             ENDDO
          ENDDO
!==========================================================================
...
!***********************************************************************
  ...
  SUBROUTINE  INPROD_BETWEEN_STATES(WDES, WA1, WA2, & 
    ...
    INTEGER NK, ISP  
    REAL(q) :: NFLOAT
    TYPE (wavedes )    WDES           ! descriptor for 1._q k-point
...
!            N4, NSTRIP_ACT, WA1%WDES1%NPL_RED,WA1%WDES1%NPRO_O_RED,WA1%WDES1%NRPLWV_RED,WA1%WDES1%NPROD_RED,CHAM(1,1))
    ...
    ENDDO strip
    CALL M_sum_d(WDES%COMM,CHAM(1,1),WDES%NB_TOT*N4UP)
...
!
! identical version but with REAL(q)
!
  ...
  SUBROUTINE APPLY_PHASE_GDEF(GRID, SV, CR , CRP)
    ...
    TYPE (grid_3d)     GRID
    COMPLEX(q) ::  SV(GRID%MPLWV*2) ! complex phase factor
    REAL(q) ::  CR(GRID%MPLWV)
    REAL(q) ::  CRP(GRID%MPLWV)
    INTEGER NRSPINORS
! local variables
    INTEGER ISPINOR, M, MM
...
!   V'(r) =e^(i q r) V(r)
!  if 1._q would project this onto the local compensation charges
!  1._q would get
