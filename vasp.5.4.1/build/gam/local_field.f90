...
! CALCULATE_LOCAL_FIELD_PREPARE might also set this to 0._q, since
! 1._q can show that the effect of this correction is just
...
!   (so DELTA_COND is set to 0._q, or the G=0, G'0 in the
!    screened potential read from the file is set to 0._q)
!
!**********************************************************************
...
! this is so unclean that 1._q should cry out
...
! clear head and wing
    C(1,:)=0 ; C(:,1)=0 ; C(1,1)=1
! the second column and row are exactly 0._q, get rid of it
...
! protect against division by 0._q (usually the case if WAVEDER is missing)
...
!
!  to construct the kernel 1._q has to calculate the elements
...
! force response function to be hermitian (exact for w=0 and 0._q shift)
...
!************************ SUBROUTINE COPY_CHI **************************
!
! copy a response function from 1._q array to another
...
! conjugate charge in real space (same as interchanging the states)
             IF (LCONJG .AND. .NOT. WGWQ%LGAMMA) THEN
                GWORK(1:GRIDHF%RL%NP)=(GWORK(1:GRIDHF%RL%NP))
             ENDIF
...
! we need \int r v*_k1,n1(r) c_k1+q,n3(r) = i CDER_BETWEEN_STATE
                IF (LCONJG) THEN
                   CDER_BETWEEN_STATE=(CDER_BETWEEN_STATE)
                ENDIF
...
! local
    ...
    COMPLEX(q) :: CPHASE(GRIDHF%MPLWV)
    REAL(q), ALLOCATABLE :: CRHOLM(:)        ! augmentation occupancy matrix
    REAL(q) :: WEIGHT, WEIGHT_REL, WEIGHT_CHI
    LOGICAL LPHASE
    INTEGER I, J
    REAL(q) :: CDER_BETWEEN_STATE(3), C(3), CC(3)
    INTEGER :: NOMEGA
    COMPLEX(q) :: COMEGA
...
! update response function
                   DO I=1,3
                      DO J=1,3
                         CHI%HEAD(J,I,NOMEGA)= CHI%HEAD(J,I,NOMEGA)+CDER_BETWEEN_STATE(J)*(CDER_BETWEEN_STATE(I))*WEIGHT_CHI
                      ENDDO
                   ENDDO
                   
                   DO I=1,3
                      CHI%WING(1:NP,I,NOMEGA) =CHI%WING(1:NP,I,NOMEGA) +GCHGW(1:NP,N2,N4)*(CDER_BETWEEN_STATE(I))*WEIGHT_CHI
...
!==========================================================================
          ...
          CALL DGEMM('T','N',  NSTRIP1*NSTRIP2 , SIZE(CHAM,3)*NSTRIP4, 2* NP, 1._q, &
               GCHG12(1,1), 2* SIZE(GCHG12,1), &
               GCHG34(1,1), 2* SIZE(GCHG34,1), &
...
! for shifted meshes we have k-points with 0._q weight
...
! local variables
    ...
    COMPLEX(q) :: CPHASE(GRIDHF%MPLWV)
    REAL(q), ALLOCATABLE :: CRHOLM(:)        ! augmentation occupancy matrix
    REAL(q), ALLOCATABLE :: CHAM(:,:,:,:)    ! Hamiltonian matrix
    LOGICAL LPHASE
    REAL(q) :: POTFAK(GRIDHF%MPLWV)       ! 1/(G+dk)**2 (G)
...
! this is exactly 0._q if LGWLF is set
             IF (FEX) THEN
...
!==========================================================================
          ...
          CALL DGEMM('T','N',  NSTRIP1*NSTRIP3, NSTRIP2*NSTRIP4, 2* NP, 1._q, &
               GCHG13(1,1,1), 2* SIZE(GCHG13,1), &
               GCHG24(1,1,1), 2* SIZE(GCHG24,1), &
               0._q, CHAM(1,1,1,1), SIZE(CHAM,1)*SIZE(CHAM,2))
