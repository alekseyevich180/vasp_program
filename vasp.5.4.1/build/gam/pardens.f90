...
! to store the partial charge density
    ...
    REAL(q)    crhode(LMDIM,LMDIM,WDES%NIONS,WDES%ISPIN)  ! augmentation occupations
...
! --- now set the correct the 1._q kpoint and band
...
! now calculate charge density for this 1._q band
                IF  (ltest) CALL write_eigv()
...
! using only 1._q part of them, they all will be equall weighted
...
! first get status of the input file:
      ...
      IF (.NOT.lopen) THEN  ! the file exists put 1._q has to open it
         iupar = NXTFRU()
