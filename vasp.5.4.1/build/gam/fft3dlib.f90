...
! vectors with parallel processing of all 1._q-dimensional data sets).  *
...
! At this point the final transform result is stored in AR/AI, bye ...
      ...
      END IF
!
!     Perform the transform passes, 1._q pass for each factor:
!
      IF (ISIGN>=0) THEN
...
!     (which is currently 'third' dimension), 1._q pass for each factor:
!
      INCL=ID1*ID3
      INCN=1
      L=N2
...
!
!     Perform the transform passes needed to transform second dimension,
!     (which is currently 'first' dimension), 1._q pass for each factor:
!
         INCL=1
         INCN=ID2
         L=N2
...
! Before using CFFTM the first time 1._q must initialise trigonometric  *
...
!   INCLR: addressing increment between elements of 1._q data set       *
...
! factors present and if at least 1._q smaller factor (3 or 4) remains as first
...
! if 1._q single factor of 2 is left then try to merge it into a factor of 6
! but only if at least 1._q further small factor is left as first factor ...
      IF ((MIN(N2,N3)>0).AND.(MAX(N4+1,N3)>1).AND.((N5+N7)<2).AND.((N3+N4)<4)) THEN
        N6=N6+1
        N2=N2-1
        N3=N3-1
      ENDIF
! if 1._q single factor of 2 is left then try to merge it into a factor of 8
! but only if at least 1._q further small factor is left as first factor ...
! mind: currently only 1._q single factor of 8 may appear (only as last factor!)
...
! performs 1._q pass through data as part of a multiple complex Fast    *
! Fourier Transformation in 1._q dimension on multiple 1-D data sets.   *
! Variables:                                                           *
...
!   INC1 is the increment between elements of 1._q data set in A and B  *
!   INC2 is the increment between elements of 1._q data set in C and D  *
...
!   N is the length of 1._q data set (length of the FFT)                *
...
! 1._q had to rearrange the data externally in some way that the FFT    *
