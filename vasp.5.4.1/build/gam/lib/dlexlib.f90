...
! Returns the position of the last non-blank character in STRING
      ...
      SAVE            B5,B6,B7,B8,B9,B10,B11,B12,B13,B14
      DATA            B5 /' '/, B6 /' '/, B7 /' '/, B8 /' '/, B9 /' '/, &
     &                B10 /' '/,B11 /' '/,B12 /' '/,B13 /' '/,B14 /' '/
# 27
...
! Very crude 'scan' for 'order of length' (performance!!) ...:
      ...
      IF (L.GE.16384) THEN
         IF (STRING(L-16383:L).EQ.B14) THEN
            L=L-16384
            GOTO 1
         ELSE IF (STRING(L-8191:L).EQ.B13) THEN
            L=L-8192
            GOTO 1
         ELSE IF (STRING(L-4095:L).EQ.B12) THEN
            L=L-4096
            GOTO 1
         ELSE IF (STRING(L-2047:L).EQ.B11) THEN
            L=L-2048
            GOTO 1
         ELSE IF (STRING(L-1023:L).EQ.B10) THEN
            L=L-1024
            GOTO 1
         ELSE IF (STRING(L-511:L).EQ.B9) THEN
            L=L-512
            GOTO 1
         ELSE IF (STRING(L-255:L).EQ.B8) THEN
            L=L-256
            GOTO 1
      ...
      ENDIF
    1 IF (L.GE.8192) THEN
         IF (STRING(L-8191:L).EQ.B13) THEN
            L=L-8192
            GOTO 2
         ELSE IF (STRING(L-4095:L).EQ.B12) THEN
            L=L-4096
      ...
      ENDIF
    2 IF (L.GE.4096) THEN
         IF (STRING(L-4095:L).EQ.B12) THEN
            L=L-4096
            GOTO 3
         ELSE IF (STRING(L-2047:L).EQ.B11) THEN
            L=L-2048
      ...
      ENDIF
    3 IF (L.GE.2048) THEN
         IF (STRING(L-2047:L).EQ.B11) THEN
            L=L-2048
            GOTO 4
         ELSE IF (STRING(L-1023:L).EQ.B10) THEN
            L=L-1024
      ...
      ENDIF
    4 IF (L.GE.1024) THEN
         IF (STRING(L-1023:L).EQ.B10) THEN
            L=L-1024
            GOTO 5
         ELSE IF (STRING(L-511:L).EQ.B9) THEN
            L=L-512
...
! searching and counting at the first invalid item found ...).
      ...
      CHARACTER*32767 BUFFER,FORM,NUMBER,DUMMY
# 293
