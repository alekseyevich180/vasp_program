...
! size of arrays defined with REAL(q)
...
! number of nodes (for 1._q image)
...
!=======================================================================
    ...
    SUBROUTINE pPOTRF_TRTRI (COMM, AMATIN, NDIM, N)
      ...
      REAL(q)    AMATIN(NDIM,N) ! input/output matrix
! local variables
      INTEGER INFO
...
!=======================================================================
    ...
    SUBROUTINE pDSSYEX_ZHEEVX(COMM, AMATIN, W, NDIM, N, ABSTOL_, AMATIN_SCALA)
      ...
      REAL(q), OPTIONAL, TARGET ::  AMATIN_SCALA(GSD%MALLOCPQ) ! alternative input/output matrix (1 format)
! local variables
      ...
      REAL(q), ALLOCATABLE,TARGET ::  A(:) ! A matrix to be diagonalized (allocatable)
      REAL(q), POINTER  ::  AP(:)          ! A matrix to be diagonalized (pointer) either to A or AMATIN_SCALA
      REAL(q), ALLOCATABLE    ::  Z(:)     ! Z eigenvector matrix
      REAL(q), ALLOCATABLE ::  RWORK(:) ! work array
      REAL(q), ALLOCATABLE    ::  WWORK(:) ! work array
...
!=======================================================================
    ...
    SUBROUTINE pSSYEX_CHEEVX(COMM, AMATIN, W, NDIM, N, ABSTOL_, AMATIN_SCALA)
      ...
      INTEGER N               ! NxN matrix to be distributed
      INTEGER NDIM            ! leading dimension of matrix
      REAL(qs)   AMATIN(NDIM,N)  ! input/output matrix
      REAL(q) W(N)            ! eigenvalues
      REAL(q), OPTIONAL ::  ABSTOL_ ! tolerance
      REAL(q), OPTIONAL, TARGET ::  AMATIN_SCALA(GSD%MALLOCPQ) ! alternative input/output matrix (1 format)
! local variables
      ...
      REAL(q) :: ABSTOL
      REAL(q), ALLOCATABLE,TARGET ::  A(:)     ! A matrix to be diagonalized (allocatable)
      REAL(q), POINTER            ::  AP(:)    ! A matrix to be diagonalized (pointer) either A or AMATIN_SCALA
      REAL(q), ALLOCATABLE    ::  Z(:)     ! Z eigenvector matrix
      REAL(q), ALLOCATABLE ::  RWORK(:) ! work array
      REAL(q), ALLOCATABLE    ::  WWORK(:) ! work array
      INTEGER, ALLOCATABLE ::  IWORK(:) ! integer work array
...
!=======================================================================
    ...
    SUBROUTINE pSSYEX_CHEEVX_single(COMM, AMATIN, W, NDIM, N, ABSTOL_, AMATIN_SCALA)
      ...
      REAL(q) W(N)            ! eigenvalues
      REAL(q), OPTIONAL ::  ABSTOL_ ! tolerance
      REAL(qs), OPTIONAL, TARGET ::  AMATIN_SCALA(GSD%MALLOCPQ) ! alternative input/output matrix (1 format)
! local variables
      INTEGER ICLUSTR(2*COMM%NCPU)
      INTEGER IFAIL(N)
      ...
      REAL(qs), ALLOCATABLE,TARGET ::  A(:)     ! A matrix to be diagonalized (allocatable)
      REAL(qs), POINTER            ::  AP(:)    ! A matrix to be diagonalized (pointer) either A or AMATIN_SCALA
      REAL(qs), ALLOCATABLE    ::  Z(:)     ! Z eigenvector matrix
      REAL(qs), ALLOCATABLE ::  RWORK(:) ! work array
      REAL(qs), ALLOCATABLE    ::  WWORK(:) ! work array
      INTEGER, ALLOCATABLE ::  IWORK(:) ! integer work array
      INTEGER,EXTERNAL :: NUMROC,ICEIL
      EXTERNAL BLACS_PINFO, BLACS_GRIDINIT, BLACS_GRIDINFO,  &
               BLACS_GRIDEXIT, BLACS_EXIT
...
!=======================================================================
    ...
    SUBROUTINE BG_pDSSYEX_ZHEEVX(COMM, A, W, N)
      ...
      TYPE (communic) COMM
      INTEGER N               ! NxN matrix to be distributed
      REAL(q)    A(GSD%NP,GSD%NQ)  ! input/output matrix
      REAL(q) W(N)            ! eigenvalues
! local variables
      INTEGER ICLUSTR(2*COMM%NCPU)
      INTEGER IFAIL(N)
...
!=======================================================================
!
! merge distributed matrix into 1._q large matrix
!
!=======================================================================
...
!
! this version is for a REAL(q) input array (REAL(q) for gammaonly)
!
          ...
          DO I1=1,NP,DESCA(MB_)
            ...
            DO I2=1,I1RES
              ...
              DIFCELL=REAL(CELTOT(DESCA(NB_)*MYCOL+NPCOL*(J1-1)+J2)- & 
                          CELTOT(DESCA(MB_)*MYROW+NPROW*(I1-1)+I2))
              IF (ABS(DIFCELL)>=DIFMAX) THEN
                 CSUM=CSUM+A(IROW_JCOL)*(A(IROW_JCOL))/DIFCELL* & 
...
! 1._q can also simply take the lower triangle and change the sign
! in the upper
!
!=======================================================================
...
!=======================================================================
    ...
    SUBROUTINE DISTRI_SLICE(AMATIN,NDIM,N,A,DESCA,COLUMN_LOW, COLUMN_HIGH)
      ...
      REAL(q)    AMATIN(NDIM,COLUMN_HIGH-COLUMN_LOW+1)  ! input matrix (globally known)
      REAL(q)    A(*)                                   ! on return: output distributed matrix
      INTEGER DESCA( DLEN_ )                         ! distributed matrix descriptor array
! local
      INTEGER NP,NQ
      INTEGER I1RES,J1RES,IROW,JCOL
...
! global column index: block_size*processor coordinate +grid dimension*block number + index in block
                     COL=DESCA(NB_)*MYCOL+NPCOL*(J1-1)+J2
                       A(IROW+(JCOL-1)*DESCA(LLD_))=(AMATIN(COL, ROW-COLUMN_LOW+1))
                  ENDDO
               ENDDO
            ENDIF
         ENDDO
...
! global column index: block_size*processor coordinate +grid dimension*block number + index in block
                     COL=DESCA(NB_)*MYCOL+NPCOL*(J1-1)+J2
                     IF (COL>=ROW) A(IROW+(JCOL-1)*DESCA(LLD_))=(AMATIN(COL, ROW-COLUMN_LOW+1))
                  ENDDO
               ENDDO
            ENDIF
         ENDDO
...
!=======================================================================
    ...
    SUBROUTINE RECON_SLICE(AMATIN, NDIM, N, A, DESCA, COLUMN_LOW, COLUMN_HIGH)
      ...
      INTEGER N                          ! rank of matrix (not used except by CHECK_scala)
      REAL(q)    AMATIN(NDIM,COLUMN_HIGH-COLUMN_LOW+1)
...
!=======================================================================
     ...
     SUBROUTINE ORTH1_DISTRI(CSEL,CPTWFP,CFW,GPROJ,CPROW, & 
        ...
        REAL(q)      COVL(NBANDS,NSTRIP), COVL2(*)
        CHARACTER*(*) CSEL

        CALL CHECK_scala (NBANDSK, 'ORTH1_DISTRI' )

        COVL=0
        CALL ORTH1_NOSUBINDEX(CSEL,CPTWFP(1,1),CFW(1,1),GPROJ(1,1), &
           CPROW(1,1),NBANDS, &
           NPOS,NSTRIP,NPL,NPRO,NPLDIM,NPROD,COVL(1,1))

        CALL M_sum_d(MY_COMM,COVL(1,1),NBANDS*NSTRIP)
...
!
      ...
      SUBROUTINE ORTH1_DISTRI_HERM(CSEL,CPTWFP,CFW,GPROJ,CPROW, & 
           NBANDS,NPOS,NSTRIP,NPL,NPRO,NPLDIM,NPROD,COVL2,MY_COMM,NBANDSK)
        USE prec
        USE scala
        IMPLICIT NONE
...
!=======================================================================
      ...
      SUBROUTINE ORTH1_DISTRI_HERM_DESC(CSEL,CPTWFP,CFW,GPROJ,CPROW, & 
...
!=======================================================================
      ...
      SUBROUTINE ORTH1_DISTRI_DAVIDSON(CSEL,CPTWFP,CFW,GPROJ,CPROW, &
...
!=======================================================================
     ...
     &           NPRO,NPLDIM,NPROD,LDTRAN, MY_COMM, NBLK)
        ...
        REAL(q)        CPROF(NPROD,NIN),CPROF_RESULT(NPROD,NIN)
        REAL(q)        COVL(*)
! local
        REAL(q)        COVL_GLOBAL(LDTRAN,NBLK)
        INTEGER     NPOS, NOUT
...
! reconstruct matrix block by block and transform wavefunctions
        DO NPOS=1,NIN-NBLK,NBLK
          ...
          CALL M_sum_d(MY_COMM,COVL_GLOBAL(1,1),LDTRAN*NOUT)
...
!=======================================================================
      SUBROUTINE DISTRI_SLICE_NOINT(AMATIN,NDIM,N,A,DESCA,COLUMN_LOW, COLUMN_HIGH)
        ...
        INTEGER COLUMN_LOW, COLUMN_HIGH                ! starting and end row supplied in AMATIN
        REAL(q)    AMATIN(NDIM,COLUMN_HIGH-COLUMN_LOW+1)  ! input matrix (globally known)
        REAL(q)    A(*)                                   ! output distributed matrix
        INTEGER DESCA(*)                               ! distributed matrix descriptor array
