...
!
! maximum L for 1._q center terms (per default not included)
!
    INTEGER :: LMAXMP2=-1
!
...
!
! Calculate 2-electron 4-wannier-orbital integrals
! all terms not just 1._q-center terms
!
...
! read number of frequencies NOMEGA
    ...
    IF (NOMEGA<=0 .AND. LACFDT) THEN
       NOMEGA=12
    ENDIF
! exact exchange OEP useless to use more than 1._q frequency
...
! force NOMEGAR to be 0._q for ACFDT
    IF (LACFDT) NOMEGAR=0
...
! and 1._q can simply calculate all k-point pairs
...
!=======================================================================
! allocation
! determine how many frequencies can be 1._q in 1._q go
...
! if LGWLF is set, ALDAX and ALDAC are 0._q at this point
! hence the DFT local field effects f_xc (stored in FXCR) are exactly 0._q
...
! if NOMEGA is 0._q (test passed for ACFDT)
...
!-----------------------------------------------------------------------
    IF (LscQPGW .AND. .NOT. LOEP) THEN
       ...
       CALL M_sum_d(WDES%COMM_KINTER,WACC1%GPROJ,SIZE(WACC1%GPROJ))
       CALL M_sum_z(WDES%COMM_KINTER,WACC2%CPTWFP,SIZE(WACC2%CPTWFP))
       CALL M_sum_d(WDES%COMM_KINTER,WACC2%GPROJ,SIZE(WACC2%GPROJ))
...
! mix charge density with the 1._q of the previous step
...
! for gamma only the second entry is the sine transform corresponding to G=0, always 0._q
...
! inversion, 1._q can easily neglect local field effects entirely
...
! invert matrix
             CALL ZGETRI( NP, CHI_WORK(1,1), SIZE(CHI_WORK,1), IPIV, & 
                  WORK, SIZE(CHI%RESPONSEFUN,1)*NWORK, INFO )
! 0._q second column and row (now 1/100)
...
! calculate  Xi_0 (f_xc+v)
             ...
             DO I=1,NP
                CHI_WORK(I,I)=1+CHI_WORK(I,I)
             ENDDO
! second entry is the sin transform corresponding to G=0, always 0._q
...
! slight trouble with the head: divide it by 1._q over potential operator
...
!
! the 1._q-center exchange term is included in CELTOT_HARTREE_KINETIC
...
!
! this version considers only transition in 1._q spin channel
! which at least for RPA is what 1._q actually wants to do
