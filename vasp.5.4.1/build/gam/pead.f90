...
! d_IJ = \int r*Q_ij(r) dr
      REAL(q), PRIVATE, ALLOCATABLE, SAVE :: d_IJ(:,:,:,:,:)
...
! store the gradients dpsi/dk for 1._q k-point
      COMPLEX(q), PRIVATE, ALLOCATABLE, SAVE :: RPHI_STORE_LOCAL(:,:,:)
      REAL(q) , PRIVATE, ALLOCATABLE, SAVE :: RPHI_STORE_LOCAL_CPROJ(:,:,:)
...
! in the irreducible Brillouin (1._q,0._q)
      ...
      REAL(qs) , PRIVATE, ALLOCATABLE, SAVE :: RPHI_STORE_LOCAL_ALL_CPROJ(:,:,:,:,:)
...
! local variables
      ...
      COMPLEX(q) CDUM
      LOGICAL LDUM
      REAL(q) TMP(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ)
...
! quick return if the electric field is essentially 0._q
...
!***********************************************************************
     ...
     &   W,K,ISP,P,CQIJ,LATT_CUR,T_INFO,RPHI,RPHI_CPROJ)
      ...
      REAL(q) RPHI_CPROJ(W%WDES%NPROD, W%WDES%NBANDS,3)
...
!***********************************************************************
     ...
     &   W,KPOINTS,P,CQIJ,LATT_CUR,T_INFO,RPHI,RPHI_CPROJ)
      ...
      REAL(qs) RPHI_CPROJ(W%WDES%NPROD, W%WDES%NBANDS,W%WDES%NKPTS,W%WDES%ISPIN,3)
! local variables
      INTEGER ISP,IK
      REAL(q) K(3)
      REAL(q) WEIGHT
...
!        RPHI_CPROJ(:,:,IK,ISP,1:3)=WEIGHT*RPHI(:,:,IK,ISP,1:3)
      ...
      CALL M_sum_single(W%WDES%COMM_KINTER,RPHI_CPROJ(1,1,1,1,1),SIZE(RPHI_CPROJ))
# 826
...
!        DWDK%GPROJ(:,:,IK,ISP)=WEIGHT*DWDK%GPROJ(:,:,IK,ISP)
            ...
            DO NT=1,WDESK%NTYP
               ...
               IF (LMMAXC/=0) THEN
                  DO NI=NIS,WDESK%NITYP(NT)+NIS-1
                    ...
                    &   WK%GPROJ(NPRO_+1,NB),WK%GPROJ(NPRO+1,NB),CNL)
                     NPRO = LMMAXC+NPRO
                     NPRO_= LMMAXC+NPRO_
                  ENDDO
               ENDIF
...
! communicate
         CALL M_sum_d(WDESK%COMM_INB, CNL, 1)
! and store
...
!******************** SUBROUTINE ***************************************
!
! This subroutine is called from main.F and takes care of 1._q or all
! of the following:
!
      ...
      DO ISPINOR=0,W%WDES%NRSPINORS-1
         ...
         DO NT=1,W%WDES%NTYP
            DO NI=NIS,W%WDES%NITYP(NT)+NIS-1
              ...
              &   W%GPROJ(NPROP,NB,NK_OLD,ISP_OLD),W1%GPROJ(NPRO), &
         ...
         DO ISPINOR=0,W%WDES%NRSPINORS-1
            ...
            DO NT=1,W%WDES%NTYP
               DO NI=NIS,W%WDES%NITYP(NT)+NIS-1
                  ...
                  CALL ROTATE_VECTOR( &
                 &   KPOINTS_TRANS_FULL_PEAD%LINV(NK), &
                 &   W%GPROJ(NPROP,N,NK_OLD,ISP_OLD),WA%GPROJ(NPRO,N), &
                 &   ROT_HANDLE%MMAX,ROT_HANDLE%LDIM,ROT_HANDLE%SL(1,1,0), &
                 &   P(NT))
            ...
            DO N=1,W%WDES%NBANDS
               ...
               DO ISPINOR=0,W%WDES%NRSPINORS-1
                  ...
                  typ: DO NT=1,W%WDES%NTYP
                     ...
                     DO L=1,LMMAXC
                        ENL(NI)=ENL(NI)+CPROJXYZ(LBASE+L,N,I)*(DWDK%GPROJ(LBASE+L,N,NK,ISP))
                     ENDDO
...
! c = PREFAC, p = STRIDE
!
! For c=1 and p=1 1._q recovers Eq. (96) in PRB 63, 155107 (2001), i.e.,
...
! S is singular to working precision
            ...
            CALL M_exit(); stop
         ENDIF
! only S(1:NOCC,1:NOCC) is to be used, set the rest to 0._q
...
! calculate sum_j Qij < beta_j |wq>   (Qij = \int Q_ij(r) dr)
                  CALL OVERL_AND_APPLY_PHASE_SHIFT( &
                       &   PHASE,T_INFO,CQIJ,WDESQ,WQ,WHAM%GPROJ(:,NP))

               ELSE
! apply phase shift to the projections
                  CALL APPLY_PHASE_SHIFT_CPROJ( &
                       &   PHASE,T_INFO,WDESQ,WQ,WHAM%GPROJ(:,NP))
               ENDIF
...
!           IF (LDOIO) CALL DUMP_HAM_PEAD(" SP",W%WDES,SP)
            ...
            CALL DGEMM('N', 'N', 2* DWDK%WDES1%NPL_RED , DWDK%WDES1%NB_TOT, NSTRIP_RED, &
           &               1._q, WHAM%CW_RED(1,1), 2* DWDK%WDES1%NRPLWV_RED, &
           &               SP(1,1), NSTRIP_RED, 1._q, &
           &               DWDK%CW_RED(1,1), 2* DWDK%WDES1%NRPLWV_RED)

            IF (DWDK%WDES1%NPRO_O_RED/=0) &
            CALL DGEMM('N', 'N', DWDK%WDES1%NPRO_O_RED , DWDK%WDES1%NB_TOT, NSTRIP_RED, &
           &               1._q, WHAM%CPROJ_RED(1,1), DWDK%WDES1%NPROD_RED,  &
           &               SP(1,1), NSTRIP_RED, 1._q, &
      ...
      IF (PRESENT(RPHI)) THEN
         IF (PRESENT(RPHI_CPROJ)) THEN
            DO J=1,3
               ...
               RPHI_CPROJ(:,:,NK,ISP,J)=RPHI_CPROJ(:,:,NK,ISP,J)+WEIGHT*LATT_CUR%A(J,I)*DWDK%GPROJ(:,:)
            ENDDO
         ENDIF
      ENDIF
...
! add DWDK to DWDK_DIR
      IF (PRESENT(DIR)) THEN
         IF (PRESENT(DWDK_DIR)) THEN
            ...
            DWDK_DIR%GPROJ(:,:,NK,ISP)=DWDK_DIR%GPROJ(:,:,NK,ISP)+WEIGHT*LATT_CUR%A(DIR,I)*DWDK%GPROJ(:,:)
         ENDIF
      ENDIF
      ...
      IF (PRESENT(RPHI_1K)) THEN
         IF (PRESENT(RPHI_1K_CPROJ)) THEN
            DO J=1,3
               ...
               RPHI_1K_CPROJ(:,:,J)=RPHI_1K_CPROJ(:,:,J)+WEIGHT*LATT_CUR%A(J,I)*DWDK%GPROJ(:,:)
            ENDDO
         ENDIF
      ENDIF
...
! allocate temporary wavefunction array at NK
      ...
      directions: DO I=1,3
        ...
        &           WK%GPROJ(1,1),DWDK%GPROJ(1,1))

! unoccupied bands do not contribute
         DO N=1,WDESK%NBANDS
            IF ( (WDESK%NB_LOW+WDESK%NB_PAR*(N-1))>NOCC(ISP) ) DWDK%GPROJ(:,N)=0
         ENDDO

         WEIGHT=1._q
!        WEIGHT=W%WDES%RSPIN*W%WDES%WTKPT(NK)
! add DWDK%GPROJ to RPHI_CPROJ
         IF (PRESENT(RPHI_CPROJ)) THEN
            RPHI_CPROJ(:,:,NK,ISP,I)=RPHI_CPROJ(:,:,NK,ISP,I)+WEIGHT*DWDK%GPROJ(:,:)
         ENDIF

! add DWDK%GPROJ to DWDK_DIR
         IF (PRESENT(DIR)) THEN
            IF (PRESENT(DWDK_DIR).AND.(I==DIR)) THEN
               DWDK_DIR%GPROJ(:,:,NK,ISP)=DWDK_DIR%GPROJ(:,:,NK,ISP)+WEIGHT*DWDK%GPROJ(:,:)
            ENDIF
         ENDIF

! add DWDK%GPROJ to RPHI_1K_CPROJ
         IF (PRESENT(RPHI_1K_CPROJ)) THEN
            RPHI_1K_CPROJ(:,:,I)=RPHI_1K_CPROJ(:,:,I)+WEIGHT*DWDK%GPROJ(:,:)
         ENDIF
...
!***********************************************************************
      ...
      bands: DO IB=1,WDES%NBANDS
         ...
         DO NI=NIS,WDES1%NITYP(NT)+NIS-1
               ...
               DO LP=1,LMMAXC
                  RSUM_(1:3)=RSUM_(1:3)+ &
                 &  dIJ(1:3,LP,L,NI,ISP+ISPINOR_+2*ISPINOR)*W1%GPROJ(LP+NPRO_)*(W1%GPROJ(L+NPRO))
               ENDDO
               ENDDO
...
! local variables
      REAL(q) S_(W%WDES%NB_TOT,W%WDES%NB_TOT)
...
! test
      ...
      INTEGER ISP
      REAL(q) CQIJ(:,:,:,:)
      REAL(q) S(W%WDES%NB_TOT,W%WDES%NB_TOT)
...
!  calculate Qij < beta_j |wq>   (Qij = \int Q_ij(r) dr)
               CALL OVERL_AND_APPLY_PHASE_SHIFT(PHASE,T_INFO,CQIJ,WDESQ,WQ,WHAM%GPROJ(:,NP))
            ELSEIF (LONECENTER.AND.LUSEQIJB) THEN
!  calculate Qij(k2-k1) < beta_j |wq>   (Qij(k2-k1) = \int exp[-i(k2-k1)r]Q_ij(r) dr)
               CALL OVERL_AND_APPLY_PHASE_SHIFT_C(PHASE,T_INFO,QIJB,WDESQ,WQ,WHAM%GPROJ(:,NP))
            ELSE
               WHAM%GPROJ(:,NP)=0
            ENDIF
            IF (LPLANEWAVE) THEN
...
! communicate
      ...
      CALL M_sum_d(WDESK%COMM_KIN,S(1,1),WDESK%NB_TOT*WDESK%NB_TOT)
...
!***********************************************************************
     ...
     &   PHASE,T_INFO,CQIJ,WDES1,W1,CPROJ_RESUL)
      ...
      REAL(q) PHASE(3)
      REAL(q) CQIJ(:,:,:,:)
      REAL(q), OPTIONAL :: CPROJ_RESUL(:)
! local variables
      REAL(q) CRESUL(WDES1%NPRO)
...
!OCL NOVREC
                     DO LP=1,LMMAXC
                        CRESUL(L+NPRO)=CRESUL(L+NPRO)+ &
                           CQIJ(LP,L,NI,ISPINOR_+2*ISPINOR+1)*W1%GPROJ(LP+NPRO_)*EXP(CGDR)*CVALUE
                     ENDDO
...
! store into CPROJ_RESUL or back into W1%GPROJ
            ...
            IF (LMMAXC/=0) THEN
               DO NI=NIS,WDES1%NITYP(NT)+NIS-1
                 ...
                 &               T_INFO%POSION(3,NIP)*PHASE(3))
                  ...
                  DO L=1,LMMAXC
                     CRESUL(L+NPRO)=W1%GPROJ(L+NPRO)*EXP(CGDR)*CVALUE
                  ENDDO                                          
                  NPRO=LMMAXC+NPRO
               ENDDO
            ENDIF
...
! ) finds the points, which are within a certain cutoff around 1._q ion
...
!***********************************************************************
    SUBROUTINE SETDIJ_R(WDES,GRIDC_,GRIDUS,C_TO_US,LATT_CUR,P,T_INFO, &
     &  LOVERL,LMDIM,dIJ,IRDMAX,IDIR)
      ...
      INTEGER                    LMDIM 
      REAL(q)                    dIJ(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ)
...
!=======================================================================
      ...
      CALL M_sum_d(GRIDC%COMM, RTMP, LMDIM*LMDIM*T_INFO%NIONS*WDES%NCDIJ)
# 5130
...
!
! calculate the total 1._q centre charge density
!
!***********************************************************************
    ...
    FUNCTION ONE_CENTRE_CHARGE(WDES, T_INFO, P, ION, LMDIM, CQIJ, CRHODE)
      ...
      REAL(q)    CRHODE(LMDIM,LMDIM,WDES%NIONS,WDES%NRSPINORS*WDES%NRSPINORS)
      TYPE (potcar) :: P(T_INFO%NTYP)
