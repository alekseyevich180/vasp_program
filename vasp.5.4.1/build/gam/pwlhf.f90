...
! local variables
    ...
    CHARACTER(1) CHARAC
    REAL(q)    :: GWORK(2* GRID_FOCK%MPLWV,W%WDES%NRSPINORS) ! fock pot in real sp
    REAL(q)    :: GCHG(2* GRID_FOCK%MPLWV,W%WDES%NRSPINORS*W%WDES%NRSPINORS)
...
! determine whether redistribution is required
    IF (NCPU /= 1) THEN                    ! more than 1._q band-group
       DO_REDIS=.TRUE.
    ELSE 
       DO_REDIS=.FALSE.
    ENDIF
...
# 201
          ...
          IF (WHF%WDES%LOVERL) CALL M_bcast_d_from(WDESK%COMM_INTER,WIN(N)%GPROJ(1), &
...
! add augmentation part to charge (if required)
             IF (WHF%WDES%LOVERL) THEN
                ...
                CALL DEPSUM_TWO_BANDS_RHOLM_FULL(WIN(N)%GPROJ(:),WQ%GPROJ(:), WDESK, AUG_DES, &
             ...
             DO ISPINOR_=0,W%WDES%NRSPINORS-1
                DO NP=1,GRIDHF%RL%NP
                   SV(NP,ISP+ISPINOR_+2*ISPINOR)=SV(NP,ISP+ISPINOR_+2*ISPINOR)- &
                        (GCHG(NP,1+ISPINOR_+2*ISPINOR))*GWORK(NP,1)*WEIGHT*AEXX
                ENDDO
             ENDDO
             ENDDO
!KLIend
...
! add augmentation part to charge (if required)
       IF (W%WDES%LOVERL) THEN
          ...
          CALL DEPSUM_TWO_BANDS_RHOLM_FULL(WIN(N)%GPROJ(:),WIN(N)%GPROJ(:), WDESK, AUG_DES, &
...
! passed variables
    ...
    REAL(q)   SV(:,:)
    REAL(q)   SV_OLD(:, :)
    REAL(q)    CCORR(:,:), CCORR_LOCAL(:,:)
    REAL(q) CRHODE_CORR(:,:,:,:)
! local variables

! charge and potential
    REAL(q) GWORK(2* GRID%MPLWV,W%WDES%NRSPINORS*W%WDES%NRSPINORS) ! fock pot in real sp
    TYPE (wavedes1) WDESK
    TYPE (wavefun1) W1, WQ
    REAL(q) :: WEIGHT
    ...
    REAL(q),      ALLOCATABLE :: CPROJK(:,:)      ! stores wave character of curr. strip
    REAL(q),TARGET,ALLOCATABLE:: CPROJM(:)        ! stores the wave character of ; wavefunctions
    REAL(q),      ALLOCATABLE :: CRHOLM(:)        ! augmentation occupancy matrix
    REAL(q) ::    CLOCAL, CSUM
...
!==========================================================================
    fftn: DO N=NPOS,NPOS+NSTRIPN-1
       ...
       ENDDO
    ENDDO fftn
! if LOVERL copy the projectors into GPROJ array
    IF (W%WDES%LOVERL) THEN
       DO N=NPOS,NPOS+NSTRIPN-1
          CPROJK(1:W%WDES%NPROD,(N-NPOS)*NCPU+NODE_ME_I)= &
               W%GPROJ(1:W%WDES%NPROD,N,NK,ISP)
       ENDDO
    ENDIF
...
! wavefunction of current band (NQ,MQ) in real space (WQ%CR) and character (WQ%GPROJ)
...
! add augmentation part to charge (if required)
             IF (W%WDES%LOVERL) THEN
                ...
                CALL DEPSUM_TWO_BANDS_RHOLM_FULL(CPROJK(:,N),WQ%GPROJ(:), WDESK, AUG_DES, &
                      TRANS_MATRIX_FOCK, CRHOLM,W%WDES%LOVERL )
# 695
...
! alternatively the spin order could be reversed in the GWORK
                   CLOCAL=CLOCAL+(GWORK(NP,1+ISPINOR))*SV_OLD(NP,ISP+ISPINOR)
                ENDDO
             ENDDO
             ...
             DO ISPINOR=0,W%WDES%NRSPINORS*W%WDES%NRSPINORS-1
                DO NP=1,GRID%RL%NP
                   SV(NP,ISP+ISPINOR)=SV(NP,ISP+ISPINOR)+ &
                        (GWORK(NP,1+ISPINOR))*CSUM*WEIGHT
                ENDDO
             ENDDO
! the final augmentation contributions are real as well
             CALL DEPSUM_TWO_BANDS( CPROJK(:,N), WQ%GPROJ(:), WDESK, ISP, &
...
!-----------------------------------------------------------------------------
    ...
    CALL M_sum_d(W%WDES%COMM_INTER,CCORR_LOCAL(1,1),W%WDES%NB_TOT*W%WDES%NB_TOT)
...
!***********************************************************************
  ...
  SUBROUTINE LHF(HAMILTONIAN,GRID, GRID_SOFT, LATT_CUR, SYMM, T_INFO, INFO, &
    ...
    REAL(q),TARGET :: CRHODE(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ) ! 1._q center occupancy matrix
    REAL(q) CDIJ(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ)   ! 1._q centre strenght parameters of potentials
...
! if SWITCH_TO_OEP is selected the on site terms
    ...
    REAL(q) CDIJC(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ)  ! 1._q centre correction
    REAL(q) CRHODE_CORR(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ)! 1._q centre occupancies to correct potential
...
! get pointers for redistributed wavefunctions
      IF (DO_REDIS) THEN
        ...
        CALL SET_GPOINTER(CPROJ_RED, NPROD_RED, NB_TOT, W%GPROJ(1,1,NK,ISP))
        CALL SET_WPOINTER(CH_RED,   NRPLWV_RED, NCPU*NSTRIP, CH(1,1))
      ELSE
        CW_RED    => W%CPTWFP(:,:,NK,ISP)
        CPROJ_RED => W%GPROJ(:,:,NK,ISP)
        CH_RED    => CH(:,:)
      ENDIF
...
! CW_ is then redistributed up to and including 1...NPOS+NSTRIP_ACT
      ...
      CALL M_sum_d(WDES%COMM,CSLATER(1,1,NK,ISP),NB_TOT*NB_TOT)
    ENDDO kpoint
    ENDDO spin
!=======================================================================
!
...
# 1081
    ...
    DO ISP=1,WDES%NCDIJ
       DO NP=1,GRID_SOFT%RL%NP
          CDCHF_=CDCHF_-POT%RG(NP,ISP)*(0.5_q/GRID_SOFT%NPLWV)*(CHG%RG(NP,ISP))
       ENDDO
    ENDDO
    ...
    DO ISP=1,WDES%NCDIJ,MAX(1,WDES%NCDIJ-1)
       DO NP=1,GRID_SOFT%RL%NP
          CDCHF =CDCHF -POT%RG(NP,ISP)*(0.5_q/GRID_SOFT%NPLWV)*(CHG_CORE%RG(NP,1))
       ENDDO
    ENDDO
    CALL M_sum_z(WDES%COMM_INTER,CDCHF,1)
    CDCHF=CDCHF+CDCHF_
...
# 1176
       ...
       CALL M_sum_d(WDES%COMM_INTER, CRHODE_CORR, SIZE(CRHODE_CORR))
# 1187
...
! the update of the 1._q center terms is only correct for
! FRICTION=2
...
!*********************************************************
             ...
             CDCHF_=CDCHF_-POT%RG(NP,ISP)*(CHG%RG(NP,ISP))*(0.5_q/GRID_SOFT%NPLWV)
          ENDDO
       ENDDO
       CALL M_sum_z(WDES%COMM_INTER,CDCHF_,1)
...
!***********************************************************************
    ...
    SUBROUTINE CORE_LOCAL(GRIDC, P, T_INFO, LATT_CUR, NCDIJ, SV, D_CORE)
      ...
      REAL(q)   SV(GRIDC%MPLWV*2,NCDIJ) ! local exchange potential
      COMPLEX(q) SV_REC(GRIDC%MPLWV,NCDIJ) ! Slater energy density
      REAL(q)    D_CORE(T_INFO%NIONS,NCDIJ)
! local
...
! and potential
      ...
      ion: DO NI=NIS,NIADD+NIS-1
         ...
         DO NC=1,GRIDC%RC%NCOL
           ...
           DO N1P=0,GRIDC%RC%NROW-1
              ...
              DO ISP=1,NCDIJ
                 D_CORE(NI,ISP)=D_CORE(NI,ISP)+WORK(NG)*FACTM* (SV_REC(NG,ISP)*CONJG(CEXPF))
              ENDDO
           ENDDO
         ENDDO
      ENDDO ion
      ...
      CALL M_sum_d(GRIDC%COMM, D_CORE,SIZE(D_CORE))
# 1440
...
!***********************************************************************
    ...
    SUBROUTINE CHARGE_SYM_REAL_SPACE( WDES, GRID_SOFT, CHDEN, SYMM, T_INFO, LATT_CUR)
      ...
      TYPE (type_info)   T_INFO
      TYPE (latt)        LATT_CUR
      REAL(q)  CHDEN(GRID_SOFT%MPLWV*2,WDES%NCDIJ)! charge density
! local
      INTEGER ISP
...
!***********************************************************************
    ...
    SUBROUTINE POTENTIAL_FROM_ENERGY(GRID, SV_SLATER, SV_CORR, SV, CHDEN, CHDEN_CORE, NCDIJ)
      ...
      REAL(q)   CHDEN(:,:)
      REAL(q)   CHDEN_CORE(:,:)
      REAL(q),ALLOCATABLE ::   CHDEN_TOTAL(:,:)
      ...
      IF (NCDIJ==1 .OR. NCDIJ==2) THEN
         ...
         ENDDO
      ELSE
         ALLOCATE(CHDEN_TOTAL(GRID%MPLWV*2,NCDIJ))
         DO ISP=1,NCDIJ
...
! axis, which is achived by conjugating the energy density
            DO NP=1,GRID%RL%NP
               ...
               SV(NP,ISP)=(SV_SLATER(NP,ISP)+SV_CORR(NP,ISP))
            ENDDO
         ENDDO
...
!***********************************************************************
    ...
    SUBROUTINE SETDIJ_FOCKAE(WDES, GRIDC_,GRIDUS,C_TO_US,LATT_CUR,P,T_INFO, LOVERL, &
      ...
      REAL(q),TARGET :: CVTOT_(GRIDC_%MPLWV*2,WDES%NCDIJ)
      LOGICAL  LOVERL
!  work arrays
      LOGICAL LADDITIONAL
      REAL(q)   DLM(256)
...
! storage convention to (total,magnetization)
     ...
     &          YLM(IRDMAX,LMYDIM),NLI(IRDMAX))
      IF (LADDITIONAL) THEN
         ALLOCATE(CVTOT(GRIDUS%MPLWV*2),CWORK(GRIDC_%MPLWV*2))
      ENDIF
...
! reduce CTMP
      ...
      CALL M_sum_d(GRIDC%COMM, CTMP, LMDIM*LMDIM*T_INFO%NIONS*WDES%NCDIJ)
# 1811
...
! only terms with same quantum numbers L L' and M M' are non-0._q
!-----------------------------------------------------------------------
