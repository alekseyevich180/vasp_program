...
! use gauge transformation for 0._q moment terms
...
! for NICS (converse approach) this should be set to 0._q
...
!***********************************************************************************
    ...
    SUBROUTINE ALLOCATE_AVEC(AVEC, AVTOT, GRID, GRIDC )
      ...
      IF (ORBITALMAG) THEN
         ALLOCATE(AVEC(GRID%MPLWV*2,3),AVTOT(GRIDC%MPLWV,3))
...
! this loop goes over all possible l'', m'' being non 0._q in CG coefficien
               DO IC1=ISTART1,IEND1-1
! JS(IC1)) is a compound index for l'' and m''
! JL(IC1)) is the l quantum number L (0._q based)
...
! (see SET_RL_GRID() in mgrid.F, and M_divide() in mpi.F)
         ...
         CALL M_bcast_d(COMM_INTER, AVEC(1,I), GRID%RL%NP)
# 1024
...
!***********************************************************************************
    ...
    SUBROUTINE CURRENT( W, GRID_SOFT, GRIDC, GRIDUS, C_TO_US, SOFT_TO_C, P, LATT_CUR, &
      ...
      REAL(q), POINTER  :: AVEC(:,:)     ! vector potential on course grid
      ...
      TYPE (type_info):: T_INFO        ! type information
      INTEGER         :: LMDIM         !
      REAL(q)         :: CRHODE(LMDIM,LMDIM,W%WDES%NIONS,W%WDES%NCDIJ)
      REAL(q)         :: CDIJ(LMDIM,LMDIM,W%WDES%NIONS,W%WDES%NCDIJ)
...
! local
      ...
      TYPE (wavedes1)    WDES1
      TYPE (potcar), POINTER :: PP
      REAL(q) CDUMMY(LMDIM,LMDIM,W%WDES%NIONS,W%WDES%NCDIJ)

! Gilles: allocate instead of automatic arrays
      COMPLEX(q), ALLOCATABLE :: CWORK1(:)
      COMPLEX(q), ALLOCATABLE :: CWORK2(:)
      REAL(q), ALLOCATABLE      :: JVEC_TMP(:,:)
      COMPLEX(q), ALLOCATABLE :: JVEC(:)
      REAL(q), ALLOCATABLE      :: CHDEN_TMP(:)
...
! now merge the current from all nodes
         ...
         CALL M_sum_d(W%WDES%COMM_INTER, JVEC_TMP(1,I), W%WDES%GRID%RL%NP)
# 1223
...
! bring to full grid
      ...
      CALL M_sum_d(W%WDES%COMM_INTER, CHDEN_TMP(1), W%WDES%GRID%RL%NP)
# 1241
...
! Note that MOMTOMOM is dimensionless. In SI units this is difficult to see. In cgs units 1._q has to use that
...
!      IF (IU0>=0) WRITE (IU0,'("induced magnetic moment   *1E6",3E16.7)') MAGDIPOLOUT*1E6
      ...
      IF (.NOT. LBFCONST) THEN
! here 1._q-centre terms for converse method
         MU_PARA_ONE_CTR=0
         MU_DIA_ONE_CTR=0
         MU_LQ_ONE_CTR=0
...
! add paramagnetic 1._q centre terms for each atom
...
! type of magnetic ion
      ...
      ELSE
! here 1._q-centre for direct method
...
!
! for the converse approach the 1._q centre term for the atom where the magnetic moment
! is calculated as
!
...
!
! 1._q centre term for correction of 1._q-center augmentation via moment
!
...
!
! 1._q centre term for each atom in constant magnetic field
!
...
!****************************** CALC_MU_MAGATOM ************************************
!
! To calculate 1._q-centre magnetic moment on atom where external moment is placed.
...
!                         * MOMTOMOM
!
! these are collected into 1._q array and the moment is calculated (see explanation
! below)
!
! p = -i \hbar nabla
!
!***********************************************************************************
    ...
    SUBROUTINE CALC_MU_MAGATOM(PP,CRHODE,MU_PARA_ONE_CTR,MU_DIA_ONE_CTR,L_DO_AUG)
      ...
      REAL(q) :: CRHODE(:,:)         ! 1._q center occupancy matrix
      REAL(q) :: MU_PARA_ONE_CTR(3), MU_DIA_ONE_CTR(3)  ! 1._q centre moments
      LOGICAL :: L_DO_AUG
...
!                                (((CRHODE(LM1+M1-1,LM0+M0-1))+CRHODE(LM0+M0-1,LM1+M1-1))/2))
! multipy by -i and account for negative charge
...
!****************************** CALC_B_MAGATOM *************************************
!
! To calculate 1._q-centre contribution to magnetic field at atomic nuclei.
...
!                         * MOMTOMOM
!
! These are collected into 1._q array
!
...
!                                 TMP(K)*(CRHODE(LM1+M1-1,LM0+M0-1)+(CRHODE(LM0+M0-1,LM1+M1-1)))/2)
                                 (TMP(K)*((CRHODE(LM1+M1-1,LM0+M0-1))+CRHODE(LM0+M0-1,LM1+M1-1)))/2)
                        ENDDO
                     ENDIF
                     ENDIF
...
! collect 1._q-centre contributions to field
         IF (IDP.EQ.1) THEN
...
! #define justatest
    SUBROUTINE BLOCH_CURRENT( W, GRID_SOFT, GRIDC, GRIDUS, C_TO_US, SOFT_TO_C, P, LATT_CUR, &
      ...
      REAL(q)           :: SV(W%WDES%GRID%MPLWV*2,W%WDES%NCDIJ)
      REAL(q)         :: EFERMI
      TYPE (type_info):: T_INFO        ! type information
      INTEGER         :: LMDIM         !
      REAL(q)         :: CRHODE(LMDIM,LMDIM,W%WDES%NIONS,W%WDES%NCDIJ)
...
! store i d/ d k<beta|   | u_nk> = -d/ d (i k)<beta|   | u_nk>
               W1(I)%GPROJ=-RPHI_CPROJ(:,N,NK,ISP,I)
               CALL FFTWAV_W1(W1(I))
               W1(I)%LDO=.TRUE.

               IF (NONLR_S%LREAL ) THEN
! calculate  <beta| d/ d (i k) u_nk>
                  CALL RPRO1(NONLR_S,WDES1,W1(I))                        !  W1(I)%GPROJ = <beta | d/dik u_nk>
# 3737

               ELSE
                  CALL PROJ1(NONL_S,WDES1,W1(I))                         !  W1(I)%GPROJ = <beta | d/dik u_nk>
# 3746
...
! 1._q-centre terms from atom where magnetic moment is placed
      MU_PARA_ONE_CTR=0
      MU_DIA_ONE_CTR =0
! type of magnetic ion
      NT=T_INFO%ITYP(MAGATOM)
...
!GIL 24 01 11! 1._q-centre terms from all atome
...
!GIL 24 01 11                     MU_PARA_MIXED(2)=MU_PARA_MIXED(2)-(CRHODE1(LM0+M0-1,LM1+M1-1,NIP,ISP))*VIJ_PARA(LM0+M0-1,LM1+M1-1,3)
!GIL 24 01 11                     MU_PARA_MIXED(3)=MU_PARA_MIXED(3)+(CRHODE1(LM0+M0-1,LM1+M1-1,NIP,ISP))*VIJ_PARA(LM0+M0-1,LM1+M1-1,2)
!GIL 24 01 11                     MU_DIA_MIXED(2) =MU_DIA_MIXED(2) -(CRHODE1(LM0+M0-1,LM1+M1-1,NIP,ISP))*VIJ_DIA (LM0+M0-1,LM1+M1-1,3)
!GIL 24 01 11                     MU_DIA_MIXED(3) =MU_DIA_MIXED(3) +(CRHODE1(LM0+M0-1,LM1+M1-1,NIP,ISP))*VIJ_DIA (LM0+M0-1,LM1+M1-1,2)
!GIL 24 01 11                  ELSE IF (ICART==2) THEN
!GIL 24 01 11                     MU_PARA_MIXED(3)=MU_PARA_MIXED(3)-(CRHODE1(LM0+M0-1,LM1+M1-1,NIP,ISP))*VIJ_PARA(LM0+M0-1,LM1+M1-1,1)
!GIL 24 01 11                     MU_PARA_MIXED(1)=MU_PARA_MIXED(1)+(CRHODE1(LM0+M0-1,LM1+M1-1,NIP,ISP))*VIJ_PARA(LM0+M0-1,LM1+M1-1,3)
!GIL 24 01 11                     MU_DIA_MIXED(3) =MU_DIA_MIXED(3) -(CRHODE1(LM0+M0-1,LM1+M1-1,NIP,ISP))*VIJ_DIA (LM0+M0-1,LM1+M1-1,1)
!GIL 24 01 11                     MU_DIA_MIXED(1) =MU_DIA_MIXED(1) +(CRHODE1(LM0+M0-1,LM1+M1-1,NIP,ISP))*VIJ_DIA (LM0+M0-1,LM1+M1-1,3)
!GIL 24 01 11                  ELSE IF (ICART==3) THEN
!GIL 24 01 11                     MU_PARA_MIXED(1)=MU_PARA_MIXED(1)-(CRHODE1(LM0+M0-1,LM1+M1-1,NIP,ISP))*VIJ_PARA(LM0+M0-1,LM1+M1-1,2)
!GIL 24 01 11                     MU_PARA_MIXED(2)=MU_PARA_MIXED(2)+(CRHODE1(LM0+M0-1,LM1+M1-1,NIP,ISP))*VIJ_PARA(LM0+M0-1,LM1+M1-1,1)
!GIL 24 01 11                     MU_DIA_MIXED(1) =MU_DIA_MIXED(1) -(CRHODE1(LM0+M0-1,LM1+M1-1,NIP,ISP))*VIJ_DIA (LM0+M0-1,LM1+M1-1,2)
!GIL 24 01 11                     MU_DIA_MIXED(2) =MU_DIA_MIXED(2) +(CRHODE1(LM0+M0-1,LM1+M1-1,NIP,ISP))*VIJ_DIA (LM0+M0-1,LM1+M1-1,1)
!GIL 24 01 11                  ENDIF
!GIL 24 01 11
...
! 1._q center (on site) "occupancy" i.e
!
...
! we need i d/ d k<beta|   | u_nk> = -d/ d (i k)<beta|   | u_nk>
           CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)=CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)- &
                WEIGHT0*RPHI_CPROJ(L+LMBASE,N,NK,ISP,ICART)*(W0%GPROJ(LP+LMBASE_,N,NK,ISP)) &
               -WEIGHT0*W0%GPROJ(L+LMBASE,N,NK,ISP)*(RPHI_CPROJ(LP+LMBASE_,N,NK,ISP,ICART))
...
! charge is stored in channel 1._q
      CTMP=0
      DO NI=1,T_INFO%NIONS
      ...
      CALL M_sum_d(WDES%COMM_INB,CTMP,LMDIM*LMDIM*T_INFO%NIONS)
# 4310
...
!-----------------------------------------------------------------------
! nabla operator increases the maximum L 1._q-center number by 1._q
      LYMAX  =MAXL1(PP)*2
      LYMAXP1=LYMAX+1
...
!*********************************** CALC_VIJ **************************************
!
! To calculate 1._q-centre integrals of mixed contributions the magnetic moment on
...
!***********************************************************************************
    ...
    SUBROUTINE CALC_VIJ_ONECTR(PP,LMDIM,VIJ_PARA,VIJ_DIA)
      ...
      TYPE (potcar), POINTER :: PP   ! pseudopotential descriptor
      INTEGER LMDIM
      COMPLEX(q) :: VIJ_PARA(LMDIM,LMDIM,3), VIJ_DIA(LMDIM,LMDIM,3)  ! 1._q centre velocities
