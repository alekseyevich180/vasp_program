...
! Ti = 132 pm = 1.32 A
!
! Have a look at which radius wavefunction is nearly 0._q
...
!
! seek number of occupied 1._q electron states
...
! local variables
      ...
      REAL(q) :: ENMAX,DERIV
!
! from now on 1._q could use a smaller grid with catom%irmax gridpoints, because
! bessel functions have beyond this point 0._q value and derivative.
!
...
!
    ...
    ENDDO
!
! Only if all energy entries are all 0._q exit, because elements like Rb_pv have a energy of
! approx 0.00x in the l=2 channel so in the POTCAR the entry is 0._q.
! in l
!
...
!
       STEP = 0.005_q
!
! Set up values so that 1._q can calculate the derivative of the derivative function
!
       E(1) = E(2) - STEP
...
! if error of wavefunctions is smaller as 1._q of derivatives no self coinsistnecy
! can be reached
!
...
!     gridpoint. Now it is the supplied 1._q. Choosing the
!     the nearest point on the logarithmic grid is not desireable
!     because in the range of 4 Angstroem and more only 1._q or two
...
! local
      ...
      REAL(q) :: PROJ(1:NBF)             ! projections of wavefunctions onto current 1._q
      REAL(q) :: LINDIST
      REAL(q) :: BJ2,BJP,BJP2
...
!
      nbasis: DO I=1,NBF
!
! Calculate the two last terms of the series. The first is set to 1._q
! and the last 1._q so that the derivative at RMAX is 0._q. Mind that
...
! !!!!!!!
!
! seems that A(I+1,I) is always 1._q when A(I,I) is 1._q, why ?
