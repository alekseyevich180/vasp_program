...
! < d/dR p_j | phi > is returned in WXI%GPROJ,
...
!***********************************************************************
  ...
  SUBROUTINE LR_HAMIL(GRID,INFO,LATT_CUR,NONLR_S,NONL_S,W0, WXI, WDES, &
    ...
    REAL(q) CRHODE(LMDIM,LMDIM,WDES%NCDIJ)
...
!  to have uniform stride for result array
        ...
        i2: DO NP=1,NSIM
           ...
           ENDDO
           WXI%GPROJ(:,N,NK,ISP)=WTMP(NP)%GPROJ
...
!=======================================================================
      ...
      CALL M_sum_d(WDES%COMM_INTER, CRHODE, LMDIM*LMDIM*WDES%NCDIJ)
      CALL M_sum_d(WDES%COMM_KINTER, CRHODE, LMDIM*LMDIM*WDES%NCDIJ)
# 245
...
! e(0) is the 0._q order eigen energy
...
!***********************************************************************
    ...
    SUBROUTINE HAMILTMU_LR( &
      ...
      REAL(q)    CRHODE(LMDIM,LMDIM,WDES1%NRSPINORS*WDES1%NRSPINORS)
...
!OCL NOVREC
            DO LP=1,LMMAXC
               CRHODE(LP,L,ISPINOR_+2*ISPINOR+1)=CRHODE(LP,L,ISPINOR_+2*ISPINOR+1)+ &
                    WEIGHT*W0_1(NP)%GPROJ(L+LMBASE)*(WTMP(NP)%GPROJ(LP+LMBASE_))+ &
                    WEIGHT*WTMP(NP)%GPROJ(L+LMBASE)*(W0_1(NP)%GPROJ(LP+LMBASE_))
            ENDDO
            ENDDO
         ENDIF
