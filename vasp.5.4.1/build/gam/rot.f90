...
!-----Arrays for gradient
   REAL(q) CHAM(WDES%NB_TOT,WDES%NB_TOT,WDES%NKPTS,WDES%ISPIN)  ! sub space Hamiltionian in space spanned by orbitals
   REAL(q) CHF(WDES%NB_TOT,WDES%NB_TOT,WDES%NKPTS,WDES%ISPIN)   ! search direction in the space spanned by orbitals
   REAL(q) CROT, CB_
...
! work array
   ...
   REAL(q),ALLOCATABLE,TARGET::   CPROF(:,:)     ! stores the projected coefficients corresponding to CF
   COMPLEX(q),ALLOCATABLE,TARGET:: CFOLD(:,:) ! stores the plane wave coefficients of the previous search
   REAL(q),ALLOCATABLE :: CHAMP(:,:,:,:)         ! stores the preconditioned gradient
   TYPE (wavefuna)    WA             ! pointer to W%CPTWFP(:,:;NK,ISP)
   TYPE (wavefuna)    WA_F           ! pointer to W_F%CPTWFP(:,:;NK,ISP)
   COMPLEX(q), POINTER :: CF_RED(:,:)
   REAL(q)   , POINTER ::    CPROW_RED(:,:),CPROF_RED(:,:)
   REAL(q), ALLOCATABLE :: COVL(:,:), CUNI(:,:), CEIDB(:,:)
...
kpoint: DO NK=1,WDES%NKPTS
      ...
      CALL OVERL(WDES1, LOVERL,LMDIM,CQIJ(1,1,1,ISP), W%GPROJ(1,1,NK,ISP),CPROW(1,1))
...
! use as source the wavefunctions distributed over bands (if possible)
         ...
         ENDDO
         DO M=1,WDES%NPROD
            W1(NP)%GPROJ(M)=W%GPROJ(M,N,NK,ISP)
         ENDDO
...
! test
         ...
         CALL OVERL(WDES1, .TRUE., LMDIM, CDIJ(1,1,1,ISP), W1(NP)%GPROJ(1), CPROF(1,NP))
...
! in the subspace spanned by the present set of wavefunctions
      ...
      CALL M_sum_d(WDES%COMM_KIN,CHAM(1,NPOS_RED,NK,ISP),NB_TOT*NSTRIP_RED)
...
!     and needs to be refilled
      DO N1=NPOS_RED,NPOS_RED+NSTRIP_RED-1
         DO N2=N1+1,NB_TOT
            CHAM(N1,N2,NK,ISP)=(CHAM(N2,N1,NK,ISP))
         ENDDO
         W%CELTOT(N1,NK,ISP)=CHAM(N1,N1,NK,ISP)
         DO N2=1,N1-1
            CHAM(N2,N1,NK,ISP)=(CHAM(N1,N2,NK,ISP))
         ENDDO
      ENDDO

! set all matrix elements beyond NB_TOTK to 0._q
...
! subtract - < phi_n1 | H_local  | phi_n2 >  | phi_n2>
      CALL DGEMM('N', 'N', 2*  WDES1%NPL_RED , NSTRIP_RED, NB_TOT, -1._q, &
     &               WA%CW_RED(1,1), 2* WDES1%NRPLWV_RED, CHAM(1,NPOS_RED,NK,ISP), NB_TOT,  &
     &               1._q, CF(1,1), 2* WDES1%NRPLWV_RED)

! subtract < phi_n1 |  H_local | phi_n2 > Q_ij  < p_j| phi_n2 > from orbital character
      IF (LOVERL) THEN
         CALL DGEMM('N', 'N',  WDES1%NPRO_RED, NSTRIP_RED, NB_TOT, -1._q, &
     &               CPROW_RED(1,1), WDES1%NPROD_RED, CHAM(1,NPOS_RED,NK,ISP), NB_TOT,  &
     &               1._q, CPROF(1,1), WDES1%NPROD_RED)
      ENDIF
...
! use local contributions only
         ...
         ENDDO
         CALL M_sum_d(WDES%COMM_INB, CB_, 1)
...
! energy change for a unit step along gradient
      IF (IROT==1 .OR. IROT>=3 ) THEN
         ...
         W_G%GPROJ(:,N,NK,ISP)=0
         DO NPRO_=1,WDES%NPRO
            W_G%GPROJ(NPRO_,N,NK,ISP)=WSEARCH%GPROJ(NPRO_)
         ENDDO
...
!ENDDO
            CALL OVERL(WDES1, LOVERL,LMDIM,CQIJ(1,1,1,ISP), W_G%GPROJ(1,1,NK,ISP),CPROW(1,1))

            IF (WDES%DO_REDIS) THEN
               IF (LOVERL) CALL REDIS_PROJ(WDES1, NBANDS, CPROW(1,1))
               CALL REDIS_PROJ(WDES1, NBANDS, W%GPROJ(1,1,NK,ISP))
               CALL REDIS_PROJ(WDES1, NBANDS, W_G%GPROJ(1,1,NK,ISP))
            ENDIF

            COVL=0
            CALL ORTH2( &
                 W%CPTWFP(1,1,NK,ISP),W_G%CPTWFP(1,1,NK,ISP),W%GPROJ(1,1,NK,ISP), &
                 CPROW(1,1),NB_TOT, &
           ...
           &    COVL(:,W%WDES%NB_TOTK(NK,ISP)+1:)=0; COVL(W%WDES%NB_TOTK(NK,ISP)+1:,:)=0
            ...
            IF (WDES1%NPL_RED /=0 ) &
            CALL DGEMM('N', 'N', 2* WDES1%NPL_RED , NB_TOT, NB_TOT, -1._q, &
                 W%CPTWFP(1,1,NK,ISP), 2* WDES1%NRPLWV_RED, COVL(1,1), NB_TOT,  &
                 1._q, W_G%CPTWFP(1,1,NK,ISP), 2* WDES1%NRPLWV_RED)

            IF ( WDES1%NPRO_RED /=0) &
            CALL DGEMM('N', 'N', WDES1%NPRO_RED , NB_TOT, NB_TOT, -1._q, &
                 W%GPROJ(1,1,NK,ISP), WDES1%NPROD_RED, COVL(1,1), NB_TOT,  &
                 1._q, W_G%GPROJ(1,1,NK,ISP), WDES1%NPROD_RED)
      ...
      IF (IROT==4 .OR. IROT==5 .OR. IROT==7) THEN
         ...
         IF (IROT==5) THEN
            ...
            DO NK=1,WDES%NKPTS
               ...
               CALL SETWDES(WDES,WDES1,NK)
               IF (WDES%DO_REDIS) CALL REDIS_PROJ(WDES1, NBANDS, W_G%GPROJ(1,1,NK,ISP))
               CALL LINCOM('F',W_G%CPTWFP(:,:,NK,ISP),W_G%GPROJ(:,:,NK,ISP),CHAMP(1,1,NK,ISP), &
                    W%WDES%NB_TOTK(NK,ISP),W%WDES%NB_TOTK(NK,ISP), &
                    WDES1%NPL_RED,WDES1%NPRO_RED,WDES1%NRPLWV_RED,WDES1%NPROD_RED,NB_TOT, &
                    W_G%CPTWFP(:,:,NK,ISP),W_G%GPROJ(:,:,NK,ISP))
               IF (WDES%DO_REDIS) CALL REDIS_PROJ(WDES1, NBANDS, W_G%GPROJ(1,1,NK,ISP))

               IF (WDES%DO_REDIS) CALL REDIS_PROJ(WDES1, NBANDS, W_F%GPROJ(1,1,NK,ISP))
               CALL LINCOM('F',W_F%CPTWFP(:,:,NK,ISP),W_F%GPROJ(:,:,NK,ISP),CHAMP(1,1,NK,ISP), &
                    W%WDES%NB_TOTK(NK,ISP),W%WDES%NB_TOTK(NK,ISP), &
                    WDES1%NPL_RED,WDES1%NPRO_RED,WDES1%NRPLWV_RED,WDES1%NPROD_RED,NB_TOT, &
                    W_F%CPTWFP(:,:,NK,ISP),W_F%GPROJ(:,:,NK,ISP))
               IF (WDES%DO_REDIS) CALL REDIS_PROJ(WDES1, NBANDS, W_F%GPROJ(1,1,NK,ISP))
            ENDDO
            ENDDO
...
! preconditioned gradient equal gradient
      ...
      ELSE IF (KPOINTS%SIGMA>0 .AND. ICEL == 1) THEN
         ...
         ENDDO
      ENDIF
! set selected components of the gradient to 0._q
! in order to test things
!test
!      W_G%CPTWFP=0;  W_G%GPROJ=0; DE1=0 ; ORT1=0
      ...
      IF (IROT==1 .OR. IROT>=3) THEN
        ...
        DO N=1,WDES%NBANDS
          ...
          DO NPRO_=1,WDES%NPROD
            W%GPROJ(NPRO_,N,NK,ISP)=W%GPROJ(NPRO_,N,NK,ISP)-W_F%GPROJ(NPRO_,N,NK,ISP)*BCSTEP
          ENDDO
        ENDDO
        ENDDO
        ENDDO
...
# 1018
        ...
        IF (WDES%DO_REDIS) CALL REDIS_PROJ(WDES1, NBANDS, W%GPROJ(1,1,NK,ISP))
        CALL LINCOM('F',W%CPTWFP(:,:,NK,ISP),W%GPROJ(:,:,NK,ISP),CEIDB(1,1), &
             W%WDES%NB_TOTK(NK,ISP),W%WDES%NB_TOTK(NK,ISP), &
             WDES1%NPL_RED,WDES1%NPRO_RED,WDES1%NRPLWV_RED,WDES1%NPROD_RED,NB_TOT, &
             W%CPTWFP(:,:,NK,ISP),W%GPROJ(:,:,NK,ISP))
        IF (WDES%DO_REDIS) CALL REDIS_PROJ(WDES1, NBANDS, W%GPROJ(1,1,NK,ISP))

        IF (WDES%DO_REDIS) CALL REDIS_PROJ(WDES1, NBANDS, W_F%GPROJ(1,1,NK,ISP))
        CALL LINCOM('F',W_F%CPTWFP(:,:,NK,ISP),W_F%GPROJ(:,:,NK,ISP),CEIDB(1,1), &
             W%WDES%NB_TOTK(NK,ISP),W%WDES%NB_TOTK(NK,ISP), &
             WDES1%NPL_RED,WDES1%NPRO_RED,WDES1%NRPLWV_RED,WDES1%NPROD_RED,NB_TOT, &
             W_F%CPTWFP(:,:,NK,ISP),W_F%GPROJ(:,:,NK,ISP))
        IF (WDES%DO_REDIS) CALL REDIS_PROJ(WDES1, NBANDS, W_F%GPROJ(1,1,NK,ISP))
...
! must check whether ILOEW=2 can swap bands (if not 1._q might comment this call out)
        IF (WDES%DO_REDIS) CALL REDIS_PROJ(WDES1, NBANDS, W_F%GPROJ(1,1,NK,ISP))
        CALL LINCOM('F',W_F%CPTWFP(:,:,NK,ISP),W_F%GPROJ(:,:,NK,ISP),CEIDB(1,1), &
             W%WDES%NB_TOTK(NK,ISP),W%WDES%NB_TOTK(NK,ISP), &
             WDES1%NPL_RED,WDES1%NPRO_RED,WDES1%NRPLWV_RED,WDES1%NPROD_RED,NB_TOT, &
...
! and 1._q might need to "over-relax" e.g. FRICTION=SQRT(BTRIAL*100)
         IF (ICEL==0) THEN
...
! hence the velocities are quenched to 0._q
         DESUM0=GAMMA*ORT
         ICOUNT=ICOUNT+1
         IF (DESUM0<0) THEN
            ICOUNT=0
...
! (ORT should be essentially 0._q)
...
!-----------------------------------------------------------------------
      ...
      DO N=1,WDES%NBANDS
        ...
        ENDDO
        DO NPRO_=1,WDES%NPROD
          W_F%GPROJ(NPRO_,N,NK,ISP)=W_G%GPROJ(NPRO_,N,NK,ISP)+W_F%GPROJ(NPRO_,N,NK,ISP)*GAMMA
        ENDDO
      ENDDO
      ENDIF
...
# 1234
            ...
            W%GPROJ(NPRO_,N,NK,ISP)=W%GPROJ(NPRO_,N,NK,ISP)-W_F%GPROJ(NPRO_,N,NK,ISP)*BSTEP
          ENDDO
...
!************************ SUBROUTINE ROTDIA ****************************
!
! this subroutine calculates the rotation matrix  for 1._q k-point
...
!                   0._q (no conjugation of those elements)
...
!***********************************************************************
      ...
      REAL(q) CEIDB(WDES%NB_TOT,WDES%NB_TOT)
      REAL(q), OPTIONAL :: CHAMP(WDES%NB_TOT,WDES%NB_TOT,WDES%NKPTS,WDES%ISPIN) 
...
!  CEIDB = 1 - CHF* BSTEP
! for 1._q k-point, where CHF is the Loewdin-rotation matrix
...
!***********************************************************************
      ...
      IMPLICIT NONE
      INTEGER NBANDS
      REAL(q) CHF(NBANDS,NBANDS)
      REAL(q) CTMP(NBANDS,NBANDS),CUNI(NBANDS,NBANDS), &
     &     CEIDB(NBANDS,NBANDS)
      REAL(q) BSTEP
! local
      INTEGER N1,N2
...
!
! for 1._q k-point, the Loewdin-perturbation CHF matrix must be supplied
...
! so avoid calling 1 routines
      IF ( LscaLAPACK .AND. .FALSE. ) THEN
         ...
         CALL M_sum_d(COMM,CUNI, NBANDS*NBANDS)
      ELSE
...
!      CALL PARALLEL_GGEMM(PGEMM_HANDLE, 'N','T', NBANDS, NBANDS, NBANDS, 1._q, CTMP, &
!     &             NBANDS, CUNI, NBANDS, 0._q, CEIDB, NBANDS)
      CALL DGEMM('N','T', NBANDS, NBANDS, NBANDS, 1._q, CTMP, &
     &             NBANDS, CUNI, NBANDS, 0._q, CEIDB, NBANDS)
!=======================================================================
...
! CTMP = AUXTOT * U
         ...
         ENDDO
! CEIDB = U+ CTMP
         CALL PARALLEL_GGEMM( PGEMM_HANDLE, 'T', 'N', NBANDS, NBANDS, NBANDS, 1._q, CUNI, &
         &             NBANDS, CTMP, NBANDS, 0._q, CEIDB, NBANDS)
...
! CEIDB = U+ CHF
      CALL PARALLEL_GGEMM(PGEMM_HANDLE, 'T', 'N', NBANDS, NBANDS, NBANDS, 1._q, CUNI, &
           &             NBANDS, CHF, NBANDS, 0._q, CEIDB, NBANDS)

! CHF = (U+ CHF ) U
      CALL PARALLEL_GGEMM(PGEMM_HANDLE, 'N', 'N', NBANDS, NBANDS, NBANDS, 1._q, CEIDB, &
           &             NBANDS, CUNI, NBANDS, 0._q, CHF, NBANDS)
!=======================================================================
