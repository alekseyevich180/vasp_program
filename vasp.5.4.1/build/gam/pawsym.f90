...
!
      ...
      REAL(q) MAT(LMDIM,LMDIM,NIONS)
      TYPE (potcar) P(NTYP)

! local varibales
      REAL(q),ALLOCATABLE :: TMP(:,:,:),ROTMAT(:,:,:)
...
!
      INTEGER LMDIM      ! first dimension of MAT
      REAL(q) MAT(LMDIM,LMDIM,NIONS,3)
      TYPE (potcar) P(NTYP)

! local varibales
      REAL(q),ALLOCATABLE :: TMP(:,:,:,:),ROTMAT(:,:,:,:)
      REAL(q),ALLOCATABLE :: ROTMAT_TEMP(:,:,:),TROTMAT(:,:,:)
      REAL(q),ALLOCATABLE :: SL(:,:,:)
...
!-------------------------------------------------------------------
! subroutine to rotate 1._q matrix MAT and store result in ROTMAT
!-------------------------------------------------------------------
      ...
      REAL(q) :: MAT(LMDIM,LMDIM)    ! initial matrix
      REAL(q) :: ROTMAT(LMDIM,LMDIM) ! final matrix
