...
!***********************************************************************
  ...
  INTERFACE
     SUBROUTINE ASSIGN_TO_W1( W1, C, GPROJ)
       USE wave
       TYPE (wavefun1)    W1
       COMPLEX(q):: C
       REAL(q), OPTIONAL :: GPROJ
     END SUBROUTINE ASSIGN_TO_W1
  END INTERFACE
  
  INTERFACE
     SUBROUTINE W1_ASSIGN( W1, C, GPROJ)
       USE wave
       TYPE (wavefun1)    W1
       COMPLEX(q):: C
       REAL(q), OPTIONAL :: GPROJ
...
# 169
    ...
    IF (W1%WDES1%LGAMMA) THEN
       CALL DCOPY( W1%WDES1%NPROD, W1%GPROJ(1), 1,  W2%GPROJ(1), 1)
    ELSE
       CALL ZCOPY( W1%WDES1%NPROD, W1%GPROJ(1),  1, W2%GPROJ(1), 1)
    ENDIF
...
!***********************************************************************
  ...
  SUBROUTINE WA_COPY( W1, W2 )
    ...
    IF (SIZE(W1%GPROJ) /= SIZE(W2%GPROJ)) THEN
       WRITE(0,*)'internal error in WA_COPY: size mismatch in CPROJ',SIZE(W1%GPROJ), SIZE(W2%GPROJ)
       CALL M_exit(); stop
    ENDIF
    ...
    IF (W1%WDES1%LGAMMA) THEN
       CALL DCOPY( SIZE(W1%GPROJ), W1%GPROJ(1,1), 1,  W2%GPROJ(1,1), 1)
    ELSE
       CALL ZCOPY( SIZE(W1%GPROJ), W1%GPROJ(1,1), 1,  W2%GPROJ(1,1), 1)
    ENDIF
! remember WDES1
    W2%WDES1 =>W1%WDES1
! 1._q dimensional indexing assumed
    W2%FIRST_DIM=0
! remember spin index
    W2%ISP =W1%ISP
! set redistributed wavefunction indices
    IF (W2%WDES1%DO_REDIS) THEN
       ...
       CALL SET_GPOINTER(W2%CPROJ_RED, W2%WDES1%NPROD_RED,  W2%WDES1%NB_TOT, W2%GPROJ(1,1))
    ELSE
       W2%CW_RED=>W2%CPTWFP
       W2%CPROJ_RED=>W2%GPROJ
    ENDIF
...
!***********************************************************************
  ...
  SUBROUTINE W1_REDIS_PROJ( W1)
    ...
    IF (W1%WDES1%DO_REDIS) CALL REDIS_PROJ(W1%WDES1, 1, W1%GPROJ(1))
  ...
  SUBROUTINE WA_REDIS_PROJ( WA)
    ...
    IF (WA%WDES1%DO_REDIS) CALL REDIS_PROJ(WA%WDES1, SIZE(WA%GPROJ,2), WA%GPROJ(1,1))
...
!***********************************************************************
  ...
  SUBROUTINE W1_DAXPY( W1, SCALE, W2)
    ...
    IF (W1%WDES1%LGAMMA) THEN
       CALL DAXPY( W1%WDES1%NPRO, SCALE, W1%GPROJ(1), 1,  W2%GPROJ(1), 1)
    ELSE
       CALL DAXPY( W1%WDES1%NPRO*2, SCALE, W1%GPROJ(1),  1, W2%GPROJ(1), 1)
    ENDIF
...
! wavefunction complex, SCALE complex
       ...
       CALL ZAXPY( W1%WDES1%NPRO, SCALE, W1%GPROJ(1),  1, W2%GPROJ(1), 1)
    ENDIF
...
! since this function can be used to 0._q out an array we operate
! on all elements (dimension) and not only on those that are
! actually used
    CALL DSCAL( W1%WDES1%NRPLWV*2, SCALE, W1%CPTWFP(1), 1)

    IF (W1%WDES1%LGAMMA) THEN
       CALL DSCAL( W1%WDES1%NPROD, SCALE, W1%GPROJ(1), 1)
    ELSE
       CALL DSCAL( W1%WDES1%NPROD*2, SCALE, W1%GPROJ(1),  1)
    ENDIF
...
!***********************************************************************
  ...
  FUNCTION W1_DOT( W1, W2, CQIJ) RESULT (C)
    ...
    REAL(q), OPTIONAL :: CQIJ(:,:,:,:) ! optional overlap operator
...
! local
          ...
          DO NT=1,W1%WDES1%NTYP
             ...
             IF (LMMAXC/=0) THEN
                DO NI=NIS,W1%WDES1%NITYP(NT)+NIS-1
                   CALL ECCP_NL(LMDIM,LMMAXC,CQIJ(1,1,NI,1+ISPINOR_+2*ISPINOR),W2%GPROJ(NPRO_+1),W1%GPROJ(NPRO+1),CNL)
                   NPRO = LMMAXC+NPRO
                   NPRO_= LMMAXC+NPRO_
                ENDDO
             ENDIF
    ...
    CALL M_sum_d(W1%WDES1%COMM_INB, C, 1)
...
!***********************************************************************
!
! calculate the inproduct between 1._q wavefunction and
...
!***********************************************************************
  ...
  SUBROUTINE W1_GEMV( SCALEA, WA, W1, SCALEC, C, NC , CQIJ)
    IMPLICIT NONE
    REAL(q) :: SCALEA                  ! scaleing constant for WA x W1
    TYPE (wavefuna)    WA
    TYPE (wavefun1)    W1
    REAL(q) :: SCALEC                  ! scaling constant for C
    REAL(q) :: C(*)                    ! result
    INTEGER NC                      ! stride of C
    REAL(q), OPTIONAL :: CQIJ(:,:,:,:) ! optional overlap operator
! local
    INTEGER :: LMDIM, I
    REAL(q) CRESUL(WA%WDES1%NPRO)
    REAL(q) CTMP(SIZE(WA%CPTWFP,2))
    ...
    IF (PRESENT(CQIJ) .AND. WA%WDES1%LOVERL .AND.  WA%WDES1%NPROD>0 ) THEN
       LMDIM = SIZE(CQIJ,1)
       CALL OVERL1(WA%WDES1, LMDIM, CQIJ(1,1,1,1), CQIJ(1,1,1,1), 0.0_q, WA%GPROJ(1,1), CRESUL(1))

       IF (W1%WDES1%LGAMMA) THEN
          CALL DGEMV( 'T', WA%WDES1%NPRO,  SIZE(WA%CPTWFP,2), 1._q ,WA%GPROJ(1,1), &
               &           WA%WDES1%NPROD, CRESUL , 1 , 1._q,  CTMP(1), 1)
       ELSE
          CALL ZGEMV( 'C', WA%WDES1%NPRO,  SIZE(WA%CPTWFP,2), (1._q,0._q) ,WA%GPROJ(1,1), &
               &           WA%WDES1%NPROD, CRESUL , 1 , (1._q,0._q),  CTMP(1), 1)
       ENDIF

    ENDIF

    CALL M_sum_d(WA%WDES1%COMM_INB, CTMP, SIZE(WA%CPTWFP,2))
...
!***********************************************************************
  ...
  FUNCTION WA_FROM_WA( WA, N1, N2) RESULT (W1)
    ...
    W1%GPROJ =>WA%GPROJ(:,N1:N2)
    W1%WDES1 =>WA%WDES1
  END FUNCTION WA_FROM_WA

!***********************************************************************
!
! 1._q element from a WA array using two indices
! return WA(N1,N2)
!
!***********************************************************************
  ...
  FUNCTION W1_FROM_WA2( WA, N1, N2) RESULT (W1)
    ...
    W1%GPROJ =>WA%GPROJ (:,N1+ (N2-1)*WA%FIRST_DIM)
    W1%WDES1 =>WA%WDES1
    NULLIFY(W1%CR)
    W1%LDO=.TRUE.
...
!***********************************************************************
  ...
  FUNCTION WA_FROM_WA2( WA, N1, N12, N2) RESULT (W1)
    ...
    W1%GPROJ =>WA%GPROJ (:,N1+ (N2-1)*WA%FIRST_DIM:N12+ (N2-1)*WA%FIRST_DIM)
    W1%WDES1 =>WA%WDES1
...
!***********************************************************************
  ...
  FUNCTION ELEMENTS_SECOND( WA, N1) RESULT (W1)
    ...
    W1%GPROJ =>WA%GPROJ (:,N1::WA%FIRST_DIM)
    W1%WDES1 =>WA%WDES1
...
! set redistributed wavefunction indices
    IF (WDES1%DO_REDIS) THEN
       ...
       CALL SET_GPOINTER(WA%CPROJ_RED, WDES1%NPROD_RED,  W%WDES%NB_TOT, WA%GPROJ(1,1))
    ELSE
       WA%CW_RED=>WA%CPTWFP
       WA%CPROJ_RED=>WA%GPROJ
    ENDIF
...
!=======================================================================
  ...
  SUBROUTINE NEWWAVA(WA, WDES1, NDIM, NDIM2)
    ...
    IF (PRESENT(NDIM2)) THEN
       ALLOCATE(WA%CPTWFP(WDES1%NRPLWV,NDIM*NDIM2),WA%GPROJ(WDES1%NPROD,NDIM*NDIM2), &
            WA%CELEN(NDIM*NDIM2), WA%FERWE(NDIM*NDIM2), WA%AUX(NDIM*NDIM2))
       WA%FIRST_DIM=NDIM
       WA%CPTWFP=0
       WA%GPROJ=0
    ELSE
       ALLOCATE(WA%CPTWFP(WDES1%NRPLWV,NDIM),WA%GPROJ(WDES1%NPROD,NDIM), &
...
! set redistributed wavefunction indices
    IF (WDES1%DO_REDIS) THEN
       ...
       CALL SET_GPOINTER(WA%CPROJ_RED, WDES1%NPROD_RED,  NDIM*WDES1%NB_PAR, WA%GPROJ(1,1))
    ELSE
       WA%CW_RED=>WA%CPTWFP
       WA%CPROJ_RED=>WA%GPROJ
    ENDIF
...
!=======================================================================
!  create storage for 1._q wavefunction W array
...
!=======================================================================
  ...
  SUBROUTINE NEWWAVA_PROJ(WA, WDES1, NDIM)
    ...
    IF (PRESENT(NDIM)) THEN
       ALLOCATE(WA%GPROJ(WDES1%NPROD,NDIM))
    ELSE
       ALLOCATE(WA%GPROJ(WDES1%NPROD,WDES1%NBANDS))
    ENDIF
    ...
    IF (WDES1%DO_REDIS) THEN
       IF (PRESENT(NDIM)) THEN
          CALL SET_GPOINTER(WA%CPROJ_RED, WDES1%NPROD_RED,  NDIM*WDES1%NB_PAR, WA%GPROJ(1,1))
       ELSE
          CALL SET_GPOINTER(WA%CPROJ_RED, WDES1%NPROD_RED,  WDES1%NB_TOT, WA%GPROJ(1,1))
       ENDIF
    ELSE
       WA%CPROJ_RED=>WA%GPROJ
    ENDIF
...
!=======================================================================
!  set 1._q single wavefunction array (WA) from an array of wavefunctions
!=======================================================================
...
! set redistributed wavefunction indices
    IF (WDES1%DO_REDIS) THEN
       ...
       CALL SET_GPOINTER(WA%CPROJ_RED, WA%WDES1%NPROD_RED,  W%WDES%NB_TOT, WA%GPROJ(1,1))
    ELSE
       WA%CW_RED=>WA%CPTWFP
       WA%CPROJ_RED=>WA%GPROJ
    ENDIF
...
!=======================================================================
      ...
      DEALLOCATE(WA%CPTWFP,WA%GPROJ,WA%CELEN, WA%FERWE, WA%AUX)
...
!***********************************************************************
  ...
  FUNCTION PCPROJ( WA, N1, N2)
    ...
    PCPROJ=>WA%GPROJ(:,N1+ (N2-1)*WA%FIRST_DIM)
  END FUNCTION PCPROJ
...
!***********************************************************************
  ...
  SUBROUTINE ORTHON(NK, W, W1, CQIJ, ISP)
    ...
    INTEGER ISP
! local
    REAL(q) :: CPRO(W%WDES%NBANDS),CWORK(W%WDES%NPRO)
    REAL(q) :: WFMAG
    INTEGER :: I
    ...
    IF (W1%WDES1%LOVERL) THEN
       CALL OVERL1(W1%WDES1, SIZE(CQIJ,1),CQIJ(1,1,1,ISP),CQIJ(1,1,1,ISP), 0.0_q, W1%GPROJ(1),CWORK(1))
    ENDIF

    CALL DGEMV( 'T' , 2* W%WDES%NPLWKP(NK) , W%WDES%NBANDS ,1._q , W%CPTWFP(1,1,NK,ISP), &
         &             2* W%WDES%NRPLWV, W1%CPTWFP(1) , 1 , 0._q ,  CPRO(1), 1)

    IF (W1%WDES1%LOVERL) THEN
       IF (W%WDES%NPRO /= 0) &
            CALL DGEMV( 'T' ,  W%WDES%NPRO , W%WDES%NBANDS ,1._q , W%GPROJ(1,1,NK,ISP) , &
            W%WDES%NPROD, CWORK(1), 1 , 1._q ,  CPRO(1), 1)
    ENDIF

    CALL M_sum_d(W%WDES%COMM_KIN, CPRO(1), W%WDES%NBANDS)

    CALL DGEMM( 'N', 'N' , 2* W%WDES%NPLWKP(NK) , 1 , W%WDES%NBANDS , -1._q , &
         W%CPTWFP(1,1,NK,ISP), 2* W%WDES%NRPLWV , CPRO(1) , W%WDES%NBANDS , &
         1._q , W1%CPTWFP(1) , 2* W%WDES%NRPLWV )

    IF (W%WDES%NPRO /= 0) &
         CALL DGEMM( 'N', 'N' ,  W%WDES%NPRO , 1 , W%WDES%NBANDS  , -1._q , &
         W%GPROJ(1,1,NK,ISP) ,  W%WDES%NPROD , CPRO(1) , W%WDES%NBANDS , &
         1._q , W1%GPROJ(1) ,  W%WDES%NPROD  )
  ...
  SUBROUTINE ORTHON1P(NK, W, W1, CQIJ, ISP, NB)
    ...
    INTEGER ISP,NB(:)
! local
    REAL(q) :: CPRO(W%WDES%NBANDS,SIZE(W1)),CWORK(W%WDES%NPRO,SIZE(W1))
    REAL(q),ALLOCATABLE :: CW1(:,:),CPROJ1(:,:)
    REAL(q) :: WFMAG
    INTEGER :: I,BW
    ...
    DO I=1,SIZE(W1)
       IF (W1(I)%WDES1%LOVERL)THEN
          CALL OVERL1(W1(I)%WDES1, SIZE(CQIJ,1),CQIJ(1,1,1,ISP),CQIJ(1,1,1,ISP), 0.0_q, W1(I)%GPROJ(1),CWORK(1,I))
       ELSE
          CWORK(:,I)=0.0_q
       END IF
    END DO

    CALL DGEMM( 'T' , 'N', W%WDES%NBANDS, size(w1), 2* W%WDES%NPLWKP(NK), &
         & 1._q , W%CPTWFP(1,1,NK,ISP), 2* W%WDES%NRPLWV, CW1(1,1) , 2* W%WDES%NRPLWV, 0._q ,  CPRO(1,1), W%WDES%NBANDS)


    IF (W%WDES%NPRO /= 0) &
         CALL DGEMM( 'T' , 'N', W%WDES%NBANDS , size(w1), W%WDES%NPRO , 1._q ,&
         & W%GPROJ(1,1,NK,ISP) , W%WDES%NPROD, CWORK(1,1), W%WDES%NPRO, 1._q,  CPRO(1,1), W%WDES%NBANDS)

    CALL M_sum_d(W%WDES%COMM_KIN, CPRO(1,1), W%WDES%NBANDS*SIZE(W1))
    ...
    CALL DGEMM( 'N', 'N' , 2* W%WDES%NPLWKP(NK) , SIZE(W1) , W%WDES%NBANDS , -1._q , &
         W%CPTWFP(1,1,NK,ISP), 2* W%WDES%NRPLWV , CPRO(1,1) , W%WDES%NBANDS , &
         1._q , CW1(1,1) , 2* W%WDES%NRPLWV )

    IF (W%WDES%NPRO /= 0)THEN
       DO I=1,SIZE(W1)
          CWORK(:,i)=W1(i)%GPROJ(:)
        END DO
       CALL DGEMM( 'N', 'N' ,  W%WDES%NPRO , SIZE(W1) , W%WDES%NBANDS  , -1._q , &
            W%GPROJ(1,1,NK,ISP) ,  W%WDES%NPROD , CPRO(1,1) , W%WDES%NBANDS , &
            1._q , CWORK(1,1) ,  W%WDES%NPROD  )
    end IF
          ...
          DO ISPINOR_=0,W%WDES1%NRSPINORS-1
             ...
             DO NT=1,W%WDES1%NTYP
                ...
                DO NI=NIS,W%WDES1%NITYP(NT)+NIS-1
                   CALL ECCP_NL(SIZE(CQIJ,1),LMMAXC,CQIJ(1,1,NI,1+ISPINOR_+2*ISPINOR),W%GPROJ(NPRO_+1),W%GPROJ(NPRO+1),CP)
                   NPRO = LMMAXC+NPRO
                   NPRO_= LMMAXC+NPRO_
                ENDDO
...
!-----check that it is non-0._q
    IF(WFMAG<=0) THEN
!=======================================================================
! if it is smaller 0._q write a warning
!=======================================================================
    ...
    CALL DSCAL( W%WDES1%NPRO,WSCAL,W%GPROJ(1),1)
       ...
       spinor: DO ISPINOR=0,W%WDES1%NRSPINORS-1
          DO ISPINOR_=0,W%WDES1%NRSPINORS-1
             ...
             DO NT=1,W%WDES1%NTYP
                ...
                DO NI=NIS,W%WDES1%NITYP(NT)+NIS-1
                   CALL ECCP_NL(SIZE(CQIJ,1),LMMAXC,CQIJ(1,1,NI,ISP+ISPINOR_+2*ISPINOR),W%GPROJ(NPRO_+1),W%GPROJ(NPRO+1),CP)
                   NPRO = LMMAXC+NPRO
                   NPRO_= LMMAXC+NPRO_
                ENDDO
230             NIS = NIS+W%WDES1%NITYP(NT)
       ...
       spinor: DO ISPINOR=0,W1%WDES1%NRSPINORS-1
          DO ISPINOR_=0,W1%WDES1%NRSPINORS-1
             ...
             DO NT=1,W1%WDES1%NTYP
                ...
                DO NI=NIS,W1%WDES1%NITYP(NT)+NIS-1
                   CALL ECCP_NL(SIZE(CQIJ,1),LMMAXC,CQIJ(1,1,NI,1+ISPINOR_+2*ISPINOR),W1%GPROJ(NPRO_+1),W2%GPROJ(NPRO+1),CP)
                   NPRO = LMMAXC+NPRO
                   NPRO_= LMMAXC+NPRO_
                ENDDO
...
!************************* SUBROUTINE PROJCN ***************************
!
! this subroutine projects out from 1._q wavefunction
...
! local
    ...
    CSCPD= REAL(ZDOTC(W1%WDES1%NPL,W2%CPTWFP(1),1,W1%CPTWFP(1),1),KIND=q)
!=======================================================================
       ...
       spinor: DO ISPINOR=0,W1%WDES1%NRSPINORS-1
          DO ISPINOR_=0,W1%WDES1%NRSPINORS-1
             ...
             DO  NT=1,W1%WDES1%NTYP
                ...
                DO NI=NIS,W1%WDES1%NITYP(NT)+NIS-1
                   CALL ECCP_NL(SIZE(CQIJ,1),LMMAXC,CQIJ(1,1,NI,ISP+ISPINOR_+2*ISPINOR),W1%GPROJ(NPRO_+1),W2%GPROJ(NPRO+1),CADD)
                   NPRO = LMMAXC+NPRO
                   NPRO_= LMMAXC+NPRO_
                ENDDO
230             NIS = NIS+W1%WDES1%NITYP(NT)
...
!=======================================================================
    ...
    IF (W1%WDES1%LGAMMA) THEN
       CALL DAXPY(W1%WDES1%NPRO,-CSCPD,W2%GPROJ(1),1,W1%GPROJ(1),1)
    ELSE
       CALL ZAXPY(W1%WDES1%NPRO,-CSCPD,W2%GPROJ(1),1,W1%GPROJ(1),1)
    ENDIF
...
# 1518
          ...
          IF (W%WDES%LOVERL) CALL M_bcast_d_from(W%WDES%COMM_INTER,W1(NN)%GPROJ(1), &
...
!
  SUBROUTINE W1_GATHER_NEW( W, NB1, NB2, ISP, W1)
    ...
    REAL(q)       :: CPROJBUFF(W%WDES%NPROD,W%WDES%NB_PAR)      ! receive buffer for projectors
...
! distribute W1 to all nodes
       ...
       IF (W%WDES%DO_REDIS) THEN
          ...
          IF (W%WDES%LOVERL) CALL MPI_Allgather(W1(NN)%GPROJ(1), W%WDES%NPROD, MPI_double_precision, &
...
! store results back in correct storage position
          DO NN=1,W%WDES%NB_PAR
             ...
             W1((N-NB1)*NCPU+NN)%GPROJ(:)=CPROJBUFF(:,NN)
          ENDDO
       END IF
...
# 1689
          ...
          IF (W%WDES%LOVERL) CALL M_bcast_d_from(W%WDES%COMM_INTER,W1(N)%GPROJ(1), &
...
! set wave descriptor to current k-point for FFT
             ...
             IF (W%WDES%LGAMMA) THEN
                CALL DCOPY( W%WDES%NPROD, WAUX%GPROJ(1), 1,  WF(N_INTO_TOT-NB1+1,IK)%GPROJ(1), 1)
             ELSE
                CALL ZCOPY( W%WDES%NPROD, WAUX%GPROJ(1), 1,  WF(N_INTO_TOT-NB1+1,IK)%GPROJ(1), 1)
             ENDIF
          END DO
       ENDIF
    ENDDO
...
! the node that owns this band stores it onto CWORK
             IF (MOD(N_INTO_TOT-1,NCPU)+1==W%WDES%COMM_INTER%NODE_ME) THEN
                CWORK(1:SIZE(WF(N,IK)%CR))=WF(N,IK)%CR(:); GPROJ(1:SIZE(WF(N,IK)%GPROJ))=WF(N,IK)%GPROJ(:)
             ENDIF
! and broadcasts CWORK to all other nodes
             CALL M_bcast_z_from(W%WDES%COMM_INTER,CWORK(1),SIZE(CWORK),MOD(N_INTO_TOT-1,NCPU)+1)
             IF (W%WDES%LOVERL) THEN

                CALL M_bcast_d_from(W%WDES%COMM_INTER,GPROJ(1),SIZE(GPROJ),MOD(N_INTO_TOT-1,NCPU)+1)
# 1872
...
! but only the root nodes in COMM_SHMEM need to store CWORK back into WF
             IF (MOD(N_INTO_TOT-1,NCPU)+1/=W%WDES%COMM_INTER%NODE_ME.AND.W%WDES%COMM_SHMEM%NODE_ME==1) THEN
                WF(N,IK)%CR=CWORK(1:SIZE(WF(N,IK)%CR)); WF(N,IK)%GPROJ(:)=GPROJ(1:SIZE(WF(N,IK)%GPROJ))
             ENDIF
          ENDDO
       ENDDO
...
# 1943
             ...
             IF (W%WDES%LOVERL) CALL M_bcast_d_from(W%WDES%COMM_INTER,WF(N,IK)%GPROJ(1), &
                  W%WDES%NPROD,MOD(N_INTO_TOT-1,NCPU)+1)
...
!**********************************************************************
  ...
  SUBROUTINE W1_GATHER_ARRAY( W, NB1, NB2, ISP, W1, CR, GPROJ)
    TYPE (wavespin) W        ! wavefunction
    INTEGER :: NB1           ! starting band
    INTEGER :: NB2           ! final band
    INTEGER :: ISP           ! spin
    TYPE (wavefun1):: W1     ! array for FFT
    REAL(q)    :: CR(:,:)       ! collected real space orbitals
    REAL(q)    :: GPROJ(:,:)    ! collected projected orbitals
...
! local
    ...
    DO N=NB1,NB2
       ...
       IF (NN> SIZE(CR,2) .OR. NN >SIZE(GPROJ,2)) THEN
          WRITE(*,*) 'internal error in W1_GATHER_ARRAY: bound exceed ',NN, SIZE(CR,2),SIZE(GPROJ,2)
          CALL M_exit(); stop
       ENDIF
...
# 2020
          ...
          IF (W%WDES%LOVERL) CALL M_bcast_d_from(W%WDES%COMM_INTER,GPROJ(1,NN+(N-NB1)*NCPU), &
               W%WDES%NPROD,MOD(NN-1,NCPU)+1)
...
!**********************************************************************
  ...
  SUBROUTINE W1_GATHER_ARRAY_RECIPROCAL( W, NB1, NB2, ISP, W1, CG, GPROJ)
    TYPE (wavespin) W        ! wavefunction
    INTEGER :: NB1           ! starting band
    INTEGER :: NB2           ! final band
    INTEGER :: ISP           ! spin
    ...
    REAL(q)       :: CPROJBUFF(SIZE(GPROJ,1))  ! send buffer for projectors
...
! local
    ...
    IF (W%WDES%NRSPINORS/=1) THEN
       ...
       CALL M_exit(); stop
    ENDIF
    IF (SIZE(GPROJ,1) > SIZE(W1%GPROJ,1)) THEN
       WRITE (*,*) 'internal error in W1_GATHER_ARRAY_RECIPROCAL: CPROJ size inconsistent ', SIZE(GPROJ,1), SIZE(W1%GPROJ,1)
       CALL M_exit(); stop
    ENDIF
...
!these are local indices
       ...
       IF (NN> SIZE(CG,2) .OR. NN >SIZE(GPROJ,2)) THEN
          WRITE(*,*) 'internal error in W1_GATHER_ARRAY_RECIPROCAL: bound exceed ',NN, SIZE(CG,2),SIZE(GPROJ,2)
          CALL M_exit(); stop
       ENDIF
  
! copy data over to return arrays: CG and GPROJ
       CG(1:W1%WDES1%NPL ,NN)=W1%CPTWFP(1:W1%WDES1%NPL)
       CG(W1%WDES1%NPL+1:,NN)=0.0_q                 ! pad with 0._q

       GPROJ(:,NN)=W1%GPROJ(1:SIZE(GPROJ,1))
...
! MPI_IN_PLACE was tested but found to be much slower (code has been removed)
       IF (W%WDES%DO_REDIS) THEN
          ...
          IF (W%WDES%LOVERL) CALL MPI_Allgather(CPROJBUFF(1), SIZE(CPROJBUFF), MPI_double_precision, &
               GPROJ(1,(N-NB1)*NCPU+1), SIZE(GPROJ,1), MPI_double_precision, W%WDES%COMM_INTER%MPI_COMM, ierr)
# 2101
...
!
  ...
  SUBROUTINE W1_GATHER_ARRAY_RECIPROCAL_OLD( W, NB1, NB2, ISP, W1, CG, GPROJ)
    TYPE (wavespin) W        ! wavefunction
    INTEGER :: NB1           ! starting band
    INTEGER :: NB2           ! final band
    INTEGER :: ISP           ! spin
...
!these are local indices
       ...
       IF (NN> SIZE(CG,2) .OR. NN >SIZE(GPROJ,2)) THEN
          WRITE(*,*) 'internal error in W1_GATHER_ARRAY: bound exceed ',NN, SIZE(CG,2),SIZE(GPROJ,2)
          CALL M_exit(); stop
       ENDIF
...
!the first index GPROJ size can be smaller than the second 1._q (less cpus/tau than total number)
!but this copies only the data for valid indices in the first array (or?)
!!!test
!IF (SIZE(GPROJ,1).LT.SIZE(W1%GPROJ,1)) WRITE (*,*) NN, W1%GPROJ(SIZE(GPROJ,1)+1:)
       IF (SIZE(GPROJ,1).GT.SIZE(W1%GPROJ,1)) THEN
         WRITE (*,*) 'internal error in W1_GATHER_ARRAY: Sizes inconsistent ', SIZE(GPROJ,1), SIZE(W1%GPROJ,1)
       ENDIF
       GPROJ(:,NN)=W1%GPROJ(1:SIZE(GPROJ,1))
!IF (N.eq.NB1) WRITE (*,*) 'in gather array', SIZE(GPROJ,1), SIZE(GPROJ,2), SIZE(W1%GPROJ,1)
!!!test
...
# 2237
       ...
       IF (W%WDES%LOVERL) CALL M_bcast_d_from(W%WDES%COMM_INTER,WTMP%GPROJ(1), &
            W%WDES%NPROD,MOD(N_INTO_TOT-1,NCPU)+1)
...
# 2307
                   ...
                   IF (W%WDES%LOVERL) CALL M_recv_d(W%WDES%COMM_INTER, NN, & 
                        W1(N_INTO_TOT-NB1+1)%GPROJ(1), W%WDES%NPROD)
...
! it, 1._q explicitly select the node on which you gather the wave
...
# 2506
          ...
          IF (W%WDES%LOVERL) CALL M_bcast_d_from(W%WDES%COMM_INTER,W1(NN-NB1+1)%GPROJ(1), &
               W%WDES%NPROD,MOD(NN-1,NCPU)+1)
...
!
! search for position of G=0 0._q element
!
...
# 2715
    ...
    IF (PRESENT(GPROJ)) THEN
       IF (W1%WDES1%LGAMMA) THEN
          CALL DCOPY( W1%WDES1%NPROD, GPROJ(1), 1, W1%GPROJ(1), 1)
       ELSE
          CALL ZCOPY( W1%WDES1%NPROD, GPROJ(1), 1, W1%GPROJ(1), 1)
       ENDIF
    END IF
...
# 2745
    ...
    IF (PRESENT(GPROJ)) THEN
       IF (W1%WDES1%LGAMMA) THEN
          CALL DCOPY( W1%WDES1%NPROD,  W1%GPROJ(1), 1, GPROJ(1), 1)
       ELSE
          CALL ZCOPY( W1%WDES1%NPROD,  W1%GPROJ(1), 1, GPROJ(1), 1)
       ENDIF
    END IF
...
!***********************************************************************
  ...
  SUBROUTINE ECCP_NL(LMDIM,LMMAXC,CDIJ,CPROJ1,CPROJ2,CNL)
    ...
    REAL(q)      CNL
    INTEGER LMDIM, LMMAXC
    REAL(q) CDIJ(LMDIM,LMDIM)
    REAL(q) CPROJ1(LMMAXC),CPROJ2(LMMAXC)
! local
    INTEGER L, LP
    DO L=1,LMMAXC
       DO LP=1,LMMAXC
          CNL=CNL+CDIJ(LP,L)*CPROJ1(LP)*(CPROJ2(L))
       ENDDO
    ENDDO
  END SUBROUTINE ECCP_NL
...
!**********************************************************************
  ...
  SUBROUTINE OVERL(WDES1, LOVERL, LMDIM, CQIJ, CPROF, CRESUL)
    ...
    LOGICAL LOVERL
    INTEGER LMDIM
    REAL(q) CQIJ(LMDIM,LMDIM,WDES1%NIONS,WDES1%NRSPINORS*WDES1%NRSPINORS)
    REAL(q) CRESUL(WDES1%NPROD,WDES1%NBANDS),CPROF(WDES1%NPROD,WDES1%NBANDS)
! local
...
!
! F77 low level routine
! calculate the result of the overlap-operator acting onto 1._q
...
!
! N the ion index, nlm=i the index for the 1._q centre partial waves
!
!**********************************************************************
...
!
  SUBROUTINE OVERL1(WDES1, LMDIM, CDIJ, CQIJ, EVALUE, CPROF,CRESUL)
    ...
    REAL(q) :: EVALUE
    REAL(q) CRESUL(WDES1%NPRO),CPROF(WDES1%NPRO)
! local
