...
! initialise sum over Energy-change to 0._q
!=======================================================================
      NITER=4
...
!OCL NOVREC
      DO NPRO=1,WDES%NPRO
        WSEARCHL%GPROJ(NPRO)= WSEARCH%GPROJ(NPRO)
      ENDDO
...
!OCL NOVREC
      DO NPRO=1,WDES%NPRO
        WSEARCH%GPROJ (NPRO)=WSEARCH%GPROJ(NPRO)+ &
     &                        GAMMA*WSEARCHL%GPROJ(NPRO)
        WSEARCHL%GPROJ(NPRO)=WSEARCH%GPROJ(NPRO)
      ENDDO
...
!  This helps to save 1._q FFT (only) if we use the real space scheme
!=======================================================================
        IF (MOD(ITER,20)/=1) THEN
!DIR$ IVDEP
!OCL NOVREC
...
!----in case of serious convergence trouble 1._q might try this ???
...
!OCL NOVREC
      DO NPRO=1,WDES%NPRO
        W1%GPROJ(NPRO)= W1%GPROJ(NPRO)*DCOST+ WSEARCH%GPROJ(NPRO)*SINT
      ENDDO
