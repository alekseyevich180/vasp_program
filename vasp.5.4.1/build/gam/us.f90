...
!OCL NOVREC
        DO LP=1,LMMAXC
           CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)=CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)+ &
              WEIGHT*W%GPROJ(L+LMBASE,N,NK,ISP)*(W%GPROJ(LP+LMBASE_,N,NK,ISP))
        ENDDO
        ENDDO
...
!OCL NOVREC
                           DO LP=1,LMMAXC
                              CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)=CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)+ &
                                   WEIGHT*W1%GPROJ(L+LMBASE)*(W1%GPROJ(LP+LMBASE_))
                           ENDDO
                        ENDDO
...
!OCL NOVREC
        DO LP=1,LMMAXC
           CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)=CRHODE(LP,L,NI,ISP+ISPINOR_+2*ISPINOR)+ &
                WEIGHT1*W0%GPROJ(L+LMBASE,N,NK,ISP)*(W0%GPROJ(LP+LMBASE_,N,NK,ISP))+ &
                WEIGHT0*W1%GPROJ(L+LMBASE,N,NK,ISP)*(W0%GPROJ(LP+LMBASE_,N,NK,ISP))+ &
                WEIGHT0*W0%GPROJ(L+LMBASE,N,NK,ISP)*(W1%GPROJ(LP+LMBASE_,N,NK,ISP))
        ENDDO
        ENDDO
...
!
      ...
      REAL(q) CRHODE(LMDIM,LMDIM,NIONS,ISPIN)
      REAL(q) CDIJ(LMDIM,LMDIM,NIONS,ISPIN)
...
! CRHODE         1._q center occupancies
!
!***********************************************************************
...
! on return the following arrays are set
      REAL(q)     CRHODE(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ) ! occupancy of augmentation channels
...
!************************ SUBROUTINE MAXL1 *****************************
!
! calculate the maximum L quantum number for 1._q particular type
!
!***********************************************************************
...
! around 1._q ion using a cubic spline interpolation
...
!***********************************************************************
    ...
    SUBROUTINE SETDIJ_AVEC_(WDES, GRIDC_,GRIDUS,C_TO_US,LATT_CUR,P,T_INFO, LOVERL, &
      ...
      INTEGER  IRDMAX      ! allocation required for augmentation
      REAL(q)  CDIJ(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ)
      REAL(q),TARGET :: AVTOT_(GRIDC_%MPLWV*2,3)
...
! go to total and magnetization presentation for CDIJ
      ...
      IF (LADDITIONAL) THEN
         ALLOCATE(AVTOT(GRIDUS%MPLWV*2),CWORK(GRIDC_%MPLWV*2))
      ENDIF
...
!-----------------------------------------------------------------------
      ...
      CALL M_sum_d(GRIDC%COMM, CTMP, LMDIM*LMDIM*T_INFO%NIONS)
# 2091
...
! identical routine but with 1._q more argument A_ONE_CENTER
...
!***********************************************************************
    ...
    SUBROUTINE US_FLIP(WDES, LMDIM, CRHODE, LOVERL, LBACK)
      ...
      REAL(q) FAC
      REAL(q) :: CQU,CQD,C01,C10
      REAL(q) :: C11,C00,CX,CY,CZ
      REAL(q) :: CRHODE(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ)
...
# 2755
      ...
      ENDDO
! hard set of m_z to 0._q; this should not be necessary
...
!***********************************************************************
    ...
    SUBROUTINE AUGMENTATION_CHARGE( &
      ...
      INTEGER   LMDIM
      REAL(q)   CRHODE(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ)
      REAL(q),TARGET  :: CHTOT_(GRIDC_%MPLWV*2,WDES%NCDIJ)
      LOGICAL   LOVERL,LADDITIONAL
      REAL(q)   DISPL(3,T_INFO%NIONS)
!  work arrays
      TYPE (potcar), POINTER :: PP
      REAL(q),POINTER :: CHTOT(:)
      REAL(q)   RHOLM(256)
...
!=======================================================================
      ...
      CALL M_sum_d(WDES%COMM_INB,CTMP,LMDIM*LMDIM*T_INFO%NIONS*WDES%NCDIJ)
# 2948
...
!***********************************************************************
    ...
    SUBROUTINE CURRENT_AUGMENTATION( &
      ...
      INTEGER   LMDIM
      REAL(q)   CRHODE(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ)
      REAL(q),TARGET  :: JTOT_(GRIDC_%MPLWV*2,3)
...
! find the maximum L for augmentation charge (usually just 2 maximum l)
      ...
      ALLOCATE(JTOT(GRIDC%MPLWV*2),CTMP(LMDIM,LMDIM,T_INFO%NIONS))
!-----------------------------------------------------------------------
