...
! and 1._q center terms are not evaluated
...
! to allow use of 1._q-center dipole operators increase by 1._q
...
!*******************************************************************
!
! this subroutine calculates the 1._q-centre (on site) potentials
...
! (vasp.5.1: the latest potentials are constructed such that DEXC is essentially 0._q)
...
!-----------------------------------------------------------------------
! update reference 1._q centre AE potential
...
! if the PAW 1._q center terms are not updated:
...
! at this point POT contains the old "valence" only 1._q-electron potential
! calculate 1._q center contributions and store them in CHF
...
! get new 1._q center AE potential for atom ("valence only")
...
! exact exchange energy for 1._q-center AE wave functions
...
! free the 1._q-center terms so that AE-PS is calculated
! upon calling CALC_PAWFOCK
            IF (.NOT. USEFOCK_AE_ONECENTER()) CALL RELEASE_PAWFOCK

! difference between 1._q center AE and PS Fock energies
...
! this however is only allowed if the 1._q center terms
...
# 858
         ...
         ENDIF
!-----------------------------------------------------------------------
! update the local 1._q centre pseudo potential
!-----------------------------------------------------------------------
...
! double counting correction for PAW 1._q centre pseudo terms
...
! at this point POT contains the old valence only 1._q-electron potential
! add the core potential (PP%POTPSC-PP%POTPS, PP%POTPS is 0._q here)
...
! get new 1._q center potential for atom (valence only)
...
! 1._q-center terms over the procs in COMM_INB and COMM_INTER (=COMM_KIN).
...
! initialise the spin orbit contributions to D_ij to 0
         CSO=0
! Hartree Fock contribution set to 0._q
         CHF=0
...
! cl shifts DRHOCORE is 0._q here, since for the pseudo terms
