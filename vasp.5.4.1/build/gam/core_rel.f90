...
! this ion is mapped to type 0._q, which does not
! exist. Alternatively 1._q could consider mapping
...
! set it to the firstgrid point where all projectors are 0._q
            IF (LUSERWIGS()) THEN
               RWIGS=0._q
...
! 1._q difference; instead of using the actual onsite valence charge
...
! the channel, PP%QATO(channel,channel), differs from 0._q.
...
! inhomogenous solution using the first projector for this L
            ...
            OVL=0
! projectors should be 0._q at matching radius
...
! a particular site, using either the 1._q normalized augmentation
...
!                precision.  in particular,  rcond  is 0._q  if
...
!     estimate = norm(z)/norm(y) where  a*z = y  and  'T'(a)*y = e .
!     'T'(a)  is the transpose of a .  the components of  e  are
!     chosen to cause maximum local growth in the elements of w  where
!     'T'(u)*w = e .  the vectors are frequently rescaled to avoid
!     overflow.
!
!     solve 'T'(u)*w = e
!
...
!     error condition
!
!        a division by 0._q will occur if the input factor contains
!        a 0._q on the diagonal and the inverse is requested.
...
!                     indicate that dgesl or dgedi will divide by 0._q
...
!
         ...
         ipvt(k) = l
!
!        0._q pivot implies this column already triangularized
!
...
!        job     integer
!                = 0         to solve  a*x = b ,
!                = nonzero   to solve  'T'(a)*x = b  where
!                            'T'(a)  is the transpose.
!
!     on return
!
...
!     error condition
!
!        a division by 0._q will occur if the input factor contains a
!        0._q on the diagonal.  technically this indicates singularity
