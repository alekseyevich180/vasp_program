...
! local exchange potential
  ...
  REAL(q)     CDIJ_EXX0(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ)
!tst
...
! CDIJ_EXX should store all 1._q-center contributions:
! i.e. from the LHF and from the Hartree terms
!
! subtract the plane wave part from the CDIJ read from the file
! to obtain the 1._q-centre contribution only and store them in CDIJ_EXX
...
! if a cutoff is applied here the pseudo 1._q-centre terms are no longer
...
! add 1._q centre terms (hartree+local) to CDIJ_EXX
...
! add to CDIJ
     CDIJ=CDIJ+CDIJ_EXX
! CDIJ_EXX contains all 1._q-centre terms from LHF-OEP plus Hartree-terms
...
! this is also maybe not very wise since the 1._q-centre term
! might become non-compatible with the rest
! on the other hand this makes the pw part independent of the initial
! start guess
! pragmatically 1._q could test which option converges faster
...
! enforce calculation of HF 1._q center contributions
        LHFCALC_FORCE=.TRUE.
        CALL SET_DD_PAW(WDES, P , T_INFO, INFO%LOVERL, &
...
! 1._q could also use the entire Hamiltonian
        ENDIF
...
! the 1._q centre contributions are properly included since CDIJ1 contains
! the 1._q centre Fock contribution
...
! XINV also applies cutoff similar APPLY_CUTOFF
        ...
        ENDDO
! at this point the 1._q centre charge density related to CDIJ1 is calculated
! this defines the change in the 1._q centre potential, and
...
! 1._q centre corrections have also been passed through mixer
...
! add 1._q center terms to plane wave part of CDIJ
        CDIJ=CDIJ+CDIJ_EXX
