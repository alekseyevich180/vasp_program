...
!   the positions of all atoms of the rotated cell then 1._q of these   *
...
!   identical output we can simply perform a 1._q-by-1._q comparison of  *
...
!   There is just 1._q point 1._q has to care about: Supercells which    *
...
! Only if all TAUDIFs are 0._q TNTRAN (starting with the value TRUE)
...
! The first task will be to search the first atom with a non-vanishing moment
    ...
    3    CONTINUE
    4 CONTINUE
! all moments 0._q? -> no symmetry breaking!
    5 IF (TEST) THEN
         DO 7 ITRANS=1,NP
            DO 6 IROT=1,NROTK
...
! Apply all space group operations to the atomic coordinates:
         DO 11 IROT=1,NROTK
! Rotate the first atom of the first species having a non-0._q moment:
...
! Apply all primitive translations:
         DO 17 ITRANS=1,NP
! Translate the first atom of the first species having a non-0._q moment:
...
!         1._q might not need all the tables about atomic classes ... . *
...
!                                                                      *
!      ICL(NAX,NSX) counts the number of atoms within 1._q class        *
...
! Need at least four characters to store the group name ... :
     ...
     &                                                      LEN(SFNAME))
! Trivial case: Only 1._q symmetry operation (can only be E --> PGIND=1):
      IF (NROT==1) THEN
         PGIND=1
         SFNAME=GNAME(PGIND)
         RETURN
...
! Contains three mirror planes --> C_3v (PGIND=12):
         ...
         END IF
         IF (NC2==1) THEN
! Contains only 1._q twofold rotations --> C_6 (PGIND=21):
            PGIND=21
            SFNAME=GNAME(PGIND)
            RETURN
         END IF
         IF (NS2==1) THEN
! Contains only 1._q mirror plane --> C_3h (PGIND=22):
            PGIND=22
            SFNAME=GNAME(PGIND)
            RETURN
         END IF
...
! Contains three mirror planes --> D_2h (PGIND=8):
         ...
         END IF
         IF (NS2==1) THEN
! Contains 1._q mirror planes --> C_4h (PGIND=16):
            PGIND=16
            SFNAME=GNAME(PGIND)
            RETURN
         END IF
...
! Contains three mirror planes --> D_3d (PGIND=13):
         ...
         END IF
         IF (NS2==1) THEN
! Contains 1._q mirror planes --> C_6h (PGIND=23):
            PGIND=23
            SFNAME=GNAME(PGIND)
            RETURN
         END IF
...
! There is always 1._q trivial element: the idendity ...
      CALL SGRCOP(E,S(1,1))
      NTOT=1
! Take all generators ... :
      DO 80 IGEN=1,NGEN
...
! HH=SYMOPS_I*(SIG**IP)
   ...
   10       CONTINUE
            IF (J==1) N2=NNOW
    8    CONTINUE
! Products with more than 1._q sandwiched SIGMA-factor:
         M1=NTOT+1
         M2=NNOW
         DO 20 I=2,50
            DO N=NTOT+1,N2
...
!   WARNING: There is 1._q restriction: the two basis sets A and B must *
...
! and so it remains FALSE forever ... (if only 1._q position is not reproduced!).
! Only if all TAUDIFs are 0._q TNTRAN (starting with the value TRUE)
! will remain TRUE (that means we found an allowed symmetry operation).
               TNTRAN=TNTRAN.AND.((ABS(TAUDIF(1))<TINY) &
     &                            .AND.(ABS(TAUDIF(2))<TINY) &
