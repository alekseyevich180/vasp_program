...
!PK Probably skipping non-owned k-points is sufficient
      ...
      bands: DO IB=1,WDES%NBANDS
         ...
         DO NI=NIS,WDES1%NITYP(NT)+NIS-1
               ...
               DO LP=1,LMMAXC
                  RSUM_(1:3)=RSUM_(1:3)+ &
                 &  d_IJ(1:3,LP,L,NI,ISP+ISPINOR_+2*ISPINOR)*W1%GPROJ(LP+NPRO_)*(W1%GPROJ(L+NPRO))
               ENDDO
               ENDDO
...
!**********************************************************************
      ...
      SUBROUTINE SHIFT_PROJ(IG,CVALUE,WDES1,LOVERL,LATT_CUR,T_INFO,GPROJ)
      ...
      REAL(q) GPROJ(WDES1%NPROD,WDES1%NBANDS)
...
!OCL NOVREC
               DO L =1,LMMAXC
                  GPROJ(L+NPRO,NB)=GPROJ(L+NPRO,NB)*EXP(CGDR)*CVALUE
               ENDDO
               NPRO = LMMAXC+NPRO
            ENDDO
...
! the matrix \int Q_ij(r) dr
      ...
      REAL(q)    CHAM_(WDES%NB_TOT, WDES%NB_TOT)
      LOGICAL LOVERL, DO_REDIS

! work arrays (do max of 16 strips simultaneously)
      PARAMETER (NSTRIPD=16)

      TYPE (wavedes1)    WDES1,WDES2    ! descriptors for 1._q k-point
...
!  calculate D |cfin_n>   (D = \int Q_ij(r) e(i G r))
      CALL OVERL_BERRY(WDES, .TRUE.,LMDIM,ISP,CQIJ,W%GPROJ(:,:,NKP,ISP),CPROW)
...
# 1336
      ...
      REAL(q)                       CTMP(WDES%NB_TOT,WDES%NB_TOT)
