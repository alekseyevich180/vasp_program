...
!=======================================================================
      ...
      CALL M_sum_d(GRIDC%COMM, GTMP, LMDIM*LMDIM*T_INFO%NIONS*WDES%NCDIJ)      
# 305
...
!**********************************************************************
      ...
      TYPE (wavedes) WDES
      REAL(q) CRESUL(WDES%NPROD,WDES%NBANDS),CPROF(WDES%NPROD,WDES%NBANDS)
...
!  calculate D |cfin_n>   (D = \int Q_ij(r) e(i G r))
      CALL OVERL_WANNIER(WDES, .TRUE.,LMDIM,ISP,CDIJ, W%GPROJ(:,:,NK,ISP),CPROW)
...
!  Mind that all wavefunctions are redistributed at this point
      ...
      CALL M_sum_d(WDES%COMM,CHAM(1,1),NB_TOT*NB_TOT)
!-----------------------------------------------------------------------
! dump some arrays
!-----------------------------------------------------------------------
...
!***********************************************************************
      ...
      REAL(q) UI,UJ
      REAL(q) WEIGHT(6)      
      REAL(q)    U(WDES%NB_TOT,WDES%NB_TOT)
...
!-----------------------------------------------------------------------
     ...
     &     LMDIM, LOVERL, IRDMAX, IU0, IU6)
      ...
      REAL(q)       Z_(WDES%NB_TOT,WDES%NB_TOT)
...
!************************************************************************
     ...
     &   NIONS,POSION,MAP_,d_IJ)
      ...
      REAL(q)                       SUM(0:9),d_LM(256,0:9)
      LOGICAL                    LOVERL,LADDITIONAL

      REAL(q), ALLOCATABLE ::    XS(:),YS(:),ZS(:)
      REAL(q), ALLOCATABLE ::    DIST(:),DEP(:),YLM(:,:)

      REAL(q)                       d_IJ(LMDIM,LMDIM,0:9,NIONS,WDES%NCDIJ)
...
! test
     ...
     &   NIONS,POSION,MAP_)
      ...
      REAL(q)  CPROJ_BRA(WDES%NPROD)
      REAL(q)  CPROJ_KET(WDES%NPROD)
      ...
      spinor: DO ISPINOR=0,WDES%NRSPINORS-1
         ...
         DO NI=1,NIONS
            ...
            LMMAXC=WDES%LMMAX(NT)
            IF (LMMAXC==0) CYCLE
! Get position in GPROJ array for atom NIP
            NPRO=0; NI_=0
            IF (NT>1) THEN
               DO IT=1,NT-1
...
!                    write(71,'(7i7)') ni,map_(ni),nt,lmmaxc,im,l,lp,npro
                    ...
                    &   CPROJ_KET(LP+NPRO)*(CPROJ_BRA(L+NPRO))
!                   &   ABS(CPROJ_KET(LP+NPRO)*(CPROJ_BRA(L+NPRO)))
                  ENDDO
               ENDDO
            ENDDO
         ENDDO
...
! local variables
      ...
      REAL(q), ALLOCATABLE :: d_IJ(:,:,:,:,:)

      REAL(q)  EXPVAL(0:9,NOCC,NOCC,8)
...
# 2132
               ...
               IF (W%WDES%LOVERL) CALL M_bcast_d_from(WDES_STRIP%COMM_INTER,WSTRIP(N)%GPROJ(1), &
...
! calculate <Y_lm|O|Y_l'm'> where O=x,y,z,xx,yy,zz,xy,xz, and yz
                 ...
                 &   N1L,N1H,N2L,N2H,N3L,N3H,NIONS_,POSION_(1:3,1:NIONS_),MAP_(1:NIONS_),d_IJ)
                  ...
                  CALL ONSITE_RSPACE(WLOC%GPROJ(:),WSTRIP(N)%GPROJ(:),WDES,LMDIM, &
...
! Communicate
      CALL M_sum_d(WDES%COMM_INTER,EXPVAL(0,1,1,1),10*NOCC*NOCC*8)
