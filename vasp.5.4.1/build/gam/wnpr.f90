...
! be constructed (1._q entry for each of the WNPR_num_wann sites)
...
! even if we need them for only 1._q function, we simply generate
...
! redistribute over plane wave coefficients
            ...
            DO IBLK=0,WDESK%NPL_RED-1,NBLK
               ...
               CALL DGEMM(&
              &   'N','N',2* NBLK_ACT,WDESP%NB_TOT,WDES%NB_TOT, & 
              &   1._q,CBLOCK(1,1),2* NBLK,U(1,1),WDES%NB_TOT, &
              &   0._q,WAP%CW_RED(IBLK+1,1),2* WDESKP%NRPLWV_RED  &
              &   )
            ...
            IF (WDESK%NPRO_RED/=0) THEN
               ...
               DO IBLK=0,WDESK%NPRO_RED-1,NBLK
                 ...
                 &   1._q,GBLOCK(1,1),NBLK,U(1,1),WDES%NB_TOT, &
                 &   0._q,WAP%CPROJ_RED(IBLK+1,1),WDESKP%NPROD_RED  &
                 &   )
               ENDDO
            ENDIF
...
! local variables
      ...
      COMPLEX(q), ALLOCATABLE :: CBLOCK(:,:)
      REAL(q), ALLOCATABLE :: GBLOCK(:,:),COVL(:,:)
      INTEGER ISP,IK,NPOS
      ...
      spin: DO ISP=1,WDES%ISPIN
         kpoint: DO IK=1,WDES%NKPTS
            ...
            CALL OVERL(WDESKP,WDESKP%LOVERL,LMDIM,CQIJ(1,1,1,ISP),WAP%GPROJ(1,1),WOVL%GPROJ(1,1))
...
!           CALL DGEMM(&
!          &   'T','N',WDESK%NB_TOT,WDESKP%NB_TOT,2* WDESK%NPL_RED, &
!          &   1._q,WA%CW_RED,2* WDESK%NRPLWV_RED,WAP%CW_RED,2* WDESKP%NRPLWV_RED, &
!          &   0._q,COVL,WDESK%NB_TOT &
!          &   )
            ...
            DO ISTRIP=0,WDESK%NB_TOT-1,NSTRIP
               ...
               CALL DGEMM(&
              &   'T','N',NSTRIP_ACT,WDESKP%NB_TOT,2* WDESK%NPL_RED, &
              &   1._q,CBLOCK(1,1),2* WDESK%NPL_RED,WAP%CW_RED,2* WDESKP%NRPLWV_RED, &
              &   0._q,COVL(ISTRIP+1,1),WDESK%NB_TOT &
              &   )
...
!           CALL DGEMM(&
!          &   'T','N',WDESK%NB_TOT,WDESKP%NB_TOT,WDESK%NPRO_O_RED, &
!          &   1._q,WA%CPROJ_RED,WDESK%NPROD_RED,WOVL%CPROJ_RED,WDESKP%NPROD_RED, &
!          &   1._q,COVL,WDESK%NB_TOT &
!          &   )
            ...
            IF (WDESK%NPRO_O_RED/=0) THEN
               ...
               DO ISTRIP=0,WDESK%NB_TOT-1,NSTRIP
                  ...
                  CALL DGEMM(&
                 &   'T','N',NSTRIP_ACT,WDESKP%NB_TOT,WDESK%NPRO_O_RED, &
                 &   1._q,GBLOCK(1,1),WDESK%NPRO_O_RED,WOVL%CPROJ_RED,WDESKP%NPROD_RED, &
                 &   1._q,COVL(ISTRIP+1,1),WDESK%NB_TOT &
                 &   )
               ENDDO
               DEALLOCATE(GBLOCK)
            ENDIF

            CALL M_sum_d(WDES%COMM,COVL(1,1),WDES%NB_TOT*WDESP%NB_TOT)
...
! Make the desired linear combinations
            DO M=1,WDES%NB_TOT
               GPROJ=(0._q,0._q)
               DO N=1,(LMAX+1)**2
                  GPROJ=GPROJ+AP(M,N)*HYBRID_ORBITAL(N)
               ENDDO
               WNPR_projections(M,IK,ISP+ISPINOR-1,IFNC)=GPROJ
            ENDDO
...
!              CALL REDIS_PROJ(WDESK, WDES%NBANDS, W%GPROJ(1,1,IK,ISP))
!              CALL REDIS_PW  (WDESK, WDES%NBANDS, W%CPTWFP   (1,1,IK,ISP))
!
!              CALL REDIS_PROJ(WDESKP, WDESP%NBANDS, WP%GPROJ(1,1,IK,ISP))
!              CALL REDIS_PW  (WDESKP, WDESP%NBANDS, WP%CPTWFP   (1,1,IK,ISP))
!           ENDIF

!           CALL DGEMM(&
!          &   'N','N',2* WDESK%NPL_RED,WDESP%NB_TOT,WDES%NB_TOT, &
!          &   1._q,WA%CW_RED,2* WDESK%NRPLWV_RED,U(1,1),WDES%NB_TOT, &
!          &   0._q,WAP%CW_RED,2* WDESKP%NRPLWV_RED  &
!          &   )
...
!           CALL DGEMM(&
!          &   'N','N',WDESK%NPRO_RED,WDESP%NB_TOT,WDES%NB_TOT, &
!          &   1._q,WA%CPROJ_RED,WDESK%NPROD_RED,U(1,1),WDES%NB_TOT, &
!          &   0._q,WAP%CPROJ_RED,WDESKP%NPROD_RED  &
!          &   )
