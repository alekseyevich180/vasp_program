...
! choose 1._q of AUGER_EDENS and AUGER_HDENS, the other 1._q is determined automatically
...
!
        ...
        NULLIFY(WF%CPTWFP,WF%CR,WF%GPROJ)
        IF(ALLOC_CW)THEN
        ...
        END IF
        IF(ALLOC_CPROJ)THEN
           ALLOCATE(WF%GPROJ(WDES%NPROD,WDES%NBANDS,WDES%NKPTS))
        END IF
      ...
      SUBROUTINE DEALLOCATE_WAVEFUN(WF)
        ...
        IF(ASSOCIATED(WF%GPROJ))  DEALLOCATE(WF%GPROJ)
        NULLIFY(WF%CPTWFP,WF%CR,WF%GPROJ)
...
! local variables
        ...
        REAL(q), ALLOCATABLE :: M4O_eeh_dir(:,:,:,:),M4O_eeh_ex(:,:,:,:),&
                          ...
                          IF(LAUGER_COLLECT)THEN
                             IF (IK1==WDES%NB_LOW) THEN
                                DO I=NB1_START_eeh_COLLECT,NB1_STOP_eeh_COLLECT
                                   ...
                                   W1_eeh(I-NB1_START_eeh_COLLECT+1)%GPROJ=>WF_ALL(I,K1_COLLECT)%GPROJ(:)
                                END DO
                                DO I=NB3_START_eeh_COLLECT,NB3_STOP_eeh_COLLECT
                                   W3_eeh(I-NB3_START_eeh_COLLECT+1)%CR=>WF_ALL(I,K3_COLLECT)%CR(:)
                                   W3_eeh(I-NB3_START_eeh_COLLECT+1)%GPROJ=>WF_ALL(I,K3_COLLECT)%GPROJ(:)
                                END DO
                             END IF
                          ELSE
                          ...
                          IF(LAUGER_COLLECT)THEN
                             IF (IK1==WDES%NB_LOW) THEN
                                DO I=NB1_START_ehh_COLLECT,NB1_STOP_ehh_COLLECT
                                   ...
                                   W1_ehh(I-NB1_START_ehh_COLLECT+1)%GPROJ=>WF_ALL(I,K1_COLLECT)%GPROJ(:)
                                END DO
                                DO I=NB3_START_ehh_COLLECT,NB3_STOP_ehh_COLLECT
                                   W3_ehh(I-NB3_START_ehh_COLLECT+1)%CR=>WF_ALL(I,K3_COLLECT)%CR(:)
                                   W3_ehh(I-NB3_START_ehh_COLLECT+1)%GPROJ=>WF_ALL(I,K3_COLLECT)%GPROJ(:)
                                END DO
                             END IF
                          ELSE
! On all nodes: collect wavefunctions
...
! If there is at least 1._q K1-point that needs calculation,
...
! copy wavefunctions only on local node
                       DO I=NB2_START_eeh,NB2_STOP_eeh
                          ...
                          W2_eeh(I-NB2_START_eeh+1)%GPROJ=>WF_ALL(I,K2)%GPROJ(:)
                       END DO
                       DO I=NB4_START_eeh,NB4_STOP_eeh
                          W4_eeh(I-NB4_START_eeh+1)%CR=>WF_ALL(I,K4)%CR(:)
                          W4_eeh(I-NB4_START_eeh+1)%GPROJ=>WF_ALL(I,K4)%GPROJ(:)
                       END DO
                    END IF
!---------------------------------------------------------------------------
...
!       = 2( 2 |M_dir|^2 + 2 |M_ex|^2 + 2 Re[ M_dir*M_ex ] )
                    M4O_eeh=4._q*REAL( (M4O_eeh_dir)*M4O_eeh_dir + (M4O_eeh_ex)*M4O_eeh_ex - REAL((M4O_eeh_ex)*M4O_eeh_dir),q )
...
! copy wavefunctions only on local node
                       DO I=NB2_START_ehh,NB2_STOP_ehh
                          ...
                          W2_ehh(I-NB2_START_ehh+1)%GPROJ=>WF_ALL(I,K2)%GPROJ(:)
                       END DO
                       DO I=NB4_START_ehh,NB4_STOP_ehh
                          W4_ehh(I-NB4_START_ehh+1)%CR=>WF_ALL(I,K4)%CR(:)
                          W4_ehh(I-NB4_START_ehh+1)%GPROJ=>WF_ALL(I,K4)%GPROJ(:)
                       END DO
                    END IF
!---------------------------------------------------------------------------
...
!---------------------------------------------------------------------------
! direct and exchange contribution
                    M4O_ehh=4._q*REAL( (M4O_ehh_dir)*M4O_ehh_dir + (M4O_ehh_ex)*M4O_ehh_ex - REAL((M4O_ehh_ex)*M4O_ehh_dir),q )
...
!    i.e. only 1._q combination is actually calculated. Thus we cannot avoid the double
