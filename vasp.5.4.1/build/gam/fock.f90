...
! - 21.10.03 1._q center onsite PAW corrections included gK
...
! H phi
! throughout all routines (even if EXXOEP not 0._q)
...
! use Hartree Fock only in 1._q center AE terms
...
! works only in combination with at least 1._q NKRED set to 2
...
! In some routines like GW, RPA correlation, the 1._q center terms
! are not implemented and 1._q would like to restore the AE charge density
! very accurately on the plane wave grid.
! This has very significant performance impact, and since the 1._q
...
! structure to store the data layout of the 1._q-center charge densities
! in the HF routines
...
! NOTE: for Gamma only in real space the arrays must be declared as REAL(q)
...
! for each k-point 1._q value is stored
!
...
!
  TYPE fock_handle
     ...
     REAL(q), POINTER :: CKAPPA(:,:,:)     ! stores NL accelerations
     REAL(q), POINTER :: CDLM(:)
     REAL(q), POINTER :: CDIJ(:,:,:,:)
     TYPE (wavedes1) :: WDESK           !
     TYPE (wavefun1) :: W1              ! really only a dummy W1%GPROJ=> CDLM
...
! HF calculations 1._q center terms
...
!  generate the communication maps (patterns) for FFT
      ...
      IF (.NOT.GRIDHF%LREAL) THEN
         WRITE(0,*) 'internal error in SETUP_FOCK: GRIDHF is not real'
...
! set up AUG_DES describing data layout of 1._q center terms
         CALL SETUP_AUG_DES(GRIDHF, WDES_FOCK, AUG_DES, FAST_AUG_FOCK )
      ELSE
         ALLOCATE(AUG_DES%LMMAX(SIZE(WDES%LMMAX)))
! to make live easier set LMMAX (number of channels) to 0._q for each type
! also set NPRO, NPROD etc. to 0._q
         AUG_DES%LMMAX=0
         AUG_DES%NPRO =0
         AUG_DES%NPROD=0
         AUG_DES%NPRO_TOT=0
...
! 4 pi e^2 / (G+k+q)**2 * screening
!
! additionally set the 0._q grid point value to the convergence
...
! all k-points in the full grid have equal weight equal to first 1._q
...
! requires 1._q to pass down two k-points for which the Coulomb kernel is required
...
! I think it should be 0._q anyhow
!
!
!**********************************************************************
...
! additionally set the 0._q grid point value to FSG
...
! add contribution from 1._q over volume dependency of FSG
         FSG(1)=FSG(1)-FSG(0)/2
         FSG(3)=FSG(3)-FSG(0)/2
         FSG(6)=FSG(6)-FSG(0)/2
...
! selfenergy of an isolated erfc(r) charge distribution
         ...
         ELSE IF (L_THOMAS_FERMI) THEN
! Thomas Fermi, no analytical formula but small hence 0._q it
            FSG=0
         ELSE IF (LRHFCALC) THEN
            HFALPHAP=HFALPHA+1/HFSCREEN/HFSCREEN/4
!
...
!endtest
    ...
    REAL(q),ALLOCATABLE ::  GWORK(:) ! fock pot in real sp
...
! local variables
    ...
    INTEGER IERR
    LOGICAL LSHIFT
    REAL(q),      ALLOCATABLE :: CPROJKXYZ(:,:,:) ! stores ionic derivative of wave character of curr. strip
    REAL(q),      ALLOCATABLE :: CRHOLM(:)        ! augmentation occupancy matrix
    REAL(q),      ALLOCATABLE :: CDIJ(:,:,:,:,:)  ! D_lml'm'
    REAL(q),ALLOCATABLE,TARGET:: CDLM(:)          ! D_LM
...
! allocate memory
    ALLOCATE(POTFAK(GRIDHF%MPLWV,0:NDIR-3),FSG(0:NDIR-3), GWORK(2* GRIDHF%MPLWV))
...
# 3141
             ...
             CALL M_bcast_d_from(WDESK%COMM_INTER,WIN(N)%GPROJ(1), &
                  WHF%WDES%NPROD,MOD(N_-1,WHF%WDES%NB_PAR)+1)
             DO IDIR=1,NDIR
                CALL M_bcast_d_from(WDESK%COMM_INTER,CPROJKXYZ(:,N,IDIR), &
                  WHF%WDES%NPROD,MOD(N_-1,WHF%WDES%NB_PAR)+1)
             ENDDO
...
! add augmentation part to charge (if required)
             IF (WHF%WDES%LOVERL) THEN
                CALL DEPSUM_TWO_BANDS_RHOLM_TRACE(WIN(N)%GPROJ(:),WQ%GPROJ(:), WDESK, AUG_DES,  &
...
! calculate D_LM
             W1%GPROJ => CDLM(:) ! build the descriptor for RPRO1
...
# 3298
                      ...
                      CALL ECCP_NL_FOCK(LMDIM,LMMAXC,CDIJ(1,1,NI,1+ISPINOR,0),CDIJ(1,1,NI,1+ISPINOR,IDIR), &
                            WQ%GPROJ(LBASE+1),CPROJKXYZ(LBASE+1,N,IDIR),WIN(N)%GPROJ(LBASE+1),ENL(NI),WEIGHT)
...
!**********************************************************************
  ...
  SUBROUTINE FOCK_CHARGE( W1, W2, GCHG, CRHOLM)
    ...
    REAL(q) ::  GCHG(:)           ! accumulated charged
    REAL(q) ::  CRHOLM(:)         ! temporary
...
! add augmentation part to charge (if required)
    IF (W1%WDES1%LOVERL) THEN
       CALL DEPSUM_TWO_BANDS_RHOLM_TRACE(W1%GPROJ(:),W2%GPROJ(:), W1%WDES1, AUG_DES, &
...
! add augmentation part to charge (if required)
    IF (W1(N)%WDES1%LOVERL) THEN
       CALL DEPSUM_TWO_BANDS_RHOLM_TRACE(W1(N)%GPROJ(:),W2%GPROJ(:), W1(N)%WDES1, AUG_DES, &
...
! add augmentation part to charge (if required)
    IF (W1%WDES1%LOVERL) THEN
       CALL DEPSUM_TWO_BANDS_RHOLM_NO_CONJG(W1%GPROJ(:),W2%GPROJ(:), W1%WDES1, AUG_DES, &
            TRANS_MATRIX_FOCK, CRHOLM, 1._q, W1%WDES1%LOVERL)
...
! the first 1._q blocks over pairs of states
...
! local variables
    ...
    REAL(q),      ALLOCATABLE :: CKAPPA(:,:)      ! stores NL accelerations
    REAL(q),      ALLOCATABLE :: CRHOLM(:,:)      ! augmentation occupancy matrix
    REAL(q),      ALLOCATABLE :: CDIJ(:,:,:,:)    ! D_lml'm'
    REAL(q),ALLOCATABLE,TARGET:: CDLM(:,:)        ! D_LM
    REAL(q),   ALLOCATABLE :: POTFAK(:)        ! 1/(G+dk)**2 (G)
...
! applies to any orbital
          FSG_AEXX=0
       ELSE
! this 1._q is more accurate but works only for wavefunctions in the
...
! correct for self-interaction
                   IF (N_==(MQ-1)*W%WDES%NB_PAR+W%WDES%NB_LOW .AND. NQ==NK) THEN
                      ...
                      IF (W%WDES%COMM_INB%NODE_ME==1) THEN
                         EXX=EXX+FSG_AEXX*0.5_q  ! 1._q node adds corrections
                      ENDIF
# 3724
...
! add D_lml'm' to kappa_lm_N (sum over l'm')
                CALL OVERL_FOCK(WHF%WDES, LMDIM, CDIJ(1,1,1,1), WQ%GPROJ(1), CKAPPA(1,N),.TRUE.)
             ENDDO
          ENDIF
       ENDDO mband
    ENDDO qpoints
...
! collect CXI and CKAPPA
    IF (WHF%WDES%DO_REDIS) THEN
       ...
       CALL M_sum_d(WDESK%COMM_INTER,CKAPPA(1,1),WHF%WDES%NPROD*NGLB)
    END IF
...
! convergence correction non local part
             WQ%GPROJ(:)=FSG_AEXX*WHF%GPROJ(:,N_+NPOS-1,NK,ISP)*WHF%FERWE(N_+NPOS-1,NK,ISP)
             CALL OVERL1(WDESK, LMDIM, CQIJ(1,1,1,1), CQIJ(1,1,1,1), 0.0_q, WQ%GPROJ(1), WIN(1)%GPROJ(1))
             CKAPPA(:,N)=CKAPPA(:,N)+WIN(1)%GPROJ
          ENDIF
...
!**********************************************************************
  ...
  SUBROUTINE FOCK_CHARGE_NOINT( W1, W2, GCHG, CRHOLM, NCRHOLM)
    ...
    REAL(q) ::  CRHOLM(NCRHOLM)   ! work array
...
! add augmentation part to charge (if required)
    IF (W1%WDES1%LOVERL) THEN
       CALL DEPSUM_TWO_BANDS_RHOLM_TRACE_NOAE(W1%GPROJ(:),W2%GPROJ(:), W1%WDES1, AUG_DES, &
...
!   w1 * w2
  ...
  SUBROUTINE FOCK_CHARGE_NO_CONJG_NOINT( W1, W2, GCHG, CRHOLM, NCRHOLM)
    ...
    REAL(q) ::  GCHG(*)           ! accumulated charged
    INTEGER :: NCRHOLM         ! size of CRHOLM
    REAL(q) ::  CRHOLM(NCRHOLM)   ! temporary
...
! calculate the total charge density (i.e. plane wave +
! fast augmentation) from two wavefunctions  w1 x w2^*
! also set the 1._q center charge density
! function without explicit interface
!
!**********************************************************************
  ...
  SUBROUTINE FOCK_CHARGE_ONE_CENTER_NOINT( W1, W2, GCHG, H, CRHO_ONE_CENTER, CRHOLM, NCRHOLM)
    ...
    REAL(q) ::  CRHO_ONE_CENTER(H%TOTAL_ENTRIES) ! accumulated 1._q center charge
    INTEGER :: NCRHOLM         ! size of CRHOLM
    REAL(q) ::  CRHOLM(NCRHOLM)   ! work array
...
! add augmentation part to charge (if required)
    IF (W1%WDES1%LOVERL) THEN
       CALL DEPSUM_TWO_BANDS_ONE_CTR_TR(W1%GPROJ(:),W2%GPROJ(:), W1%WDES1, AUG_DES, &
...
! passed variables
    ...
    TYPE (latt) LATT_CUR
    TYPE (wavespin) W
    TYPE (nonlr_struct) NONLR_S, NONLR_D       ! second 1._q for derivatives
    TYPE (nonl_struct)  NONL_S, NONL_D         ! second 1._q for derivatives
    TYPE (potcar)      P(:)
...
! local variables
    ...
    INTEGER NQ, NQ_USED
    LOGICAL LSHIFT
    REAL(q)    :: GWORK (2* GRIDHF%MPLWV,W%WDES%NRSPINORS) ! fock pot in real sp
    REAL(q)    :: GWORK2(2* GRIDHF%MPLWV,W%WDES%NRSPINORS) ! fock pot in real sp
    COMPLEX(q),ALLOCATABLE :: CXI(:,:)         ! acc. in real space
    REAL(q),      ALLOCATABLE :: CKAPPA1(:,:)     ! stores NL accelerations
    REAL(q),      ALLOCATABLE :: CKAPPA2(:,:)     ! stores NL accelerations
    REAL(q),TARGET,ALLOCATABLE:: CPROJD(:,:)      ! derivatives of proj operators with respect to q
    REAL(q),      ALLOCATABLE :: CRHOLM(:)        ! augmentation occupancy matrix
    REAL(q),      ALLOCATABLE :: CDIJ(:,:,:,:)    ! D_lml'm'
    REAL(q),ALLOCATABLE,TARGET:: CDLM(:)          ! D_LM
...
! using strips of size m for the second band
    ...
    DO N=1,NGLB
       CALL NEWWAV(WIN(N) , WDESK,.TRUE.)
! WTMP is identical to WIN, except for GPROJ entry
! which will contain the derivative of the projectors w.r.t. k
       WTMP(N)=WIN(N)
       WTMP(N)%GPROJ => CPROJD(:,N)
    ENDDO
...
! add D_lml'm' to kappa_lm_N (sum over l'm')
                CALL OVERL_FOCK(WHF%WDES, LMDIM, CDIJ(1,1,1,1), WQ%GPROJ(1), CKAPPA1(1,N),.TRUE.)
             ELSE
                GWORK2=0
             ENDIF
...
! transform D_LM -> D_lml'm'
                CALL CALC_DLLMM_TRANS(WHF%WDES, AUG_DES, TRANS_MATRIX_FOCK, CDIJ,CDLM)
! add D_lml'm' to kappa_lm_N (sum over l'm')
                CALL OVERL_FOCK(WHF%WDES, LMDIM, CDIJ(1,1,1,1), WQ%GPROJ(1), CKAPPA2(1,N),.TRUE.)
             ENDIF
          ENDDO nband
       ENDDO mband
    ENDDO qpoints
...
! collect CXI and CKAPPA1 and CKAPPA2
    IF (WHF%WDES%DO_REDIS) THEN
       CALL M_sum_z(WDESK%COMM_INTER,CXI(1,1),GRID_FOCK%MPLWV*WHF%WDES%NRSPINORS*NGLB)
       CALL M_sum_d(WDESK%COMM_INTER,CKAPPA1(1,1),WHF%WDES%NPROD*NGLB)
       CALL M_sum_d(WDESK%COMM_INTER,CKAPPA2(1,1),WHF%WDES%NPROD*NGLB)
    END IF
...
!
! interface function that returns true if Hartree-Fock is used
! for the 1._q center AE term (1._q center HF treatment only)
!
!***********************************************************************
...
!***********************************************************************
!
! function to apply 1._q-center HF type Hamiltonians
!
!***********************************************************************
...
! for the 1._q centre augmentation charges
!
!***********************************************************************
...
!***********************************************************************
!
! interface function that returns true if the 1._q center
...
!**********************************************************************
      ...
      SUBROUTINE OVERL_FOCK(WDES, LMDIM, CDIJ, GPROJ , CRESUL, LADD)
      USE prec
      USE wave
...
! passed variables
      TYPE (wavedes) WDES
      REAL(q) CRESUL(WDES%NPROD),GPROJ(WDES%NPROD)
      LOGICAL LOVERL
      INTEGER LMDIM
      REAL(q) CDIJ(LMDIM,LMDIM,WDES%NIONS,WDES%NRSPINORS)
      LOGICAL LADD
! local variables
...
!**********************************************************************
    ...
    SUBROUTINE ECCP_NL_FOCK(LMDIM,LMMAXC,CDIJ0,CDIJ1, &
         GPROJ,CPROJ0,CPROJ1,ENL,WEIGHT)
      USE prec
      IMPLICIT NONE
      INTEGER LMMAXC, LMDIM
      REAL(q) ENL
      REAL(q) WEIGHT
      REAL(q) CDIJ0(LMDIM,LMDIM),CDIJ1(LMDIM,LMDIM)
      REAL(q) GPROJ(LMMAXC),CPROJ0(LMMAXC),CPROJ1(LMMAXC)
! local
      INTEGER L, LP
      REAL(q) ENL_ADD
...
!OCL NOVREC
         DO LP=1,LMMAXC
            ENL_ADD=ENL_ADD+REAL(GPROJ(LP)*(CDIJ0(LP,L)*(CPROJ0(L))+ &
                                            CDIJ1(LP,L)*(CPROJ1(L))))
         ENDDO
      ENDDO
...
!==========================================================================
    ...
    SUBROUTINE QUADRATIC_FIELD_CORRECTION(GRID, GWORK, LATT_CUR) 
      ...
      TYPE (grid_3d) GRID
      REAL(q)           GWORK(2* GRID%MPLWV)    ! fock pot in real sp
      TYPE (latt)    LATT_CUR
! local
...
! standard calling interface to the SET_GFAC routine
! requires 1._q to pass down two k-points for which the Coulomb kernel is required
! this 1._q is without explicitly compiled interface
!
!==========================================================================
