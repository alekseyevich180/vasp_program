...
! this subroutine integrates the entire calculation in 1._q
...
# 328
    ...
    ELSE
       ...
       WRITE(0,*) 'internal error in ROTLN_TRACE: LREAL is not set, but gamma only code'
       CALL M_exit(); stop
...
# 400
    ...
    ELSE
       ...
       WRITE(0,*) 'internal error in ROTLN: LREAL is not set, but gamma only code'
       CALL M_exit(); stop
...
! with a truncated  Coulomb kernel smoothly going from 1/G^2 to 0._q
...
! smoothly going from 1/G^2 to 0._q between ENCUTGWSOFT and ENCUTGW
!=======================================================================
...
! since the head stores the q^2, the final contribution is 0._q
! from the head
          IF (HFRCUT/=0) THEN
             POTFAK=0
...
! left multiply with RESPONSE
       CALL DGEMM( 'N', 'N', NP, NP,  NP, 1._q, CHI2, SIZE(CHI2,1), & 
            CHI_WORK(1,1), SIZE(CHI_WORK,1), 0._q, CHI3, SIZE(CHI3,1))

! right multiply with RESPONSE
       CALL DGEMM( 'N', 'N', NP, NP,  NP, 1._q, CHI3, SIZE(CHI3,1), & 
            CHI2(1,1), SIZE(CHI2,1), 0._q, CHI_WORK, SIZE(CHI_WORK,1))
    ENDIF
...
! left multiply with X W -> CHI2
       CALL DGEMM( 'N', 'N', NP, NP, NP, 1._q, CHI%RESPONSER(1,1,NOMEGA), SIZE(CHI%RESPONSER,1), & 
         CHI_WORK(1,1), SIZE(CHI_WORK,1), 0._q, CHI2, SIZE(CHI2,1))
! right multiply with (X W) X+
       CALL DGEMM( 'N', 'T', NP, NP, NP, 1._q, CHI2, SIZE(CHI2,1), & 
         CHI%RESPONSER(1,1,NOMEGA), SIZE(CHI%RESPONSER,1), SIZE(CHI2,1), 0._q, CHI3, SIZE(CHI3,1))
    ELSE
! left multiply with X W -> CHI2
       CALL DGEMM( 'N', 'N', NP, NP, NP, 1._q, CHI%RESPONSEFUN(1,1,NOMEGA), SIZE(CHI%RESPONSEFUN,1), & 
         CHI_WORK(1,1), SIZE(CHI_WORK,1), 0._q, CHI2, SIZE(CHI2,1))
! right multiply with (X W) X+
       CALL DGEMM( 'N', 'T', NP, NP, NP, 1._q, CHI2, SIZE(CHI2,1), & 
         CHI%RESPONSEFUN(1,1,NOMEGA), SIZE(CHI%RESPONSEFUN,1), 0._q, CHI3, SIZE(CHI3,1))
    ENDIF
...
! multiply by X+ (v X W) -> CHI2
    IF (CHI%LREALSTORE) THEN
       CALL DGEMM( 'T', 'N', NP, NP, NP, 1._q, CHI%RESPONSER(1,1,NOMEGA), SIZE(CHI%RESPONSER,1), & 
            CHI3, SIZE(CHI3,1), 0._q, CHI2, SIZE(CHI2,1))
    ELSE
       CALL DGEMM( 'T', 'N', NP, NP, NP, 1._q, CHI%RESPONSEFUN(1,1,NOMEGA), SIZE(CHI%RESPONSEFUN,1), & 
            CHI3, SIZE(CHI3,1), 0._q, CHI2, SIZE(CHI2,1))
    ENDIF
...
! quadratic terms are removed since the enter only in fourth order
    CALL DGEMM( 'N', 'N', NP, NP, NP, 1._q, CHI_WORK, SIZE(CHI_WORK,1), & 
         CHI2, SIZE(CHI2,1), 0._q, CHI3, SIZE(CHI3,1))
...
! left multiply with CHI_WORK:   v X W
    CALL DGEMM( 'N', 'N', NP, NP, NP, 1._q, CHI2, SIZE(CHI2,1), & 
         CHI_WORK(1,1), SIZE(CHI_WORK,1), 1._q, CHI3, SIZE(CHI3,1))

! add right multiply: W X+ v
    CALL DGEMM( 'N', 'T', NP, NP, NP, 1._q, CHI_WORK, SIZE(CHI_WORK,1), & 
         CHI2(1,1), SIZE(CHI2,1), 1._q, CHI3, SIZE(CHI3,1))
...
! the weight is set to 1._q usually
...
! add correction from integral of OMEGATL to infinity
               ...
               F=-(F/(PI)-1)
             ENDIF
!            if the integral is 1._q analytically, 1._q obtains the following result
...
! work arrays
    REAL (q) :: VKPT(3)
    TYPE (wavespin) WHF
    REAL(q), ALLOCATABLE :: GWORK(:)   ! charge from 1._q band in real space
    REAL(q), ALLOCATABLE :: GHTOT(:)   ! total charge
    REAL(q), ALLOCATABLE :: CRHOLM(:)  ! augmentation occupancy matrix
    TYPE( rotation_handle), POINTER :: ROT_HANDLE
...
! hence if GWORK is REAL 1._q needs to double the allocation
    ALLOCATE(GWORK(2* GRIDHF%MPLWV), & 
         GHTOT(2* GRIDHF%MPLWV), &
...
# 2124
    ...
    CALL M_sum_d(WHF%WDES%COMM_INTER, GHTOT(1), GRIDHF%RL%NP)
    CALL M_sum_d(WHF%WDES%COMM_KINTER, GHTOT(1), GRIDHF%RL%NP)
# 2139
