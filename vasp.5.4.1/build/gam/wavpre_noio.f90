...
!*********************************************************************
      ...
      REAL(q) CQIJ(LMDIM,LMDIM,T_INFO%NIOND,WDES%ISPIN)
...
! work arrays  (CPTWFP=CPTWFL)
      ...
      REAL(q),ALLOCATABLE,TARGET :: CPROW(:,:),CPRO0(:),CPRO1(:),CPROL(:)
      REAL(q),ALLOCATABLE :: CA(:,:),CAP(:,:),CU(:,:)
! redistributed plane wave coefficients
      COMPLEX(q), POINTER :: CW_RED(:,:)
      REAL(q)   , POINTER :: CPROW_RED(:,:),CPROJ_RED(:,:)
...
!-----------------------------------------------------------------------
      CU=0
!  caclulate Q |cfin_n>
      CALL OVERL(WDES1, LOVERL,LMDIM,CQIJ(1,1,1,1),W%GPROJ(1,1,NK,ISP),CPROW(1,1))
...
! calculate the matrix U(N,M) = <N,Mp>
      ...
      CALL ORTH2( &
     &  CW_RED(1,1),W_1%CPTWFP(1,1,NK,ISP),CPROW_RED(1,1),W_1%GPROJ(1,1,NK,ISP), &
     &  NB_TOT,1,NB_TOT,NPL,NPRO_O,NRPLWV_RED,NPROD_RED,CU(1,1))

      CALL M_sum_d(WDES%COMM_KIN,CU(1,1),NB_TOT*NB_TOT)
...
# 411
      ...
      CTMP=0._q
      CALL DGEMM( 'T', 'N', NB_TOT, NCOL(WDES%COMM_KIN%NODE_ME), NB_TOT, 1._q, CU, NB_TOT, &
     &            CU(1,1+OFFSET(WDES%COMM_KIN%NODE_ME)), NB_TOT, 0._q, CTMP, NB_TOT )
...
! Jacobi if available in parallel version
      IF ( LJACOBI ) THEN
          ...
          CALL M_sum_d(WDES%COMM_KIN, CAP, NB_TOT*NB_TOT)
...
# 536
      ...
      CU = TRANSPOSE( (CU) ); CTMP=0._q
      CALL DGEMM( 'N','N', NB_TOT, NCOL(WDES%COMM_KIN%NODE_ME), NB_TOT, 1._q, CA, NB_TOT, &
      &            CU(1,1+OFFSET(WDES%COMM_KIN%NODE_ME)), NB_TOT, 0._q, CTMP, NB_TOT )
...
# 564
      ...
      CTMP=0._q
      CALL DGEMM( 'T', 'N', NB_TOT, NCOL(WDES%COMM_KIN%NODE_ME), NB_TOT, 1._q, CAP, NB_TOT, &
     &            CA(1,1+OFFSET(WDES%COMM_KIN%NODE_ME)), NB_TOT, 0._q, CTMP, NB_TOT )
...
! rotate old wavefunction CPTWFP by multiplying with CAP
      CALL LINCOM('F',W_1%CPTWFP(:,:,NK,ISP),W_1%GPROJ(:,:,NK,ISP),CAP(1,1), &
     &      NB_TOT,NB_TOT, NPL,NPRO,NRPLWV_RED,NPROD_RED, NB_TOT, &
     &      W_1%CPTWFP(:,:,NK,ISP),W_1%GPROJ(:,:,NK,ISP))

! rotate last change of wavefunction by CAP
      CALL LINCOM('F',W_2%CPTWFP(:,:,NK,ISP),W_2%GPROJ(:,:,NK,ISP),CAP(1,1), &
     &      NB_TOT,NB_TOT,NPL,NPRO,NRPLWV_RED,NPROD_RED, NB_TOT, &
     &      W_2%CPTWFP(:,:,NK,ISP),W_2%GPROJ(:,:,NK,ISP))
...
!-----------------------------------------------------------------------
      ...
      CPROL(1:NPRO)      = W_1%GPROJ(1:NPRO,N,NK,ISP)
      CDCEL1             = W_2%CELTOT(N,NK,ISP)
      CD1(1:NPL)         = W_2%CPTWFP(1:NPL,N,NK,ISP)
      CPRO1(1:NPRO)      = W_2%GPROJ(1:NPRO,N,NK,ISP)
...
!-----------------------------------------------------------------------
      ...
      IF (LOVERL) THEN
      WFMAG =WFMAG + DDOT( NPRO_O,CPROW_RED(1,N),1,CPROL(1),1)
      WFMAGL=WFMAGL+ DDOT( NPRO_O,CPROJ_RED(1,N),1,CPROW_RED(1,N),1)
      ENDIF
      ...
      W_1%GPROJ(1:NPRO,N,NK,ISP) = CPROJ_RED(1:NPRO,N)
      W_2%CELTOT(N,NK,ISP)       = CDCEL0
      W_2%CPTWFP(1:NPL,N,NK,ISP)     = CD0(1:NPL)
      W_2%GPROJ(1:NPRO,N,NK,ISP) = CPRO0(1:NPRO)
...
! shift slot by 1._q and store predicted charge
               CHTOT_2(N,ISP)=CH0
