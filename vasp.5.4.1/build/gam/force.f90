...
! 1._q due to the tendency to put the charge density  at the largest
...
!=======================================================================
      ...
      row: DO N1=1,GRIDC%RC%NROW
        ...
        GSQU=GX**2+GY**2+GZ**2
        GMOD=SQRT(GSQU)
        PSCV=PSCV+FACTM* REAL( CONJG(CHTOT(NG))*CWORK1(NG) ,KIND=q)
!     G=0 part is handled somewhere else
        IF (GMOD>1E-7_q) THEN ! avoid division by 0

         PSCD= FACTM* REAL( CONJG(CHTOT(NG))*CWORK2(NG) ,KIND=q)
         ...
         DUM=FACTM* CHTOT(NG)*CONJG(CHTOT(NG))/GSQU
         DENC=DENC+DUM
...
! as in STRELO there is 1._q 1/ volume term plus other terms
!=======================================================================
      ...
      row: DO N1=1,GRIDC%RC%NROW
        ...
        GSQU=GX**2+GY**2+GZ**2
        GMOD=SQRT(GSQU)
        PSCV=PSCV+FACTM* REAL( CONJG(CHGGA(NG))*CWORK1(NG) ,KIND=q)
!=======================================================================
! avoid G=0
!=======================================================================
        IF(GMOD>1E-7_q) THEN
         PSCD= FACTM* REAL( CONJG(CHGGA(NG))*CWORK2(NG) ,KIND=q)
...
!=======================================================================
         ...
         FOR3=0
!=======================================================================
! CGXDX,Y,Z = I* the changes in the phase factor g.r on moving 1._q
...
!=======================================================================
           FOR=WORK(NG)*FACTM* AIMAG(CONJG(CHTOT(NG))*CEXPF)
...
!COMPLEX(q),  ALLOCATABLE ::  CWORK1(:),CWORK2(:)
    REAL(q),  ALLOCATABLE ::  CWORK1(:),CWORK2(:)
    INTEGER ::  NALLOC
    INTEGER :: NI,NT,NIS
...
!NALLOC=GRIDC%NGX*GRIDC%NGY
               NALLOC=GRIDC%MPLWV*2
               ALLOCATE(CWORK1(GRIDC%MPLWV*2),CWORK2(GRIDC%MPLWV*2))

! overlapping atomic charges to AECCAR1
! add overlapping atomic charges on dense regular grid
               CHTOT1=0;CHDEN1=0
! TB: 1._q of the following two calls  doesn't work if ADDGRID==.TRUE. !!!
...
!&    NALLOC,GRIDC%MPLWV*2,CWORK1,CWORK2,RELVOL,.FALSE.)
...
! a frequency used option, and the cost is less than 1._q SCF cycle
