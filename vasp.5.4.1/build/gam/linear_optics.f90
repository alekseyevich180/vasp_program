...
!
  REAL(qs), ALLOCATABLE,   SAVE :: CDER_BETWEEN_STATES(:,:,:,:,:)
!
...
!test
    ...
    CALL M_sum_single(WDES%COMM_KINTER, CHAM, SIZE(CHAM))
# 657
...
! local variables
      ...
      TYPE (wavedes1)    WDES1_W1       ! descriptor for 1._q k-point
      TYPE (wavefuna)    WA             ! array to store wavefunction
      REAL(q), ALLOCATABLE   :: CHAM_NORMAL(:,:)
!-----------------------------------------------------------------------
...
! redistribute the projected wavefunctions
      IF (WDES1%DO_REDIS) THEN
        ...
        CALL REDIS_PROJ(WDES1, W0%WDES%NBANDS, W0%GPROJ(1,1,NK,ISP))
        CALL REDIS_PW  (WDES1_W1, W1%WDES%NBANDS, W1%CPTWFP   (1,1,NK,ISP))
        CALL REDIS_PROJ(WDES1_W1, W1%WDES%NBANDS, W1%GPROJ(1,1,NK,ISP))
      ENDIF
      ...
      DO NPOS=1,NB_TOT_W1,NSTRIP
         ...
         CALL M_sum_d(WDES1%COMM_KIN,CHAM_NORMAL(1,1),NB_TOT*NSTRIP_ACT)
...
! triangle is correct, fill the upper 1._q
      DO  N1=1,NB_TOT_W1
...
!
    SUBROUTINE ROTATE_CDER_(WDES, CTRANS, NK, ISP)
      ...
      INTEGER :: NK, ISP
! local
      REAL(qs), ALLOCATABLE ::  C(:,:), CTRANS_single(:,:)
...
!
    SUBROUTINE ROTATE_CDER(WDES, CTRANS, NK, ISP)
      ...
      INTEGER :: NK, ISP
! local
      REAL(qs), ALLOCATABLE ::  C(:,:),C0(:,:), CTRANS_single(:,:)
      INTEGER :: NB_TOT, IDIR, NB
...
! overwrite with supplied matrix
      ...
      DO IDIR=1,3
         ...
         DO NB=1,NBANDS_CDER
            C0(NB,NBANDS_CDER+1:NB_TOT)=(CDER_BETWEEN_STATES(NBANDS_CDER+1:NB_TOT,NB,NK,ISP,IDIR))
         ENDDO
...
! second index is constrained to occupied bands
            A     =(CHAM1(N2,N1,NK,ISP))*CHAM2(N2,N1,NK,ISP)
        ...
        DO NK = 1, NKPTS
          DO N1 = 1, NBVAL
            DO N2 = 1, NBCON
              J = (N1 - 1) * NBCON + N2
              NC = NBVAL + N2
              AMP(J,NK,ISP) = (CHAM1(NC,N1,NK,ISP)) * CHAM2(NC,N1,NK,ISP) * CONST
...
! 1._q obtained by the Kramers-Kronig relation (Hilbert transform)
...
! local
      ...
      IF (IERR==0) THEN
         ...
         CALL M_bcast_s(WDES%COMM, CDER_BETWEEN_STATES(1,1,1,1,1), SIZE(CDER_BETWEEN_STATES))
# 2281
