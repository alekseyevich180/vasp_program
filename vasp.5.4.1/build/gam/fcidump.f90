...
# 761
      ...
      SUBROUTINE CONSTRUCT_IJAB(I,J,A,TWOE4ORB)
         ...
         if (ISP1 == ISP3) then
             ...
             CALL ZGEMM('T','N',(PROCS*WDES_MOD%NBANDS),(PROCS*WDES_MOD%NBANDS),NGVECTOR, &
             ...
             IF (ASSOCIATED(H)) then
                CALL ZGEMM('T','N',(PROCS*WDES_MOD%NBANDS),(PROCS*WDES_MOD%NBANDS),NHVECTOR, &
...
# 858
      ...
      SUBROUTINE TEST_IJAB(W)
         ...
         INTEGER :: I,J,A,IS,JS
         REAL(q) :: fock
         REAL(q) :: TE4O(WDES_MOD%NBANDS,WDES_MOD%NBANDS)
...
!               ENDDO
      ...
      SUBROUTINE TEST_IJAB_one_MP2(W,WDES) 
         ...
         REAL(q) :: TE4O(WDES_MOD%NBANDS,WDES_MOD%NBANDS),TE4O2
                  ...
                  DO B=1,PROCS*WDES_MOD%NBANDS*WDES_MOD%NKPTS*WDES%ISPIN
                     ...
                     IF (W%FERTOT(NI,NKI,ISP1)>0.1_q) then
                        ...
                        IF (W%FERTOT(NB,NKB,ISP2)<0.1_q) then
                           ...
                           hartree=hartree+TE4O(1,1)*(WDES_MOD%WTKPT(NKI))*(WDES_MOD%WTKPT(NKJ))
                           emp2=emp2+(TE4O(1,1)*((2.0_q,0.0_q)/(WDES%ISPIN)*TE4O(1,1)-TE4O2))/&
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      SUBROUTINE WRITE4INDEX2DISK(W,WDES)
         ...
         REAL(q) :: emp2,fock,hartree
         REAL(q) :: TE4O(WDES_MOD%NBANDS,WDES_MOD%NBANDS),TE4O2,FAC
         REAL(q) :: SCREENER,TMP
         INTEGER :: UNPAIRED
...
!      This is simply the period of the smallest non-0._q component
...
! Find the smallest non-0._q component of a wavevector in each
                  ...
                  kjloop: DO KJ=1,WDES%NKPTS
                     ...
                     DO NJ=1,VBMAX
                        ...
                        CALL PDGEMM('T','n',(PROCS*WDES%NBANDS),(PROCS*WDES%NBANDS),&
                          2* NGVECTOR,1._q, FTOD_PW(1,1,NI,KI,KQ,1,1),1,1,&
...
!mkj=2
               ...
               do mnb=VBMAX+1,PROCS*WDES%NBANDS
                  ...
                  do mnj=1,vbmax
                     do mng=1,ngvector
!testsum=testsum+conjg(FTOD_PW(mng,mna,mni,mki,mkq,1))-(FTOD_PW(mng,mni,mna,mka,mkq_,2))
!testsum=testsum+conjg(FTOD_PW(mng,mna,mni,mki,mkq,1))*FTOD_PW(mng,mnb,mnj,mkj,mkq,2)*KPOINTS_FULL%WTKPT(1)
                        testsum=testsum+(FTOD_PW(mng,mnb,mni,mki,mkq,1,1))*(FTOD_PW(mng,mnj,mna,mka,mkq,2,1))!*&
...
!testsum2=testsum2+conjg(FTOD_PW(mng,mni,mna,mka,mkq,1))*FTOD_PW(mng,mnj,mnb,mkb,mkq,2)!*KPOINTS_FULL%WTKPT(1)
                        testsum2=testsum2+(FTOD_PW(mng,mnj,mna,mka,mkq_,1,1))*(FTOD_PW(mng,mnb,mni,mki,mkq_,2,1))!*&
               ...
               DO NBI=1,WDES%NB_TOT
                  ...
                  DO NBA=1,WDES%NBANDS
                     ...
                     IF (rnba==NBI) THEN
                        ...
                        W%GPROJ(:,NBI,KI_INV,ISP)=(WI(1)%GPROJ(:))
                    ENDIF
                  ENDDO
               ENDDO
            ENDIF
