...
! if LZERO is set W2%GPROJ is set to 0._q prior to adding the term
!
!***********************************************************************
...
! local variables
      ...
      DO NK=1,WDES%NKPTS
         ...
         DO N=1,WDES%NBANDS
            CALL OVERL1(WDES1, LMDIM,CQIJ,CQIJ, 0.0_q, W1%GPROJ(1,N,NK,ISP),CRESUL)
            
            IF (LZERO) THEN
               DO M=1,WDES%NPRO
                  W2%GPROJ(M,N,NK,ISP)=CRESUL(M)
               ENDDO
            ELSE
               DO M=1,WDES%NPRO
                  W2%GPROJ(M,N,NK,ISP)=W2%GPROJ(M,N,NK,ISP)+CRESUL(M)
               ENDDO
            ENDIF
         ENDDO
      ENDDO
...
!              * f_nk * w_k
!  where phi(1)_nk^i is the wavefunction character stored in GPROJ
!
!***********************************************************************
    ...
    SUBROUTINE SUM_INPROD( WDES, W1, W2, SUM, LOVERL, LMDIM, CQIJ)
      ...
      REAL(q)    SUM
      INTEGER    LMDIM
      REAL(q), OPTIONAL :: CQIJ(LMDIM,LMDIM,WDES%NIONS,WDES%NRSPINORS*WDES%NRSPINORS)
! local variables
...
!***********************************************************************
    ...
    SUBROUTINE SETDIJ_R(WDES,GRIDC_,GRIDUS,C_TO_US,LATT_CUR,P,T_INFO, &
     &  LOVERL,LMDIM,d_IJ,IRDMAX,IDIR)
      ...
      INTEGER                    LMDIM 
      REAL(q)                    d_IJ(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ)
...
!=======================================================================
    ...
    REAL(q)       SV1(GRID%MPLWV*2,WDES%NCDIJ) ! derivative of potential on GRID
    REAL(q)       SV2(GRID%MPLWV*2,WDES%NCDIJ) ! work array
    INTEGER LMDIM
    REAL(q)  CDIJ1(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ) ! derivative of strength parameters
    REAL(q)  CDIJ2(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ)   ! work array
    REAL(q)  CQIJ(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ)
    REAL(q)  CRHODE(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ)  ! 1._q center occupancies
    REAL(q)  CRHODE1(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ) ! derivative of 1._q center occupancies
    REAL(q)  CRHODE_(LMDIM,LMDIM,WDES%NIONS,WDES%NCDIJ) ! work array
    INTEGER N_MIX_PAW
    REAL(q)  RHOLM(N_MIX_PAW,WDES%NCDIJ)      ! 1._q centre occupancies passed through mixer
    REAL(q)  RHOLM1(N_MIX_PAW,WDES%NCDIJ)     ! derivative of 1._q centre occupancies
...
! 1._q from the displaced ion, and 1._q from the change of the potential
! 1._q call to SETDIJ could be saved by calculating the first term
...
!
! setup index array NPRO_NI into the GPROJ array
                ...
                DO NT=1, WDES%NTYP
                   DO NI=NIS, WDES%NITYP(NT)+NIS-1
                      ...
                      CALL ROTATE_VECTOR_ADD( KPOINTS_TRANS%LINV(NK),RPHI_CPROJ(NPROP,N,NK_OLD,ISP_OLD,IDIR_OLD), &
                           W%GPROJ(NPRO,N,NK,ISP),MMAX,LDIM,SL(1,1,0),P(NT),FAKT)
                      NPRO= WDES%LMMAX(NT)+NPRO
                   ENDDO
                   NIS=NIS+WDES%NITYP(NT)
                ENDDO
...
! only 1._q ion is moved, set occupancies for other ions to 0
    NIP=NI_LOCAL(ION, WDES%COMM_INB)  ! local storage index in CRHODE

    CTMP=0
!  copy the 1._q center occupancy matrix to CTMP
    DO ISP=1,WDES%NCDIJ
       IF (NIP/=0) THEN
...
!***********************************************************************
    ...
    SUBROUTINE POTXC2(GRID,LATT_CUR, CHTOT0, DENCOR, CHTOT1, CWORK, E)
      ...
      REAL(q) CHTOT1(GRID%MPLWV*2)
      REAL(q) CHTOT0(GRID%MPLWV*2)
      REAL(q) CVTOT(GRID%MPLWV*2)
      REAL(q) CWORK(GRID%MPLWV*2),CHTOT0_(GRID%MPLWV*2)
