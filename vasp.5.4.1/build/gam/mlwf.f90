...
! Only 1._q node will do the actual work,
...
! whereas VASP uses the symmetry reduced 1._q.
...
! redistribute over plane wave coefficients
            IF (WDES%DO_REDIS) THEN
               CALL REDIS_PROJ(WDES1, WDES%NBANDS, W%GPROJ(1,1,NK,ISP))
...
! redistribute back over bands
      ...
      TYPE(wavedes) WDES
      TYPE(wavespin) W
      REAL(q) :: U(WDES%NB_TOT,WDES%NB_TOT,WDES%NKPTS,WDES%ISPIN)
! local variables
      TYPE(wavedes1) WDES1
      TYPE(wavefuna) WA
      INTEGER I,J,K,IP,IPP
      INTEGER ISP,NK,NKP,NW,NB,NBEXCL,NBwin
      REAL(q) :: UU(WDES%NB_TOT,WDES%NB_TOT)
...
!           WRITE(*,*) WDESK%NB_LOW+WDESK%NB_PAR*(NB-1),N,C
      ...
      CALL M_sum_d(WDESK%COMM_INTER,S(1,1),WDESK%NB_TOT*NYLM)
...
# 1995
            ...
            ELSE
               ...
               CALL M_exit(); stop
            ENDIF
            CALL M_sum_d(WP%WDES%COMM_INB,WVFN(1),NGX*NGY*NGZ)
              ...
              DO B1=1,W%WDES%NB_TOTK(IK,ISP)
                 DO B2=1,W%WDES%NB_TOTK(IK,ISP) 
                    DO IB=1,W%WDES%NB_TOTK(IK,ISP)
                       HAM(B2,B1,IK,ISP)=HAM(B2,B1,IK,ISP)+(U(IB,B2,IK,ISP))*W%CELTOT(IB,IK,ISP)*U(IB,B1,IK,ISP)
                    END DO
                 END DO
              END DO
           END DO kpoint1
