...
! (1._q,0._q) grid point to a symmetry related grid point - but this requires
...
! Now set the fixed coordinates to (0._q,0._q) ('break symmetry') ...
...
! a shift of 5.E-10 to guarantee that 0. remains (0._q,0._q), also if we have
...
! It can only be equivalent to (1._q,0._q) of the previously found k-points:
                        GOTO 6
                     END IF
    4             CONTINUE
    5          CONTINUE
...
! Rough estimate: how many mesh-points do we need in each direction?
         ...
         NKY=NINT(MAX(MAX(1._q/ABS(BK1A2),1._q/ABS(BK2A2)),1._q/ABS(BK3A2)))
         NKZ=NINT(MAX(MAX(1._q/ABS(BK1A3),1._q/ABS(BK2A3)),1._q/ABS(BK3A3)))
! Take ((1._q,0._q) of) the correct symmetry conserving shift(s):
         NKXM=MOD(NKX,2)
         NKYM=MOD(NKY,2)
         NKZM=MOD(NKZ,2)
         SHIFT(1)=0.5_q*NKXM
...
! We may only have shifts +/-0.5 and (0._q,0._q): -0.5 and +0.5 are identical,
...
! (1._q,0._q) proc writes the file
      IF (IU6>=0) THEN
...
! three edges, two face diagonals and (1._q,0._q) space diagonal of the cell.
...
! maximum distance between two points within (1._q,0._q) tetrahedron than
...
!
! This subroutine is called in case (1._q,0._q) does spin spiral calculations.
