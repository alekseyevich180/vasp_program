...
! only specify the non-(0._q,0._q) components ...
...
! Adjustment: Cosine between A1 and A2 shall be lower than (0._q,0._q), the
...
! Adjustment: Cosine(gamma) should be lower than (0._q,0._q), special axis
...
! orthogonal to (1._q,0._q) another and (adjustment!): |A1+A3|=|A2+A3|<|A1+A2|
            IF ((ABS(ABS1-ABS2)<TINY).AND. &
...
!              between A2 and A3 should be lower than (0._q,0._q).
...
! Further additional criterions are: (A1+A2), (A1+A3) and (A2+A3) are
! orthogonal to (1._q,0._q) another and (adjustment!): |A1+A2|>|A1+A3|>|A2+A3|
         IF ((ABS(ABS1-ABS2)<TINY).AND. &
     &            (ABS(ABS2-ABS3)<TINY).AND. &
...
! Now there exists only (1._q,0._q) further possibility - triclinic (IBRAV=14):
! Adjustment: All three cosines shall be greater than (0._q,0._q) and ordered:
...
! The algorithm is the following: Take first (1._q,0._q) vector and subtract or
...
! IFLAG will be set equal to (1._q,0._q) whenever a shorter vector was found
...
! Vector was not longer than the starting vector. Set ILOOP ... :
      ILOOP=1
      GOTO 100
! After leaving loop 1a we have subtracted A2 (1._q,0._q) times too often ... :
  102 CONTINUE
      ICOUNT=ICOUNT+1
      RB(1,1)=RB(1,1)+RB(1,2)
      RB(2,1)=RB(2,1)+RB(2,2)
...
! Found some shorter vector ? Then set IFLAG (and store the length):
         ...
         END IF
         GOTO 103
! After leaving loop 1b we have added A2 (1._q,0._q) times too often ... :
  105    CONTINUE
         ICOUNT=ICOUNT+1
         RB(1,1)=RB(1,1)-RB(1,2)
         RB(2,1)=RB(2,1)-RB(2,2)
...
!   -----------------                                                  *
!                                                                      *
!      TAU contains the atomic positions (for (1._q,0._q) species) to be       *
!        ordered.                                                      *
...
!     comparison of two different supercells by a simple (1._q,0._q)-to-(1._q,0._q)    *
...
!   LATTYP of this package. Therefore we only have to find (1._q,0._q)         *
...
!            primitive cells within (1._q,0._q) supercell.                     *
!                                                                      *
!                                                                      *
!***********************************************************************
...
! Order the atomic coordiantes (for this species) ... :
    ...
    2 CONTINUE
! Trivial case: There exists only (1._q,0._q) atom of species ISMIN. So we
...
! Only if all DIFs are (0._q,0._q) TNTRAN (starting with the value TRUE) will
