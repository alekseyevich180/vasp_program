...
! used from Alan Miller's webside
INTEGER, PARAMETER :: dp = 16
INTEGER, PARAMETER :: amax=33
INTEGER, PARAMETER :: bmax=33

CONTAINS


SUBROUTINE varpro (l, nl, n, nmax, lpp2, iv, t, y, w, ada, a,  &
                   iprint, alf, beta, itmax, itype, ierr, inu, nwrite)
...
REAL (dp), INTENT(IN OUT)  :: alf(amax)       ! alf(nl)
REAL (dp), INTENT(IN OUT)  :: beta(amax)
...
!DO b1=1,NL
!WRITE(100,'(I4,2F20.10)')NL,ALF(b1),BETA(b1)
!ENDDO
...
10 CALL vpdpa (l, nl, n, nmax, lpp2, iv, t, y, w, alf, ada, ierr,  &
               iprint, a, beta, a(:,lp1), r, itype, inu, nwrite)
gnstep = 1.0
iterin = 0
IF (iter > 0) GO TO 20
IF (nl == 0) GO TO 160
...
if (inu>=0 .and. nwrite >2) WRITE (inu,220) iterin,r
if (inu>=0 .and. nwrite >2) WRITE (inu,190) nu
!                              BEGIN TWO-STAGE ORTHOGONAL FACTORIZATION
20 CALL vpfac1 (nlp1, n, l, iprint, a(:,b1:), prjres, ierr, inu,nwrite)
IF (ierr < 0) GO TO 180
ierr = 2
IF (nu == 0.) GO TO 40
...
60 CALL vpdpa (l, nl, n, nmax, lpp2, iv, t, y, w, a(:,b1), ada, ierr,  &
               iprint, a, beta, a(:,lp1), rnew, itype, inu,nwrite)
IF (ierr /= 2) GO TO 180
iter = iter + 1
iterin = iterin + 1
skip = MOD(iter,modit) /= 0
IF (skip) GO TO 70
if (inu>=0 .and. nwrite >2) WRITE (inu,200) iter
if (inu>=0 .and. nwrite >2) WRITE (inu,240) a(1:nl,b1)
if (inu>=0 .and. nwrite >2) WRITE (inu,220) iterin, rnew

70 IF (iter < itmax) GO TO 80
ierr = -1
CALL vperr (iprint, ierr, 1, inu,nwrite)
...
100 nu = 1.5*nu
IF (.NOT.skip) then
  if (inu>=0 .and. nwrite >2) WRITE (inu,210) nu
endif
IF (nu <= 100.) GO TO 110
ierr = -2
CALL vperr (iprint, ierr, 1, inu,nwrite)
...
IF (skip) GO TO 150
if(inu>=0 .and. nwrite >2) WRITE (inu,190) nu
if(inu>=0 .and. nwrite >2) WRITE (inu,230) acum
150 ierr = 3
...
END IF
CALL vppost (l, nl, n, lnl2, eps1, r, iprint, alf, w, a, a(:,lp1), beta, inu, nwrite)
180 RETURN
...
SUBROUTINE vpfac1 (nlp1, n, l, iprint, b, prjres, ierr, inu,nwrite)
...
!                                                   COLUMN WAS ZERO
  ierr = -8
  CALL vperr (iprint, ierr, lp1+k, inu,nwrite)
  GO TO 70
...
SUBROUTINE vpdpa (l, nl, n, nmax, lpp2, iv, t, y, w, alf, ada,  &
                  isel, iprint, a, u, r, rnorm, itype, inu,nwrite)
...
INTEGER   :: i, inc(amax,bmax), j, k, kp1, ksub, m
...
CALL vpinit (l, nl, n, nmax, lpp2, iv, t, w, alf, ada, isel,  &
             iprint, a, inc, ncon, nconp1, philp1, nowate,itype, inu,nwrite)
IF (isel /= 1) GO TO 180
GO TO 30

10 CALL ada (lp1, nl, n, nmax, lpp2, iv, a, inc, t, alf, MIN(isel,3), itype)
IF (isel == 2) GO TO 20
!                                                 ISEL = 3 OR 4
firstc = lp2
lastc = lpp1
...
isel = -7
CALL vperr (iprint, isel, ncon, inu,nwrite)
GO TO 180
!                                                  ISEL = 1 OR 2
...
DO  k=1,l
  ...
  isel = -8
  CALL vperr (iprint, isel, k, inu,nwrite)
  GO TO 180
...
INTEGER, INTENT(OUT)       :: inc(amax,bmax)
...
IF (l >= 0 .AND. nl >= 0 .AND. l+nl <= n .AND. lnl2 <= lpp2 .AND.   &
    ...
    .AND. l == 0)) GO TO 10
isel = -4
CALL vperr (iprint, isel, 1, inu,nwrite)
GO TO 90
...
30 CALL ada (lp1, nl, n, nmax, lpp2, iv, a, inc, t, alf, isel, itype)
...
!                                                ERROR IN WEIGHTS
  isel = -6
  CALL vperr (iprint, isel, i,inu,nwrite)
  GO TO 90
  40 w(i) = SQRT(w(i))
END DO
...
ncon = nconp1-1
IF (iprint >= 0) then
  if (inu>=0 .and. nwrite >2) WRITE (inu,100) ncon
endif
...
IF (nl == 0) GO TO 70
isel = 4
CALL vpfac1 (nl+1, n, l, iprint, a(:,l+2:), prjres, isel, inu,nwrite)
DO  i=1,n
  a(i,lnl2) = r(i)
  DO  k=lp1,lnl1
    a(i,k) = a(i,k+1)
...
if ( inu>=0 .and. nwrite >2) WRITE (inu,90)
IF (l > 0 .and. nl > 0 .and. nl==l ) then
  if (inu>=0 .and. nwrite >2) WRITE(inu,'(" 0 WEIGHTS and ABSCISSAS:")')
   alf(1:nl)=abs(alf(1:nl))
   CALL PIKSRT2(nl,u)
   CALL PIKSRT2(nl,alf)
   DO i=1, nl
  if ( inu>=0 .and. nwrite >2) WRITE(inu,'(2F30.20)')u(i),alf(i)
   ENDDO 
else
   if ( inu>=0 .and. nwrite >2) write(*,*)'error in vppost nl/=l',nl,l
   stop
endif
if ( inu>=0 .and. nwrite >2) WRITE (inu,120) rnorm, SAVE, acum
IF (ABS(SAVE) > eps .and. inu>=0 .and. nwrite>2) WRITE (inu,130)
if (inu>=0 .and. nwrite >2) WRITE (inu,90)
80 RETURN
...
SUBROUTINE vperr (iprint, ierr, k, inu,nwrite)
...
if(inu>=0 .and. nwrite >2)    WRITE (inu,90)
  CASE (    2)
if(inu>=0 .and. nwrite >2)    WRITE (inu,100)
  CASE (    3)
    RETURN
  CASE (    4)
if(inu>=0 .and. nwrite >2)    WRITE (inu,110)
  CASE (    5)
if(inu>=0 .and. nwrite >2)    WRITE (inu,120)
  CASE (    6)
if(inu>=0 .and. nwrite >2)    WRITE (inu,130) k
  CASE (    7)
if(inu>=0 .and. nwrite >2)    WRITE (inu,140) k
  CASE (    8)
if(inu>=0 .and. nwrite >2)    WRITE (inu,150) k
END SELECT
...
END FUNCTION vpnorm
      ...
      FUNCTION random_vector(N,ISTART,IFIN)
         INTEGER      :: N,ISEED,I
         REAL         :: ISTART,IFIN,X
         REAL(dp)     :: random_vector(N)
...
! A is in the interval (0,1), scale the number to [A,B]
            random_vector(I)=REAL( X*(IFIN-ISTART)+ISTART,dp)
         ENDDO  

     ENDFUNCTION random_vector

      FUNCTION random_scalar(ISTART,IFIN)
         INTEGER      :: ISEED,I
         REAL         :: ISTART,IFIN,X
         REAL(dp)     :: random_scalar
...
!we need N random numbers in total
         CALL RANDOM_NUMBER(HARVEST=X)
! A is in the interval (0,1), scale the number to [A,B]
         random_scalar=REAL( X*(IFIN-ISTART)+ISTART,dp)

     ENDFUNCTION random_scalar
