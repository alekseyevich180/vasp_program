...
! get the index into the redistributed array
          ...
          CALL M_sum_z(WDES%COMM_KIN,COVL(1,NPOS_RED),NSUBD*NSIM_)
          CALL M_sum_z(WDES%COMM_KIN,CHAM(1,NPOS_RED),NSUBD*NSIM_)
...
! now broadcase to make sure all nodes use identical CEIG
          CALL M_bcast_z(WDES%COMM_KIN, CEIG, NSUBD*ITER*NSIM_)
...
!=======================================================================
          IF (LSTOP) THEN
             ...
             IF (WDES1%NPL_RED/=0) &
                  ...
                  &               WOPT%CW_RED(1,1),  WDES%NRPLWV_RED, CEIG(1,1), NSUBD,  &
...
!-----------------------------------------------------------------------
          ...
          CALL M_sum_z(WDES%COMM_KIN, CORTHO(1,1), NB_TOT*NSIM_)
          ...
          IF (WDES1%NPL_RED /=0 ) &
               CALL ZGEMM( 'N', 'N' ,  WDES1%NPL_RED , NSIM_ , NB_TOT , -(1._q,0._q) , &
               WA%CW_RED(1,1),  WDES%NRPLWV_RED , CORTHO(1,1) , NB_TOT , &
               (1._q,0._q) , WOPT%CW_RED(1,NPOS_RED) ,  WDES%NRPLWV_RED )
          
          IF (WDES1%NPRO_RED /= 0) &
               CALL ZGEMM( 'N', 'N' ,  WDES1%NPRO_RED , NSIM_ , NB_TOT  , -(1._q,0._q) , &
...
! sum subspace matrix over all nodes
          IF (.NOT. LscaAWARE) THEN
             CALL M_sum_z(WDES%COMM_KIN,CHAM_ALL(1,1),NB_TOT*NB_TOT)
! add lower triangle
             DO N=1,NB_TOT
                DO NP=N+1,NB_TOT
                   CHAM_ALL(NP,N)=CONJG(CHAM_ALL(N,NP))
                ENDDO
             ENDDO
          ENDIF
...
# 895
          ...
          CALL ZHEEVX( 'V', 'A', 'U', WDES%NB_TOTK(NK,ISP), CHAM_ALL(1,1) , NB_TOT, VL, VU, IL, IU, &
...
!CALL M_bcast_z(WDES%COMM, CHAM_ALL(1,1), NB_TOT*NB_TOT)
